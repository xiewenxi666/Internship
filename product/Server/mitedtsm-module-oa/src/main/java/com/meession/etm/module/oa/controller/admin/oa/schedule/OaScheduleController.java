package com.meession.etm.module.oa.controller.admin.oa.schedule;
...
import com.meession.etm.module.oa.controller.admin.oa.vo.schedule.OaScheduleRespVO;
...
public class OaScheduleController {
    ...
    public CommonResult<Boolean> update(@Valid @RequestBody OaScheduleCreateReqVO updateReqVO) {
        scheduleService.updateSchedule(BeanUtils.toBean(updateReqVO, OaScheduleDO.class));
        return success(true);
    }
    ...
    @PreAuthorize("@ss.hasPermission('oa:schedule:query')")
    @Operation(summary = "获得日程列表（按日期范围）")
    public CommonResult<List<OaScheduleRespVO>> listByDateRange(
        ...
        List<OaScheduleDO> list = scheduleService.getScheduleListByDateRange(getLoginUserId(), startTime, endTime);
