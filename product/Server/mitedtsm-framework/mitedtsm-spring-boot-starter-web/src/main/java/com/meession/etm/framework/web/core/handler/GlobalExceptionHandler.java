...
import static com.meession.etm.framework.common.exception.enums.GlobalErrorCodeConstants.*;
...
public class GlobalExceptionHandler {
    ...
    public CommonResult<?> serviceExceptionHandler(ServiceException ex) {
        ...
        // 使用 i18n 获取国际化错误消息
        return CommonResult.error(ex.getCode(), ex.getI18nMessage());
    }
    ...
    public CommonResult<?> defaultExceptionHandler(HttpServletRequest req, Throwable ex) {
        ...
        // 例如说：https://gitee.com/zhijiantianya/mitedtsm-cloud/issues/ICSSRM、https://gitee.com/zhijiantianya/mitedtsm-cloud/issues/ICT6FM
    ...
    private CommonResult<?> handleTableNotExists(Throwable ex) {
        ...
        if (message.contains("report_")) {
            log.error("[报表模块 mitedtsm-module-report - 表结构未导入]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[报表模块 mitedtsm-module-report - 表结构未导入]");
        }
        // 2. 工作流
        if (message.contains("bpm_")) {
            log.error("[工作流模块 mitedtsm-module-bpm - 表结构未导入]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[工作流模块 mitedtsm-module-bpm - 表结构未导入]");
        }
        // 3. 微信公众号
        if (message.contains("mp_")) {
            log.error("[微信公众号 mitedtsm-module-mp - 表结构未导入]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[微信公众号 mitedtsm-module-mp - 表结构未导入]");
        }
        // 4. 商城系统
        if (StrUtil.containsAny(message, "product_", "promotion_", "trade_")) {
            log.error("[商城系统 mitedtsm-module-mall - 已禁用]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[商城系统 mitedtsm-module-mall - 已禁用]");
        }
        // 5. ERP 系统
        if (message.contains("erp_")) {
            log.error("[ERP 系统 mitedtsm-module-erp - 表结构未导入]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[ERP 系统 mitedtsm-module-erp - 表结构未导入]");
        }
        // 6. CRM 系统
        if (message.contains("crm_")) {
            log.error("[CRM 系统 mitedtsm-module-crm - 表结构未导入]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[CRM 系统 mitedtsm-module-crm - 表结构未导入]");
        }
        // 7. 支付平台
        if (message.contains("pay_")) {
            log.error("[支付模块 mitedtsm-module-pay - 表结构未导入]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[支付模块 mitedtsm-module-pay - 表结构未导入]");
        }
        // 8. AI 大模型
        if (message.contains("ai_")) {
            log.error("[AI 大模型 mitedtsm-module-ai - 表结构未导入]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[AI 大模型 mitedtsm-module-ai - 表结构未导入]");
        }
        // 9. IoT 物联网
        if (message.contains("iot_")) {
            log.error("[IoT 物联网 mitedtsm-module-iot - 表结构未导入]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[IoT 物联网 mitedtsm-module-iot - 表结构未导入]");
        }
