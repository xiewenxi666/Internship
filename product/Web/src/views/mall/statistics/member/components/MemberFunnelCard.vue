    ...
    <div class="min-w-225 py-1.75" v-loading="loading">
      <div class="relative h-24 flex">
        <div class="h-full w-75% bg-blue-50 <lg:w-35% <xl:w-55%">
          <div class="ml-15 h-full flex flex-col justify-center">
            <div class="font-bold">
              {{ t('registerUserCount') }}：{{ analyseData?.comparison?.value?.registerUserCount || 0 }}
            </div>
            <div class="mt-2 text-3.5">
              {{ t('common.chainGrowthRate') }}：{{
      ...
      <div class="relative h-24 flex">
        <div class="h-full w-75% flex bg-cyan-50 <lg:w-35% <xl:w-55%">
          <div class="ml-15 h-full flex flex-col justify-center">
            <div class="font-bold">
              {{ t('activeUserCount') }}：{{ analyseData?.comparison?.value?.visitUserCount || 0 }}
            </div>
            <div class="mt-2 text-3.5">
              {{ t('common.chainGrowthRate') }}：{{
                calculateRelativeRate(
        ...
        <div class="w-75% flex bg-slate-50 <lg:w-35% <xl:w-55%">
          <div class="ml-15 h-full flex flex-row gap-x-16">
            <div class="flex flex-col justify-center">
              <div class="font-bold">
                {{ t('rechargeUserCount') }}：{{ analyseData?.comparison?.value?.rechargeUserCount || 0 }}
              </div>
              <div class="mt-2 text-3.5">
                {{ t('common.chainGrowthRate') }}：{{
                  calculateRelativeRate(
            ...
            <div class="flex flex-col justify-center">
              <div class="font-bold">{{ t('atv') }}：{{ fenToYuan(analyseData?.atv || 0) }}</div>
            </div>
          </div>
        </div>
        <div
