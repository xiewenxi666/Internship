...
<template>
  <el-container class="kefu">
    <el-header class="kefu-header">
      <div
        :class="{ 'kefu-header-item-activation': tabActivation('memberInfo') }"
        class="kefu-header-item cursor-pointer flex items-center justify-center"
        @click="handleClick('memberInfo')"
      >
        {{ t('memberInfo') }}
      </div>
      <div
        :class="{ 'kefu-header-item-activation': tabActivation('recentBrowse') }"
        class="kefu-header-item cursor-pointer flex items-center justify-center"
        @click="handleClick('recentBrowse')"
      >
        {{ t('recentBrowse') }}
      </div>
      <div
        :class="{ 'kefu-header-item-activation': tabActivation('tradeOrder') }"
        class="kefu-header-item cursor-pointer flex items-center justify-center"
        @click="handleClick('tradeOrder')"
      >
        {{ t('tradeOrder') }}
      </div>
    </el-header>
    <el-main class="kefu-content p-10px!">
      <div v-if="!isEmpty(conversation)" v-loading="loading">
        <!-- 基本信息 -->
        <UserBasicInfo v-if="activeTab === 'memberInfo'" :user="user" mode="kefu">
          <template #header>
            <CardTitle :title="t('basicInfo')" />
          </template>
        </UserBasicInfo>
        <!-- 账户信息 -->
        <el-card v-if="activeTab === 'memberInfo'" class="h-full mt-10px" shadow="never">
          <template #header>
            <CardTitle :title="t('accountInfo')" />
          </template>
      ...
      <div v-show="!isEmpty(conversation)">
        <el-scrollbar ref="scrollbarRef" always @scroll="handleScroll">
          <!-- 最近浏览 -->
          <ProductBrowsingHistory v-if="activeTab === 'recentBrowse'" ref="productBrowsingHistoryRef" />
          <!-- 交易订单 -->
          <OrderBrowsingHistory v-if="activeTab === 'tradeOrder'" ref="orderBrowsingHistoryRef" />
        </el-scrollbar>
      </div>
      <el-empty v-show="isEmpty(conversation)" :description="t('selectConversation')" />
    </el-main>
  </el-container>
</template>
...
const { t } = useI18n('mall.promotion.kefu') // 国际化

const activeTab = ref('memberInfo')
