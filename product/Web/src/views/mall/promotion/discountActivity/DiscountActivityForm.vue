<template>
  <Dialog v-model="dialogVisible" :title="dialogTitle" width="65%">
    ...
    >
      <!-- 先选择 -->
      <template #spuId>
        <el-button @click="spuSelectRef.open()">{{ t('mall.promotion.discount.selectProduct') }}</el-button>
        <SpuAndSkuList
          ...
          @delete="deleteSpu"
        >
          <el-table-column align="center" :label="t('mall.promotion.discount.discountAmountLabel')" min-width="168">
          ...
          <el-table-column align="center" :label="t('mall.promotion.discount.discountPercentLabel')" min-width="168">
            <template #default="{ row }">
              <el-input-number
...
const ruleConfig: RuleConfig[] = [
  {
    ...
    rule: (arg) => arg > 0,
    message: t('mall.promotion.coupon.minAmountError')
  }
]
...
const getSpuDetails = async (
  ...
  if (spuIds.value.includes(spuId)) {
    if (type !== 'load') {
      message.error(t('mall.promotion.discount.dataRepeatSelect'))
    }
    return
  }
