<template>
  <Dialog v-model="dialogVisible" :appendToBody="true" :title="dialogTitle" width="70%">
    <ContentWrap>
      <el-row :gutter="20" class="mb-10px">
        <el-col :span="6">
          <el-input
            ...
            class="!w-240px"
            clearable
            :placeholder="t('product.spu.namePlaceholder')"
        ...
        <el-col :span="6">
          <el-tree-select
            ...
            class="w-1/1"
            node-key="id"
            :placeholder="t('product.spu.categoryIdPlaceholder')"
          />
        </el-col>
        <el-col :span="6">
          <el-date-picker
      ...
      >
        <el-table-column v-if="isSelectSku" type="expand" min-width="30">
          <template #default>
            <SkuList
              v-if="isExpand"
              ref="skuListRef"
        ...
        <el-table-column align="center" :label="t('product.spu.salesCount')" min-width="90" prop="salesCount" />
        <el-table-column align="center" :label="t('product.spu.stock')" min-width="90" prop="stock" />
        <el-table-column align="center" :label="t('product.spu.sort')" min-width="70" prop="sort" />
        <el-table-column
...
const selectSku = (val: ProductSpuApi.Sku[]) => {
  ...
  if (selectedSpuId.value === 0) {
    message.warning(t('promotion.selectSpuFirst'))
...
const expandChange = async (row: ProductSpuApi.Spu, expandedRows?: ProductSpuApi.Spu[]) => {
  ...
  if (selectedSpuId.value !== 0) {
    if (row.id !== selectedSpuId.value) {
      message.warning(t('promotion.cancelSpuFirst'))
...
const confirm = () => {
  if (selectedSpuId.value === 0) {
    message.warning(t('promotion.noProductSelected'))
    return
  }
  if (props.isSelectSku && selectedSkuIds.value.length === 0) {
    message.warning(t('promotion.noSkuSelected'))
    return
  }
  // 返回各自 id 列表
  props.isSelectSku
...
/** 打开弹窗 */
const open = () => {
  dialogTitle.value = t('product.select.selectProduct')
