<template>
  <el-button class="ml-10px" type="text" @click="selectCoupon">{{ t('couponAdd') }}</el-button>
  ...
  >
    <div class="coupon-list-item-left flex items-center flex-wrap">
      <div class="mr-10px"> {{ t('couponName') }}：{{ item.name }}</div>
      <div class="mr-10px">
        {{ t('couponScope') }}：
    ...
    <div class="coupon-list-item-right">
      ...
      {{ t('couponCountUnit') }}
      <el-button class="ml-20px" link type="danger" @click="deleteCoupon(index)">{{ t('couponDelete') }}</el-button>
    </div>
  </div>
