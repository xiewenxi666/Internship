<template>
  <Dialog v-model="dialogVisible" :title="dialogTitle" width="65%">
    ...
    >
      <!-- 先选择 -->
      <template #spuId>
        <el-button @click="spuSelectRef.open()">{{ t('mall.promotion.seckill.selectProduct') }}</el-button>
        <SpuAndSkuList
        ...
        >
          <el-table-column align="center" :label="t('mall.promotion.seckill.seckillStock')" min-width="168">
            <template #default="{ row: sku }">
              <el-input-number v-model="sku.productConfig.stock" :min="0" class="w-100%" />
            </template>
          </el-table-column>
          <el-table-column align="center" :label="t('mall.promotion.seckill.seckillPriceLabel')" min-width="168">
            <template #default="{ row: sku }">
              <el-input-number
...
const ruleConfig: RuleConfig[] = [
  {
    ...
    rule: (arg) => arg >= 1,
    message: t('mall.promotion.seckill.stockRequired')
  },
  {
    name: 'productConfig.seckillPrice',
    rule: (arg) => arg >= 0.01,
    message: t('mall.promotion.seckill.seckillPriceRequired')
  }
]
