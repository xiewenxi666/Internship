<template>
  <Dialog v-model="dialogVisible" :title="dialogTitle" width="65%">
    ...
    >
      <!-- 先选择 -->
      <template #spuId>
        <el-button v-if="!isFormUpdate" @click="spuSelectRef.open()">{{ t('selectProduct') }}</el-button>
        <SpuAndSkuList
        ...
        >
          <el-table-column align="center" :label="t('redeemableStock')" min-width="168">
          ...
          <el-table-column align="center" :label="t('redeemableCount')" min-width="168">
            <template #default="{ row: sku }">
              <el-input-number v-model="sku.productConfig.count" :min="0" class="w-100%" />
            </template>
          </el-table-column>
          <el-table-column align="center" :label="t('requiredPoint')" min-width="168">
            <template #default="{ row: sku }">
              <el-input-number v-model="sku.productConfig.point" :min="0" class="w-100%" />
            </template>
          </el-table-column>
          <el-table-column align="center" :label="t('requiredAmount')" min-width="168">
            <template #default="{ row: sku }">
              <el-input-number
...
const ruleConfig: RuleConfig[] = [
  {
    ...
    rule: (arg) => arg >= 1,
    message: t('stockMinError')
  },
  {
    name: 'productConfig.point',
    rule: (arg) => arg >= 1,
    message: t('pointMinError')
  },
  {
    name: 'productConfig.count',
    rule: (arg) => arg >= 1,
    message: t('countMinError')
  }
]
