...
<template>
  <el-form ref="formRef" :model="formData" :rules="rules" label-width="auto" :disabled="isDetail">
    <el-row>
      <el-col :span="24">
        <el-form-item :label="t('delivery.deliveryType')" prop="deliveryTypes">
    ...
    <el-row>
      <el-col :span="24">
        <el-form-item
          :label="t('delivery.deliveryTemplate')"
          prop="deliveryTemplateId"
          v-if="formData.deliveryTypes?.includes(DeliveryTypeEnum.EXPRESS.type)"
        >
          <el-select :placeholder="t('delivery.selectDeliveryTemplatePlaceholder')" v-model="formData.deliveryTemplateId" class="w-80">
            <el-option
...
const validate = async () => {
  ...
  try {
    ...
    message.error(t('delivery.deliveryFormIncomplete'))
