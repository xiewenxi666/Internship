<template>
  <Dialog v-model="dialogVisible" :appendToBody="true" :title="t('select.selectSpec')" width="700">
    <el-table v-loading="loading" :data="list" show-overflow-tooltip :table-layout="'auto'">
      <el-table-column label="#" min-width="55">
      ...
      <el-table-column :label="t('sku.image')" min-width="80">
        <template #default="{ row }">
          <el-image
            :src="row.picUrl"
            class="h-30px w-30px"
      ...
      <el-table-column :label="t('select.specName')" align="center" min-width="80">
        <template #default="{ row }">
