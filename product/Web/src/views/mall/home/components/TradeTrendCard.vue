...
const timeRange = new Map()
  .set(TimeRangeTypeEnum.DAY30, {
    name: t('day30'),
    series: [
      { name: t('orderAmount'), type: 'bar', smooth: true, data: [] },
      { name: t('orderQuantity'), type: 'line', smooth: true, data: [] }
    ]
  })
  .set(TimeRangeTypeEnum.WEEK, {
    name: t('week'),
    series: [
      { name: t('lastWeekAmount'), type: 'bar', smooth: true, data: [] },
      { name: t('thisWeekAmount'), type: 'bar', smooth: true, data: [] },
      { name: t('lastWeekQuantity'), type: 'line', smooth: true, data: [] },
      { name: t('thisWeekQuantity'), type: 'line', smooth: true, data: [] }
    ]
  })
  .set(TimeRangeTypeEnum.MONTH, {
    name: t('month'),
    series: [
      { name: t('lastMonthAmount'), type: 'bar', smooth: true, data: [] },
      { name: t('thisMonthAmount'), type: 'bar', smooth: true, data: [] },
      { name: t('lastMonthQuantity'), type: 'line', smooth: true, data: [] },
      { name: t('thisMonthQuantity'), type: 'line', smooth: true, data: [] }
    ]
  })
  .set(TimeRangeTypeEnum.YEAR, {
    name: t('year'),
    series: [
      { name: t('lastYearAmount'), type: 'bar', smooth: true, data: [] },
      { name: t('thisYearAmount'), type: 'bar', smooth: true, data: [] },
      { name: t('lastYearQuantity'), type: 'line', smooth: true, data: [] },
      { name: t('thisYearQuantity'), type: 'line', smooth: true, data: [] }
    ]
  })
/** 图表配置 */
const eChartOptions = reactive<EChartsOption>({
  ...
  toolbox: {
    feature: {
      ...
      saveAsImage: { show: true, name: t('tradeTrend') } // 保存为图片
    }
  },
  tooltip: {
    trigger: 'axis',
      ...
      formatter: (date: string) => {
        switch (timeRangeType.value) {
          ...
          case TimeRangeTypeEnum.WEEK:
            ...
            if (weekDay == '0') weekDay = t('sunday')
            return t('weekDay') + weekDay
          case TimeRangeTypeEnum.MONTH:
            return formatDate(date, 'D')
          case TimeRangeTypeEnum.YEAR:
            return formatDate(date, 'M') + t('monthUnit')
          default:
            return date
        }
      }
