  ...
  <ContentWrap>
    ...
    >
        ...
        <el-col :span="8">
          <el-form-item :label="t('device.device.nickname')" prop="nickname">
            <el-input
              ...
              :placeholder="t('common.inputNickname')"
              clearable
              @keyup.enter="handleQuery"
              class="!w-240px"
            />
      ...
      <el-row :gutter="20">
        <el-col :span="8">
          <el-form-item :label="t('common.deviceType')" prop="deviceType">
            <el-select
              ...
              :placeholder="t('common.selectDeviceType')"
              clearable
              class="!w-240px"
            >
              <el-option
        ...
        <el-col :span="8">
          <el-form-item :label="t('device.device.deviceStatus')" prop="status">
            <el-select
              v-model="queryParams.status"
              :placeholder="t('common.selectDeviceStatus')"
              clearable
              class="!w-240px"
            >
              <el-option
        ...
        <el-col :span="8">
          <el-form-item :label="t('device.device.group')" prop="groupId">
            <el-select
              v-model="queryParams.groupId"
              :placeholder="t('device.group.selectGroup')"
              clearable
              class="!w-240px"
            >
              <el-option
      ...
      <el-row>
        <el-col :span="24">
          <el-form-item>
            ...
            >
              <Icon icon="ep:folder-add" class="mr-5px" /> {{ t('device.group.addToGroup') }}
            </el-button>
            <el-button
              type="danger"
              plain
  ...
  <ContentWrap>
              ...
              <div class="flex items-center text-[14px]">
                <div class="flex-1">
                  <div class="mb-2.5 last:mb-0">
                    <span class="text-[#717c8e] mr-2.5">{{ t('device.device.product') }}</span>
                    <el-link class="text-[#0070ff]" @click="openProductDetail(item.productId)">
                      {{ products.find((p) => p.id === item.productId)?.name }}
                    </el-link>
                  </div>
                  <div class="mb-2.5 last:mb-0">
                    <span class="text-[#717c8e] mr-2.5">{{ t('common.deviceType') }}</span>
                    <dict-tag :type="DICT_TYPE.IOT_PRODUCT_DEVICE_TYPE" :value="item.deviceType" />
                  </div>
                  <div class="mb-2.5 last:mb-0">
                    <span class="text-[#717c8e] mr-2.5">{{ t('device.device.nickname') }}</span>
                    <span
    ...
    <el-table
     ...
     :table-layout="'auto'">
      ...
      <el-table-column :label="t('device.device.nickname')" align="center" prop="nickname" />
      <el-table-column :label="t('device.device.product')" align="center" prop="productId">
        <template #default="scope">
      ...
      <el-table-column :label="t('common.deviceType')" align="center" prop="deviceType">
        <template #default="scope">
          <dict-tag :type="DICT_TYPE.IOT_PRODUCT_DEVICE_TYPE" :value="scope.row.deviceType" />
        </template>
      </el-table-column>
      <el-table-column :label="t('device.device.groups')" align="center" prop="groupId">
        <template #default="scope">
      ...
      <el-table-column :label="t('device.device.deviceStatus')" align="center" prop="status">
        <template #default="scope">
          <dict-tag :type="DICT_TYPE.IOT_DEVICE_STATE" :value="scope.row.status" />
        </template>
      </el-table-column>
      <el-table-column
        :label="t('device.device.onlineTime')"
        align="center"
        prop="onlineTime"
...
const handleExport = async () => {
  try {
    ...
    download.excel(data, t('common.exportDevice') + '.xls')
  } catch {
  } finally {
