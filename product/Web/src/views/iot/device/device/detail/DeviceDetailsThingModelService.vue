...
<template>
  ...
  <ContentWrap>
    ...
    <el-table v-loading="loading" :data="list" :stripe="true" :show-overflow-tooltip="true" :table-layout="'auto'">
      <el-table-column label="调用时间" align="center" prop="requestTime" min-width="180">
        <template #default="scope">
          {{ scope.row.request?.reportTime ? formatDate(scope.row.request.reportTime) : '-' }}
        </template>
      </el-table-column>
      <el-table-column label="响应时间" align="center" prop="responseTime" min-width="180">
        <template #default="scope">
          {{ scope.row.reply?.reportTime ? formatDate(scope.row.reply.reportTime) : '-' }}
        </template>
      </el-table-column>
      <el-table-column label="标识符" align="center" prop="identifier" min-width="160">
        <template #default="scope">
      ...
      <el-table-column label="服务名称" align="center" prop="serviceName" min-width="160">
        <template #default="scope">
          {{ getServiceName(scope.row.request?.identifier) }}
        </template>
      </el-table-column>
      <el-table-column label="调用方式" align="center" prop="callType" min-width="100">
        <template #default="scope">
