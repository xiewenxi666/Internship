...
<template>
  <ContentWrap>
    ...
    <el-table
     ...
     :table-layout="'auto'">
      ...
      <el-table-column :label="t('common.productName')" align="center" prop="productName" />
      <el-table-column :label="t('device.device.deviceStatus')" align="center" prop="state">
        <template #default="{ row }">
  ...
  <!-- 添加子设备弹窗 -->
  <Dialog :title="t('common.add')" v-model="bindDialogVisible" width="900px">
    <ContentWrap>
      <!-- 搜索区域 -->
      <el-form :model="bindQueryParams" ref="bindQueryFormRef" :inline="true" class="-mb-15px">
        ...
        <el-form-item :label="t('device.device.name')" prop="deviceName">
          <el-input
    ...
    <template #footer>
      <el-button type="primary" @click="handleBindSubmit" :loading="bindFormLoading">
        {{ t('common.ok') }} ({{ bindSelectedIds.length }})
      </el-button>
...
const handleBindSubmit = async () => {
  if (bindSelectedIds.value.length === 0) {
    message.warning(t('common.selectPlaceholder'))
    return
  }
  bindFormLoading.value = true
  try {
