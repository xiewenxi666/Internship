  ...
  <ContentWrap>
    ...
    <el-card class="mb-16px" shadow="never">
      <el-form
        ...
        @submit.prevent
      >
        <el-form-item :label="t('rule.scene.name')">
          <el-input
            v-model="queryParams.name"
            :placeholder="t('rule.scene.namePlaceholder')"
            clearable
        ...
        >
          <div class="flex items-center">
            ...
            <div>
              ...
              <div class="text-14px text-[#909399] mt-4px">{{ t('rule.scene.triggers') }}</div>
            </div>
          </div>
        </el-card>
      </el-col>
        ...
        >
          <div class="flex items-center">
            ...
            <div>
              ...
              <div class="text-14px text-[#909399] mt-4px">{{ t('common.enable') }}</div>
            </div>
          </div>
        </el-card>
      </el-col>
        ...
        >
          <div class="flex items-center">
            ...
            <div>
              ...
              <div class="text-14px text-[#909399] mt-4px">{{ t('common.disable') }}</div>
            </div>
          </div>
        </el-card>
      </el-col>
        ...
        >
          <div class="flex items-center">
            ...
            <div>
              ...
              <div class="text-14px text-[#909399] mt-4px">{{ t('rule.scene.timer.title') }}</div>
            </div>
          </div>
        </el-card>
      </el-col>
    ...
    <el-card class="mb-20px" shadow="never">
      <el-table v-loading="loading" :data="list" stripe @selection-change="handleSelectionChange" :table-layout="'auto'">
        ...
        <!-- 触发条件列 -->
        <el-table-column :label="t('rule.scene.triggers')" min-width="280">
          <template #default="{ row }">
            <div class="space-y-4px">
              ...
              <div v-if="hasTimerTrigger(row)" class="mt-4px">
                ...
                <div v-if="getNextExecutionTime(row)" class="text-12px text-[#909399] mt-2px">
                  ...
                  {{ t('common.next') }}: {{ formatDate(getNextExecutionTime(row)!) }}
                </div>
              </div>
            </div>
          </template>
        ...
        <!-- 执行动作列 -->
        <el-table-column :label="t('rule.scene.actions')" min-width="250">
          <template #default="{ row }">
            <div class="flex flex-wrap gap-4px">
        ...
        <el-table-column :label="t('common.createTime')" prop="createTime" min-width="180">
          <template #default="{ row }">
...
const getRuleSceneSummary = (rule: IotSceneRule) => {
  const triggerSummary =
    rule.triggers?.map((trigger: any) => {
      ...
      // 添加设备信息（如果有）
      if (trigger.deviceId) {
        description += ` [${t('device.device.title')} ID: ${trigger.deviceId}]`
      } else if (trigger.productId) {
        description += ` [${t('common.product')} ID: ${trigger.productId}]`
      }
      return description
    }) || []
  ...
  const actionSummary =
    rule.actions?.map((action: any) => {
      ...
      // 添加设备信息（如果有）
      if (action.deviceId) {
        description += ` [${t('device.device.title')} ID: ${action.deviceId}]`
      } else if (action.productId) {
        description += ` [${t('common.product')} ID: ${action.productId}]`
      }
      // 添加告警配置信息（如果有）
      if (action.alertConfigId) {
        description += ` [${t('alert.config.title')} ID: ${action.alertConfigId}]`
      }
      return description
    }) || []

  return {
    triggerSummary: triggerSummary.join(', ') || '-',
    actionSummary: actionSummary.join(', ') || '-'
  }
}
...
const handleToggleStatus = async (row: IotSceneRule) => {
  try {
    // 修改状态的二次确认
    const text = row.status === CommonStatusEnum.ENABLE ? t('common.disable') : t('common.enable')
