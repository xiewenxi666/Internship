  ...
  <el-card class="border border-[var(--el-border-color-light)] rounded-8px mb-10px" shadow="never">
    <template #header>
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-8px">
          ...
          <span class="text-16px font-600 text-[var(--el-text-color-primary)]">{{ t('rule.scene.triggerConfig.title') }}</span>
          <el-tag size="small" type="info">{{ triggers.length }} {{ t('rule.scene.triggerConfig.triggerCount') }}</el-tag>
        </div>
        <el-button type="primary" size="small" @click="addTrigger">
          <Icon icon="ep:plus" />
          {{ t('rule.scene.triggerConfig.addTrigger') }}
        </el-button>
      </div>
    </template>
    ...
    <div class="p-16px space-y-24px">
          ...
          >
            <div class="flex items-center gap-12px">
              <div class="flex items-center gap-8px text-16px font-600 text-green-700">
                ...
                <span>{{ t('rule.scene.triggerConfig.triggerIndex') }} {{ index + 1 }}</span>
              </div>
            ...
            <div class="flex items-center gap-8px">
              ...
              >
                <Icon icon="ep:delete" />
                {{ t('rule.scene.triggerConfig.delete') }}
              </el-button>
            </div>
          </div>
            ...
            >
              ...
              >
                ...
                <span class="text-14px font-500 text-[var(--el-text-color-primary)]"
                  >{{ t('rule.scene.triggerConfig.timerTriggerConfig') }}</span
                >
              </div>
              ...
              >
                <el-form-item :label="t('rule.scene.triggerConfig.cronExpression')" required>
                  <Crontab
      ...
      <div v-else class="py-40px text-center">
        <el-empty :description="t('rule.scene.triggerConfig.noTriggerConfig')">
          <template #description>
            <div class="space-y-8px">
              <p class="text-[var(--el-text-color-secondary)]">{{ t('rule.scene.triggerConfig.noTriggerConfig') }}</p>
              <p class="text-12px text-[var(--el-text-color-placeholder)]">
                {{ t('rule.scene.triggerConfig.addTriggerTip') }}
              </p>
            </div>
          </template>
        </el-empty>
