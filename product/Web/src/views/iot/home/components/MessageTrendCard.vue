  ...
  <el-card class="chart-card" shadow="never" :loading="loading">
    <template #header>
      <div class="flex items-center justify-between">
        <span class="text-base font-medium text-gray-600">{{ t('home.messageStatistics') }}</span>
        <div class="flex flex-wrap items-center gap-4">
          <el-form-item :label="t('home.timeRange')" class="!mb-0">
            <el-date-picker
          ...
          <el-form-item :label="t('home.timeInterval')" class="!mb-0">
            <el-select
              v-model="queryParams.interval"
              class="!w-120px"
              :placeholder="t('home.intervalType')"
              @change="handleQuery"
            >
              <el-option
    ...
    <div v-if="loading && !hasData" class="h-[300px] flex justify-center items-center">
      <el-empty :description="t('home.loading')" />
    </div>
    <div v-else-if="!hasData" class="h-[300px] flex justify-center items-center">
      <el-empty :description="t('home.noData')" />
    </div>
...
const initChart = () => {
  ...
  try {
    ...
    chart.setOption({
      ...
      legend: {
        data: [t('home.upstreamMessageCount'), t('home.downstreamMessageCount')],
        textStyle: {
          color: '#374151',
          fontWeight: 500
        }
      ...
      series: [
        {
          name: t('home.upstreamMessageCount'),
          type: 'line',
          smooth: true,
        ...
        {
          name: t('home.downstreamMessageCount'),
          type: 'line',
          smooth: true,
