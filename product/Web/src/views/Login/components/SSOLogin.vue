    ...
    <div>
      <el-form :model="formData" class="login-form">
        ...
        <el-form-item class="w-full">
          ...
          >
            <span v-if="!formLoading">{{ t('login.ssoAuthorize') }}</span>
            <span v-else>{{ t('login.ssoAuthorizing') }}</span>
          </el-button>
          <el-button class="w-3/10" @click.prevent="handleAuthorize(false)">{{ t('login.ssoDeny') }}</el-button>
        </el-form-item>
      </el-form>
    </div>
  </div>
...
const formatScope = (scope) => {
  ...
  switch (scope) {
    case 'user.read':
      return t('login.ssoScopeUserRead')
    case 'user.write':
      return t('login.ssoScopeUserWrite')
    default:
      return scope
  }
}
