    ...
    >
      ...
      <div class="flex flex-col items-start gap2" :id="`activity-task-${activity.id}-${index}`">
        ...
        <div class="flex w-full">
          <div class="font-bold">
            {{ activity.name }} <span v-if="activity.status === TaskStatusEnum.SKIP">{{ t('approval.skip') }}</span>
          </div>
          <!-- 信息：时间 -->
          <div
        ...
        >
          ...
          <el-tooltip :content="t('approval.addUser')" placement="left">
            <el-button
              class="!px-6px"
          ...
          <div v-for="(task, idx) in activity.tasks" :key="idx" class="flex flex-col pr-2 gap2">
            ...
            <teleport defer :to="`#activity-task-${activity.id}-${index}`">
              ...
              >
                ...
                {{ t('approval.opinionText') }}{{ task.reason }}
              </div>
              <div
