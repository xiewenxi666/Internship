...
const getPrintTemplateHTML = () => {
  ...
  if (processRecords.length > 0) {
    ...
    headTd.innerHTML = t('extraSettings.processRecord')
  ...
  <el-dialog v-loading="loading" v-model="visible" :show-close="false">
    <div id="printDivTag" style="word-break: break-all">
      ...
      <div v-else>
        ...
        <div class="text-right text-15px">{{ t('extraSettings.printUser') + ': ' + userName }}</div>
        <div class="flex justify-between">
          <div class="text-15px">{{ t('extraSettings.processNum') + ': ' + printData.processInstance.id }}</div>
          <div class="text-15px">{{ t('extraSettings.printTime') + ': ' + printTime }}</div>
        </div>
        <table class="mt-20px w-100%" border="1" style="border-collapse: collapse">
          <tbody>
            <tr>
              <td class="p-5px w-25%">{{ t('extraSettings.startUser') }}</td>
              <td class="p-5px w-25%">{{ printData.processInstance.startUser.nickname }}</td>
              <td class="p-5px w-25%">{{ t('extraSettings.startTime') }}</td>
              <td class="p-5px w-25%">{{ formatDate(printData.processInstance.startTime) }}</td>
            </tr>
            <tr>
              <td class="p-5px w-25%">{{ t('extraSettings.startUserDept') }}</td>
              <td class="p-5px w-25%">{{ printData.processInstance.startUser.deptName }}</td>
              <td class="p-5px w-25%">{{ t('extraSettings.processStatus') }}</td>
              <td class="p-5px w-25%">
                {{
                  getDictLabel(
                    DICT_TYPE.BPM_PROCESS_INSTANCE_STATUS,
            ...
            <tr>
              <td class="p-5px w-100% text-center" colspan="4">
                <h4>{{ t('extraSettings.processRecord') }}</h4>
              </td>
            </tr>
    ...
    <template #footer>
      <div class="dialog-footer">
        ...
        <el-button type="primary" v-print="printObj">{{ t('common.print') }}</el-button>
      </div>
    </template>
  </el-dialog>
</template>
