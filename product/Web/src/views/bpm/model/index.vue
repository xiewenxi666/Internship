<template>
  <ContentWrap>
    <div class="flex justify-between pl-20px items-center">
      <h3 class="font-extrabold">{{ t('process.model.title') }}</h3>
      <!-- 搜索工作栏 -->
      <el-form
      ...
      >
        <el-form-item prop="name" class="ml-auto">
          <el-input
            ...
            :placeholder="t('process.model.searchProcess')"
            clearable
        ...
        <el-form-item>
          <el-button type="primary" @click="openForm('create')" v-hasPermi="['bpm:model:create']">
            <Icon icon="ep:plus" class="mr-5px" /> {{ t('process.model.newModel') }}
          </el-button>
        </el-form-item>
        <el-form-item>
      ...
      <div class="mr-20px" v-else>
        ...
        <el-button type="primary" @click="handleCategorySortSubmit"> {{ t('process.model.saveSort') }} </el-button>
      </div>
    </div>
...
const handleCategorySortSubmit = async () => {
  ...
  // 刷新列表
  isCategorySorting.value = false
  message.success(t('process.model.sortSuccess'))
  await getList()
}
