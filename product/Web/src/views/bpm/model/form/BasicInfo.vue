<template>
  <el-form ref="formRef" :model="modelData" :rules="rules" label-width="120px" class="mt-20px">
    <el-form-item :label="t('process.model.key')" prop="key" class="mb-20px">
      <div class="flex items-center">
        <el-input
          class="!w-440px"
          v-model="modelData.key"
          :disabled="!!modelData.id"
          :placeholder="t('process.model.keyPlaceholder') + '，' + t('process.model.keyRule')"
        />
        <el-tooltip
          class="item"
          :content="modelData.id ? t('process.model.keyDisabledTip2') : t('process.model.keyDisabledTip')"
          effect="light"
          placement="top"
        >
    ...
    <el-form-item :label="t('process.model.name')" prop="name" class="mb-20px">
      <el-input
        v-model="modelData.name"
        :disabled="!!modelData.id"
        clearable
        :placeholder="t('process.model.namePlaceholder')"
      />
    </el-form-item>
    <el-form-item :label="t('process.model.category')" prop="category" class="mb-20px">
      <el-select
        class="!w-full"
        v-model="modelData.category"
        clearable
        :placeholder="t('process.model.categoryPlaceholder')"
      >
        <el-option
    ...
    <el-form-item :label="t('process.model.icon')" class="mb-20px">
      <UploadImg v-model="modelData.icon" :limit="1" height="64px" width="64px" />
    </el-form-item>
    <el-form-item :label="t('process.model.description')" prop="description" class="mb-20px">
      <el-input v-model="modelData.description" clearable type="textarea" />
    </el-form-item>
    <el-form-item :label="t('process.model.type')" prop="type" class="mb-20px">
    ...
    <el-form-item :label="t('process.model.visible')" prop="visible" class="mb-20px">
    ...
    <el-form-item :label="t('process.model.startUserType')" prop="startUserType" class="mb-20px">
      <el-select
        v-model="modelData.startUserType"
        :placeholder="t('process.model.startUserType')"
        @change="handleStartUserTypeChange"
      >
        <el-option :label="t('process.model.startUserTypeAll')" :value="0" />
        <el-option :label="t('process.model.startUserTypeUser')" :value="1" />
        <el-option :label="t('process.model.startUserTypeDept')" :value="2" />
      </el-select>
      <div v-if="modelData.startUserType === 1" class="mt-2 flex flex-wrap gap-2">
        ...
        <el-button type="primary" link @click="openStartUserSelect">
          <Icon icon="ep:plus" /> {{ t('process.model.selectUser') }}
        </el-button>
      </div>
      <div v-if="modelData.startUserType === 2" class="mt-2 flex flex-wrap gap-2">
        ...
        <el-button type="primary" link @click="openStartDeptSelect">
          <Icon icon="ep:plus" /> {{ t('process.model.selectDept') }}
        </el-button>
      </div>
    </el-form-item>
    <el-form-item :label="t('process.model.managerUserIds')" prop="managerUserIds" class="mb-20px">
...
const rules = computed(() => ({
  name: [{ required: true, message: t('process.model.namePlaceholder'), trigger: 'blur' }],
  key: [
    { required: true, message: t('process.model.keyPlaceholder'), trigger: 'blur' },
    {
      validator: (_rule: any, value: string, callback: any) => {
        ...
        if (!/^[a-zA-Z_][\-_.0-9_a-zA-Z$]*$/.test(value)) {
          callback(new Error(t('process.model.keyRule')))
          return
        }
        callback()
      },
  ...
  category: [{ required: true, message: t('process.model.categoryPlaceholder'), trigger: 'blur' }],
  type: [{ required: true, message: t('process.model.type'), trigger: 'blur' }],
  visible: [{ required: true, message: t('process.model.visible'), trigger: 'blur' }],
  managerUserIds: [{ required: true, message: t('process.model.managerUserIds'), trigger: 'blur' }]
}))
