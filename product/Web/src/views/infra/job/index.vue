<template>
        ...
        <el-col :span="24">
          <el-form-item>
            ...
            <el-button type="info" plain @click="handleJobLog()" v-hasPermi="['infra:job:query']">
              <Icon icon="ep:zoom-in" class="mr-5px" /> {{ t('job.log.title') }}
            </el-button>
          </el-form-item>
        </el-col>
      </el-row>
  ...
  <ContentWrap>
    <el-table v-loading="loading" :data="list" @selection-change="handleRowCheckboxChange" :table-layout="'auto'">
      ...
      <el-table-column :label="t('job.id')" align="center" prop="id" />
      <el-table-column :label="t('job.name')" align="center" prop="name" />
      ...
      <el-table-column :label="t('job.handlerName')" align="center" prop="handlerName"  fixed="right" />
      <el-table-column :label="t('job.handlerParams')" align="center" prop="handlerParam" />
      <el-table-column :label="t('job.cronExpression')" align="center" prop="cronExpression" />
      <el-table-column :label="t('common.operation')" align="center" min-width="200">
        <template #default="scope">
          ...
          <el-button
            ...
            v-hasPermi="['infra:job:update']"
          >
            {{ scope.row.status === InfraJobStatusEnum.STOP ? t('job.start') : t('job.pause') }}
          </el-button>
          <el-button
            type="danger"
            link
...
const handleExport = async () => {
  try {
    ...
    download.excel(data, t('common.exportFilename.jobData') + '.xls')
  } catch {
  } finally {
...
const handleChangeStatus = async (row: JobApi.JobVO) => {
  try {
    // 修改状态的二次确认
    const text = row.status === InfraJobStatusEnum.STOP ? t('job.start') : t('job.stop')
    await message.confirm(
      t('job.confirmStatus', { text: text, id: row.id }),
      t('common.reminder')
    )
    const status =
...
const handleRun = async (row: JobApi.JobVO) => {
  try {
    // 二次确认
    await message.confirm(t('job.confirmRun', { name: row.name }), t('common.reminder'))
    // 提交执行
