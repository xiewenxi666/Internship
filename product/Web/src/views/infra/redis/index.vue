<template>
  <doc-alert :title="t('redis.title')" url="https://doc.iocoder.cn/redis-cache/" />
  ...
  <el-scrollbar height="calc(100vh - 88px - 40px - 50px)">
    <el-row>
      ...
      <el-col :span="24" class="card-box" shadow="hover">
        <el-card>
          <el-descriptions :title="t('redis.basicInfo')" :column="6" border>
            <el-descriptions-item :label="t('redis.redisVersion') + ' :'">
              {{ cache?.info?.redis_version }}
            </el-descriptions-item>
            <el-descriptions-item :label="t('redis.runMode') + ' :'">
              {{ cache?.info?.redis_mode == 'standalone' ? t('redis.runModeStandalone') : t('redis.runModeCluster') }}
            </el-descriptions-item>
            <el-descriptions-item :label="t('redis.port') + ' :'">
              {{ cache?.info?.tcp_port }}
            </el-descriptions-item>
            <el-descriptions-item :label="t('redis.clientCount') + ' :'">
              {{ cache?.info?.connected_clients }}
            </el-descriptions-item>
            <el-descriptions-item :label="t('redis.uptimeDays') + ' :'">
              {{ cache?.info?.uptime_in_days }}
            </el-descriptions-item>
            <el-descriptions-item :label="t('redis.usedMemory') + ' :'">
              {{ cache?.info?.used_memory_human }}
            </el-descriptions-item>
            <el-descriptions-item :label="t('redis.usedCpu') + ' :'">
              {{ cache?.info ? parseFloat(cache?.info?.used_cpu_user_children).toFixed(2) : '' }}
            </el-descriptions-item>
            <el-descriptions-item :label="t('redis.memoryConfig') + ' :'">
              {{ cache?.info?.maxmemory_human }}
            </el-descriptions-item>
            <el-descriptions-item :label="t('redis.aofEnabled') + ' :'">
              {{ cache?.info?.aof_enabled == '0' ? t('common.no') : t('common.yes') }}
            </el-descriptions-item>
            <el-descriptions-item :label="t('redis.rdbStatus') + ' :'">
              {{ cache?.info?.rdb_last_bgsave_status }}
            </el-descriptions-item>
            <el-descriptions-item :label="t('redis.keyCount') + ' :'">
              {{ cache?.dbSize }}
            </el-descriptions-item>
            <el-descriptions-item :label="t('redis.networkIO') + ' :'">
...
const usedMemoryInstance = async () => {
  try {
    ...
    usedmemoryEchartChika.series[0].data[0] = {
      ...
      name: t('redis.memoryConsumption')
    }
