<template>
  <div class="relative" style="width: 100%; height: 700px">
    ...
    <!-- 测试窗口 -->
    <el-drawer v-model="showTestDrawer" :title="tAi('workflow.workflowTest')" :modal="false">
      <fieldset>
        <legend class="ml-15px"><h3>{{ tAi('workflow.runParamsConfig') }}</h3></legend>
        <div class="p-20px">
          <div
            ...
            :key="index"
          >
            <el-select class="w-200px!" v-model="param.key" :placeholder="tAi('workflow.paramName')">
              <el-option
            ...
            <el-input class="w-200px!" v-model="param.value" :placeholder="tAi('workflow.paramValue')" />
            <el-button type="danger" plain :icon="Delete" circle @click="removeParam(index)" />
          </div>
          <!-- TODO @lesan：是不是不用添加和删除参数，直接把必填和选填列出来，然后加上参数校验？ -->
          <el-button type="primary" plain @click="addParam">{{ tAi('workflow.addParam') }}</el-button>
        </div>
      </fieldset>
      <fieldset class="mt-20px bg-#f8f9fa">
        <legend class="ml-15px"><h3>{{ tAi('workflow.runResult') }}</h3></legend>
        <div class="p-20px">
          <div v-if="loading"> <el-text type="primary">{{ tAi('workflow.executing') }}</el-text></div>
          <div v-else-if="error">
          ...
          <div v-else> <el-text type="info">{{ tAi('workflow.clickToRun') }}</el-text> </div>
        </div>
      </fieldset>
      <el-button class="mt-20px w-100%" size="large" type="success" @click="goRun">
        {{ tAi('workflow.runProcess') }}
      </el-button>
    </el-drawer>
  </div>
</template>
...
const goRun = async () => {
  try {
    ...
    error.value = err.response?.data?.message || tAi('workflow.runFailed')
  } finally {
    loading.value = false
  }
}
...
const getStartNode = () => {
  ...
  if (!startNode) {
    throw new Error(tAi('workflow.missingStartNode'))
  }
  return startNode
}
...
const convertParamValue = (value, dataType) => {
  ...
  switch (dataType) {
    ...
    case 'Number':
      ...
      if (isNaN(num)) throw new Error(tAi('workflow.notNumber'))
      return num
    case 'Boolean':
      if (value.toLowerCase() === 'true') return true
      if (value.toLowerCase() === 'false') return false
      throw new Error(tAi('workflow.mustBeBoolean'))
    case 'Object':
    case 'Array':
      try {
        return JSON.parse(value)
      } catch (e) {
        throw new Error(`${tAi('workflow.jsonFormatError')}: ${e.message}`)
      }
    default:
      throw new Error(`${tAi('workflow.unsupportedType')}: ${dataType}`)
  }
}
...
const validate = async () => {
  try {
    ...
    if (!workflowData.value) {
      throw new Error(tAi('workflow.designWorkflow'))
    }
