...
<template>
  <el-aside
    width="260px"
    class="h-full flex flex-col px-2.5 pt-2.5 pb-0 overflow-hidden"
  >
    <!-- 左顶部：对话 -->
    <div class="shrink-0">
      ...
      <el-input
        ...
        size="large"
        class="mt-5"
        :placeholder="t('chat.conversation.searchHistory')"
    ...
    <!-- 左中间：对话列表 -->
    <div class="flex-1 overflow-y-auto min-h-0 mt-2.5">
      <!-- 情况一：加载中 -->
          ...
          >
            ...
            >
              <el-button class="m-0" link @click.stop="handleTop(conversation)">
                <el-icon :title="t('chat.conversation.pinned')" v-if="!conversation.pinned"><Top /></el-icon>
                <el-icon :title="t('chat.conversation.pinned')" v-if="conversation.pinned"><Bottom /></el-icon>
              </el-button>
              <el-button class="m-0" link @click.stop="updateConversationTitle(conversation)">
                <el-icon :title="t('chat.conversation.editTitle')">
                  <Icon icon="ep:edit" />
                </el-icon>
              </el-button>
              <el-button class="m-0" link @click.stop="deleteChatConversation(conversation)">
                <el-icon :title="t('chat.conversation.deleteConversation')">
                  <Icon icon="ep:delete" />
                </el-icon>
              </el-button>
            </div>
    ...
    <!-- 左底部：工具栏 -->
    <div
      class="shrink-0 px-5 leading-8.75 flex justify-between items-center mt-2.5"
      style="
    ...
    >
      <div
        class="flex items-center p-0 m-0 cursor-pointer shrink-0"
        style="color: var(--el-text-color-regular)"
        @click="handleRoleRepository"
      >
        <Icon icon="ep:user" />
        <el-text class="ml-1.25" size="small">{{ t('chat.role.repository') }}</el-text>
      </div>
      <div
        class="flex items-center p-0 m-0 cursor-pointer shrink-0"
        style="color: var(--el-text-color-regular)"
      ...
      >
        <Icon icon="ep:delete" />
        <el-text class="ml-1.25 max-w-120px truncate" size="small">{{ t('chat.conversation.confirmClearAll') }}</el-text>
      </div>
    </div>

    <!-- 角色仓库抽屉 -->
    <el-drawer v-model="roleRepositoryOpen" :title="t('chat.role.repository')" size="754px">
      <RoleRepository />
    </el-drawer>
  </el-aside>
</template>
...
const getConversationGroupByCreateTime = async (list: ChatConversationVO[]) => {
  ...
  const groupMap = {
    [t('chat.conversation.pinned')]: [] as ChatConversationVO[],
    [t('chat.conversation.today')]: [] as ChatConversationVO[],
    [t('chat.conversation.oneDayAgo')]: [] as ChatConversationVO[],
    [t('chat.conversation.threeDaysAgo')]: [] as ChatConversationVO[],
    [t('chat.conversation.sevenDaysAgo')]: [] as ChatConversationVO[],
    [t('chat.conversation.thirtyDaysAgo')]: [] as ChatConversationVO[]
  }
  // 当前时间的时间戳
  const now = Date.now()
  // 定义时间间隔常量（单位：毫秒）
  ...
  for (const conversation of list) {
    ...
    if (conversation.pinned) {
      groupMap[t('chat.conversation.pinned')].push(conversation)
      continue
    }
    // 计算时间差（单位：毫秒）
    ...
    if (diff < oneDay) {
      ...
      groupMap[t('chat.conversation.oneDayAgo')].push(conversation)
    } else if (diff < sevenDays) {
      groupMap[t('chat.conversation.threeDaysAgo')].push(conversation)
    } else if (diff < thirtyDays) {
      groupMap[t('chat.conversation.sevenDaysAgo')].push(conversation)
    } else {
      groupMap[t('chat.conversation.thirtyDaysAgo')].push(conversation)
    }
  }
  return groupMap
}
...
const updateConversationTitle = async (conversation: ChatConversationVO) => {
  // 1. 二次确认
  const { value } = await ElMessageBox.prompt(t('chat.conversation.editTitle'), {
    inputPattern: /^[\s\S]*.*\S[\s\S]*$/, // 判断非空，且非空格
    inputErrorMessage: t('chat.conversation.namePlaceholder'),
  ...
  message.success(t('chat.conversation.renameSuccess'))
  // 3. 刷新列表
  await getChatConversationList()
  // 4. 过滤当前切换的
...
const deleteChatConversation = async (conversation: ChatConversationVO) => {
  try {
    // 删除的二次确认
    await message.delConfirm(`${t('chat.conversation.confirmDelete')} - ${conversation.title}?`)
    // 发起删除
    await ChatConversationApi.deleteChatConversationMy(conversation.id)
    message.success(t('chat.conversation.deleted'))
    // 刷新列表
    await getChatConversationList()
    // 回调
...
const handleClearConversation = async () => {
  try {
    await message.confirm(t('chat.conversation.confirmClearAll'))
    await ChatConversationApi.deleteChatConversationMyByUnpinned()
    ElMessage({
      message: t('chat.conversation.clearSuccess'),
      type: 'success'
    })
    // 清空 对话 和 对话内容
    activeConversationId.value = null
