<template>
  <el-container class="h-full w-full">
    ...
    />
    <!-- 右侧：对话详情 -->
    <el-container class="flex flex-col h-full bg-[var(--el-bg-color)]">
      <el-header
        class="flex flex-row items-center justify-between bg-[var(--el-bg-color-page)] shadow-[0_0_0_0_var(--el-border-color-light)]"
      >
        <div class="text-18px font-bold">
          {{ activeConversation?.title ? activeConversation?.title : t('chat.conversation.dialogTitle') }}
      ...
      <!-- main：消息列表 -->
      <el-main class="flex-1 m-0 p-0 overflow-hidden">
        <!-- 情况一：消息加载中 -->
      ...
      <!-- 底部 -->
      <el-footer class="flex flex-col !h-auto !p-0 shrink-0">
        <!-- TODO @芋艿：这块要想办法迁移下！ -->
        <form
        ...
        >
          <textarea
            ...
            :placeholder="t('chat.message.askAnything')"
          >
          </textarea>
          <div class="flex justify-between pb-0 pt-5px">
            <div class="flex items-center">
              ...
              <span class="ml-5px mr-15px text-14px text-#8f8f8f">{{ t('chat.message.context') }}</span>
              <el-switch v-model="enableWebSearch" />
              <span class="ml-5px text-14px text-#8f8f8f">{{ t('chat.message.webSearch') }}</span>
            </div>
            <el-button
              type="primary"
              size="default"
            ...
            >
              {{ conversationInProgress ? t('chat.message.inProgress') : t('chat.message.send') }}
            </el-button>
            <el-button
              type="danger"
              size="default"
...
const handleConversationClick = async (conversation: ChatConversationVO) => {
  ...
  if (conversationInProgress.value) {
    message.alert(t('chat.message.cannotSwitch'))
    return false
  }
...
const handleMessageDelete = () => {
  if (conversationInProgress.value) {
    message.alert(t('chat.message.cannotDelete'))
    return
  }
  // 刷新 message 列表
  getMessageList()
...
const handlerMessageClear = async () => {
  ...
  try {
    // 确认提示
    await message.delConfirm(t('chat.message.confirmClearMessages'))
    // 清空对话
...
const doSendMessage = async (content: string) => {
  ...
  if (activeConversationId.value == null) {
    message.error(t('chat.message.noConversation'))
    return
  }
  ...
  try {
    ...
    await ChatMessageApi.sendChatMessageStream(
      ...
      async (res) => {
        ...
        if (code !== 0) {
          message.alert(`${t('chat.message.chatError')} ${msg}`)
          // 如果未接收到消息，则进行删除
      ...
      (error: any) => {
        // 异常提示，并停止流
        message.alert(t('chat.message.chatError'))
        stopStream()
        // 需要抛出异常，禁止重试
        throw error
      },
