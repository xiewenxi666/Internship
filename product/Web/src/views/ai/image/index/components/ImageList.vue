<template>
  ...
  >
    <template #header>
      ...
      <el-button @click="handleViewPublic">{{ t('image.task.paintingWorks') }}</el-button>
    </template>
    <!-- 图片列表 -->
    <div
...
const handleImageButtonClick = async (type: string, imageDetail: ImageVO) => {
  ...
  // 删除
  if (type === 'delete') {
    await message.confirm(t('image.task.confirmDelete'))
    await ImageApi.deleteImageMy(imageDetail.id)
    await getImageList()
    message.success(t('image.task.deleteSuccess'))
    return
  }
  // 下载
  if (type === 'download') {
