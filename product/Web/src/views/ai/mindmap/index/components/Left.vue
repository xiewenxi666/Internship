<template>
  <div class="w-[350px] p-5 flex flex-col bg-[#f5f7f9]">
    <h3 class="w-full h-full h-7 text-5 text-center leading-[28px] title">{{ tAi('mindMap.center') }}</h3>
    <!--下面表单部分-->
    <div class="flex-grow overflow-y-auto">
      <div class="mt-[30ppx]">
        <el-text tag="b">{{ tAi('mindMap.yourRequirement') }}</el-text>
        <el-input
        ...
        >
          {{ tAi('mindMap.generateButton') }}
        </el-button>
      </div>
      <div class="mt-[30px]">
        <el-text tag="b">{{ tAi('mindMap.useExistContent') }}</el-text>
        <el-input
          ...
          class="w-100% mt-15px"
          input-style="border-radius: 7px;"
          :placeholder="tAi('mindMap.existContentPlaceholder')"
          show-word-limit
          type="textarea"
        />
        <el-button
