<template>
  <doc-alert :title="tAi('write.title')" url="https://doc.iocoder.cn/ai/write/" />
  ...
  <ContentWrap>
    ...
    >
      <el-row :gutter="20">
        ...
        <el-col :span="8">
          <el-form-item :label="tAi('write.task.type')" prop="type">
            <el-select
  ...
  <ContentWrap>
    <el-table v-loading="loading" :data="list" :stripe="true" :show-overflow-tooltip="true" :table-layout="'auto'">
      <el-table-column :label="tAi('mindMap.id')" align="center" prop="id" min-width="120" fixed="left" />
      <el-table-column :label="tAi('mindMap.user')" align="center" prop="userId" min-width="180">
        <template #default="scope">
          <span>{{ userList.find((item) => item.id === scope.row.userId)?.nickname }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="tAi('write.task.type')" align="center" prop="type">
        <template #default="scope">
      ...
      <el-table-column
        :label="tAi('write.manager.generatePrompt')"
        align="center"
        prop="prompt"
        min-width="180"
        show-overflow-tooltip
      />
      <el-table-column :label="tAi('write.task.content')" align="center" prop="generatedContent" min-width="180" />
      <el-table-column :label="tAi('write.manager.originalContent')" align="center" prop="originalContent" min-width="180" />
      <el-table-column :label="tAi('write.length')" align="center" prop="length">
        <template #default="scope">
          <dict-tag :type="DICT_TYPE.AI_WRITE_LENGTH" :value="scope.row.length" />
        </template>
      </el-table-column>
      <el-table-column :label="tAi('write.format')" align="center" prop="format">
        <template #default="scope">
          <dict-tag :type="DICT_TYPE.AI_WRITE_FORMAT" :value="scope.row.format" />
        </template>
      </el-table-column>
      <el-table-column :label="tAi('write.tone')" align="center" prop="tone">
        <template #default="scope">
          <dict-tag :type="DICT_TYPE.AI_WRITE_TONE" :value="scope.row.tone" />
        </template>
      </el-table-column>
      <el-table-column :label="tAi('write.language')" align="center" prop="language">
        <template #default="scope">
      ...
      <el-table-column :label="tAi('mindMap.operation')" align="center">
        <template #default="scope">
          <el-button
            link
            type="danger"
