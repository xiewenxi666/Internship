<template>
      ...
      <el-row :gutter="20">
        <el-col :span="8">
          <el-form-item :label="t('oa.loan.purpose')" prop="purpose">
            <el-select v-model="queryParams.purpose" class="!w-240px" clearable :placeholder="t('oa.loan.purposePlaceholder')">
              <el-option v-for="dict in getIntDictOptions(DICT_TYPE.OA_LOAN_PURPOSE)" :key="dict.value" :label="dict.label" :value="dict.value" />
            </el-select>
          </el-form-item>
        </el-col>
        <el-col :span="8">
        ...
        <el-col :span="8">
          <el-form-item :label="t('oa.loan.approvalResult')" prop="status">
            <el-select v-model="queryParams.status" class="!w-240px" clearable :placeholder="t('oa.loan.approvalResultPlaceholder')">
  ...
  <ContentWrap>
    <el-table v-loading="loading" :data="list" :table-layout="'auto'">
      ...
      <el-table-column align="center" :label="t('oa.loan.purpose')" prop="purpose" min-width="120">
        <template #default="scope">
          {{ getPurposeText(scope.row.purpose) }}
        </template>
      </el-table-column>
      <el-table-column align="center" :label="t('oa.loan.amount')" prop="amount" min-width="120" />
      <el-table-column :formatter="dateFormatter" align="center" :label="t('oa.loan.expectedRepayTime')" prop="expectedRepayTime" min-width="180" />
      <el-table-column align="center" :label="t('oa.loan.reason')" prop="reason" min-width="200" />
      <el-table-column :formatter="dateFormatter" align="center" :label="t('oa.loan.applyTime')" prop="createTime" min-width="180" />
      <el-table-column align="center" :label="t('common.operation')" min-width="200" fixed="right">
        <template #default="scope">
          <el-button v-hasPermi="['oa:loan:query']" link type="primary" @click="handleDetail(scope.row)">
            {{ t('common.detail') }}
          </el-button>
          <el-button v-hasPermi="['oa:loan:query']" link type="primary" @click="handleProcessDetail(scope.row)">
            {{ t('oa.loan.progress') }}
          </el-button>
          <el-button v-if="scope.row.status === 1" v-hasPermi="['oa:loan:create']" link type="danger" @click="cancelLoan(scope.row)">
            {{ t('approval.cancel') }}
          </el-button>
          <el-button v-if="scope.row.status !== 1" v-hasPermi="['oa:loan:create']" link type="primary" @click="handleReCreate(scope.row)">
...
const handleReCreate = (row: LoanApi.LoanVO) => {
  router.push({ name: 'OALoanCreate', query: { id: row.id } })
}

const handleDetail = (row: LoanApi.LoanVO) => {
  router.push({ name: 'OALoanDetail', query: { id: row.id } })
}

const cancelLoan = async (row) => {
