<template>
      ...
      <el-row :gutter="20">
        <el-col :span="8">
          <el-form-item :label="t('oa.visit.customerName')" prop="customerName">
            <el-input v-model="queryParams.customerName" :placeholder="t('oa.visit.customerNamePlaceholder')"
      ...
      <el-row>
        <el-col :span="24">
          <el-form-item>
            ...
            <el-button type="primary" plain @click="openForm('create')" v-hasPermi="['oa:visit:create']">
  ...
  <ContentWrap>
    <el-table v-loading="loading" :data="list" :table-layout="'auto'">
      <el-table-column :label="t('oa.visit.id')" align="center" prop="id" />
      <el-table-column :label="t('common.status')" align="center" prop="status">
        <template #default="scope">
          <dict-tag :type="DICT_TYPE.OA_VISIT_STATUS" :value="scope.row.status" />
        </template>
      </el-table-column>
      <el-table-column :label="t('oa.visit.customerName')" align="center" prop="customerName" />
      <el-table-column :label="t('oa.visit.contactPerson')" align="center" prop="contactPerson" />
      <el-table-column :label="t('oa.visit.purpose')" align="center" prop="purpose" />
      <el-table-column :label="t('oa.visit.visitTime')" align="center" prop="visitTime" min-width="160" :formatter="dateFormatter" />
      <el-table-column :label="t('common.operation')" align="center" width="320">
        <template #default="scope">
          <el-button link type="primary" @click="handleSubmit(scope.row.id)"
            v-if="scope.row.status === -1" v-hasPermi="['oa:visit:submit']">{{ t('common.submit') }}</el-button>
          <el-button link type="primary" @click="openForm('update', scope.row.id)"
            v-if="scope.row.status === -1" v-hasPermi="['oa:visit:update']">{{ t('action.edit') }}</el-button>
          <el-button link type="primary" @click="handleApprovalProgress(scope.row.processInstanceId)"
            v-if="scope.row.status >= 1 && scope.row.processInstanceId" v-hasPermi="['oa:visit:query']">{{ t('common.approvalProgress') }}</el-button>
          <el-button link type="danger" @click="handleDelete(scope.row.id)"
            v-if="scope.row.status === -1" v-hasPermi="['oa:visit:delete']">{{ t('action.del') }}</el-button>
        </template>
      </el-table-column>
    </el-table>
