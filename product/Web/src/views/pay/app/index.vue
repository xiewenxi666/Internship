<template>
  ...
  <ContentWrap>
    <el-table v-loading="loading" :data="list" :table-layout="'auto'">
      <el-table-column align="center" :label="t('app.appKey')" prop="appKey" />
      <el-table-column align="center" :label="t('app.name')" min-width="90" prop="name" />
      <el-table-column align="center" :label="t('app.status')" prop="status">
      ...
      <el-table-column align="center" :label="t('app.alipayConfig')">
        <el-table-column
      ...
      <el-table-column align="center" :label="t('app.wechatConfig')">
        <el-table-column
      ...
      <el-table-column align="center" :label="t('app.walletPayConfig')">
      ...
      <el-table-column align="center" :label="t('app.mockPayConfig')">
...
const handleStatusChange = async (row: any) => {
  const text = row.status === CommonStatusEnum.ENABLE ? t('app.confirmEnable', { name: row.name }) : t('app.confirmDisable', { name: row.name })
  try {
    await message.confirm(text)
    await AppApi.changeAppStatus({ id: row.id, status: row.status })
    message.success(row.status === CommonStatusEnum.ENABLE ? t('app.enableSuccess') : t('app.disableSuccess'))
  } catch {
    row.status =
