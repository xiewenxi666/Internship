...
<template>
  ...
  <el-card shadow="never" class="mt-16px">
    <el-table v-loading="loading" :data="list" :table-layout="'auto'">
      ...
      <el-table-column
        :label="t('customer.customerPutCount')"
        align="right"
        prop="customerPutCount"
        min-width="200"
      />
      <el-table-column
        :label="t('customer.customerTakeCount')"
        align="right"
...
const echartsOption = reactive<EChartsOption>({
  ...
  series: [
    {
      name: t('customer.customerPutCount'),
      type: 'bar',
      yAxisIndex: 0,
      data: []
    },
    {
      name: t('customer.customerTakeCount'),
      type: 'bar',
      yAxisIndex: 1,
      data: []
    }
  ...
  toolbox: {
    feature: {
      ...
      saveAsImage: { show: true, name: t('customer.poolSummary') } // 保存为图片
    }
  },
  tooltip: {
    trigger: 'axis',
