...
<template>
  ...
  <el-card shadow="never" class="mt-16px">
    <el-table v-loading="loading" :data="list" :table-layout="'auto'">
      ...
      <el-table-column :label="t('customer.followUpType')" align="center" prop="followUpType" min-width="200">
        <template #default="scope">
          <dict-tag :type="DICT_TYPE.CRM_FOLLOW_UP_TYPE" :value="scope.row.followUpType" />
        </template>
      </el-table-column>
      <el-table-column :label="t('customer.count')" align="center" prop="followUpRecordCount" min-width="200" />
      <el-table-column :label="t('customer.portion')" align="center" prop="portion" min-width="200" />
    </el-table>
  </el-card>
</template>
...
const list = ref<CrmStatisticsFollowUpSummaryByTypeRespVO[]>([]) // 列表的数
/** 饼图配置 */
const echartsOption = reactive<EChartsOption>({
  title: {
    text: t('customer.followUpType'),
    left: 'center'
  },
  legend: {
    orient: 'vertical',
  ...
  toolbox: {
    feature: {
      saveAsImage: { show: true, name: t('customer.followUpType') } // 保存为图
    }
  },
  series: [
    {
      name: t('customer.followUpType'),
      type: 'pie',
      radius: '50%',
      data: [],
      emphasis: {
