<template>
  <ContentWrap>
    <el-table :data="contract.products" :stripe="true" :show-overflow-tooltip="true" :table-layout="'auto'">
      <el-table-column
        align="center"
      ...
      />
      <el-table-column
        :label="t('crm.contract.contractPrice') + '（元）'"
        align="center"
        prop="contractPrice"
        min-width="140"
        :formatter="erpPriceTableColumnFormatter"
    ...
    <el-row class="mt-10px" justify="end">
      <el-col :span="3"> {{ t('crm.contract.discountPercent') }}：{{ erpPriceInputFormatter(contract.discountPercent) }}% </el-col>
      <el-col :span="4">
        {{ t('crm.contract.totalProductPrice') }}：{{ erpPriceInputFormatter(contract.totalProductPrice) }} 元
      </el-col>
    </el-row>
  </ContentWrap>
</template>
