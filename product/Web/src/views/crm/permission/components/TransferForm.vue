    ...
    >
      ...
      <el-form-item v-if="bizType === BizTypeEnum.CRM_CUSTOMER" :label="t('permission.transferTogether')">
        <el-checkbox-group v-model="formData.toBizTypes">
          ...
          <el-checkbox :value="BizTypeEnum.CRM_CONTRACT">{{ t('contract.title') }}</el-checkbox>
        </el-checkbox-group>
      </el-form-item>
    </el-form>
    <template #footer>
...
const getDialogTitle = () => {
  switch (props.bizType) {
    case BizTypeEnum.CRM_CLUE:
      return t('clue.title') + t('customer.transfer')
    case BizTypeEnum.CRM_CUSTOMER:
      return t('customer.title') + t('customer.transfer')
    case BizTypeEnum.CRM_CONTACT:
      return t('contact.title') + t('customer.transfer')
    case BizTypeEnum.CRM_BUSINESS:
