<template>
  ...
  <el-row justify="end">
    ...
    >
      <Icon class="mr-5px" icon="ep:circle-plus" />{{ t('crm.business.relate') }}
    </el-button>
    <el-button
      ...
      v-if="queryParams.contactId"
    >
      <Icon class="mr-5px" icon="ep:remove" />{{ t('crm.business.unrelate') }}
    </el-button>
  </el-row>
  ...
  <ContentWrap class="mt-10px">
    ...
    >
      ...
      <el-table-column :label="t('crm.business.name')" fixed="left" align="center" prop="name">
      ...
      <el-table-column
        :label="t('crm.business.price')"
        align="center"
        prop="price"
        :formatter="erpPriceTableColumnFormatter"
      />
      <el-table-column :label="t('crm.business.customerName')" align="center" prop="customerName" />
      <el-table-column :label="t('crm.business.statusTypeName')" align="center" prop="statusTypeName" />
      <el-table-column :label="t('crm.business.statusName')" align="center" prop="statusName" />
    </el-table>
    <!-- 分页 -->
    <Pagination
      :total="total"
...
const createContactBusinessList = async (businessIds: number[]) => {
  ...
  // 刷新列表
  message.success(t('crm.business.relateSuccess'))
  handleQuery()
}
...
const deleteContactBusinessList = async () => {
  ...
  if (data.businessIds.length === 0) {
    return message.error(t('crm.business.noSelectBusiness'))
  }
  await ContactApi.deleteContactBusinessList(data)
  // 刷新列表
  message.success(t('crm.business.unrelateSuccess'))
  handleQuery()
}
