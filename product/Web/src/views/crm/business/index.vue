  ...
  <ContentWrap>
    ...
    <el-table v-loading="loading" :data="list" :show-overflow-tooltip="true" :stripe="true" :table-layout="'auto'">
      ...
      <el-table-column
        ...
        min-width="120"
      />
      <el-table-column align="center" :label="t('crm.business.quotationRecord')" min-width="100">
        <template #default="scope">
          <el-link :underline="false" type="primary" @click="openQuotationRecord(scope.row.id)">
            {{ t('common.view') }}
          </el-link>
        </template>
      </el-table-column>
      ...
      <el-table-column
        ...
        min-width="180"
      />
      <el-table-column align="center" :label="t('crm.business.noFollowDays')" min-width="120">
        <template #default="scope">
          {{ calcNoFollowDays(scope.row.contactLastTime) }}
        </template>
      </el-table-column>
...
/** 计算未跟进天数 */
const calcNoFollowDays = (contactLastTime: string | null) => {
  if (!contactLastTime) return '-'
  const last = new Date(contactLastTime).getTime()
  const now = Date.now()
  const diff = Math.floor((now - last) / (1000 * 60 * 60 * 24))
  return diff >= 0 ? diff + t('crm.business.daysUnit') : '-'
}

/** 打开报价记录 */
const openQuotationRecord = (businessId: number) => {
  push({ name: 'CrmQuotationRecord', query: { businessId } })
}

/** 跟进操作 */
const handleFollow = (row: any) => {
  push({ name: 'CrmBusinessDetail', params: { id: row.id }, query: { tab: 'followUp' } })
}
...
const handleExport = async () => {
  try {
    ...
    download.excel(data, t('crm.business.title') + '.xls')
  } catch {
  } finally {
