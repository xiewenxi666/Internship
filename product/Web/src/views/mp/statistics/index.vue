<template>
  <!-- 搜索工作栏 -->
  <ContentWrap>
    <el-form class="-mb-15px" ref="queryForm" label-width="auto">
      <el-row :gutter="20">
        <el-col :span="8">
        ...
        <el-col :span="8">
          <el-form-item :label="t('statistics.dateRange')" prop="dateRange">
            <el-date-picker
              v-model="dateRange"
              type="daterange"
    ...
    <el-row>
      <el-col :span="12" class="card-box">
        <el-card>
          <template #header>
            <div>
              <span>{{ t('statistics.userSummary.title') }}</span>
            </div>
          </template>
      ...
      <el-col :span="12" class="card-box">
        <el-card>
          <template #header>
            <div>
              <span>{{ t('statistics.userCumulate.title') }}</span>
            </div>
          </template>
      ...
      <el-col :span="12" class="card-box">
        <el-card>
          <template #header>
            <div>
              <span>{{ t('statistics.upstreamMessage.title') }}</span>
            </div>
          </template>
      ...
      <el-col :span="12" class="card-box">
        <el-card>
          <template #header>
            <div>
              <span>{{ t('statistics.interfaceSummary.title') }}</span>
            </div>
          </template>
...
const userSummaryOption = reactive({
  ...
  series: [
    {
      name: t('statistics.userSummary.newUser'),
      type: 'bar' as const,
      label: {
        show: true
      },
    ...
    {
      name: t('statistics.userSummary.cancelUser'),
      type: 'bar' as const,
      label: {
        show: true
      },
...
const userCumulateOption = reactive({
  ...
  series: [
    {
      name: t('statistics.userCumulate.cumulateUser'),
...
const upstreamMessageOption = reactive({
  ...
  series: [
    {
      name: t('statistics.upstreamMessage.messageUser'),
      type: 'line' as const,
      smooth: true,
      label: {
        show: true
    ...
    {
      name: t('statistics.upstreamMessage.messageCount'),
      type: 'line' as const,
      smooth: true,
      label: {
        show: true
...
const interfaceSummaryOption = reactive({
  ...
  series: [
    {
      name: t('statistics.interfaceSummary.callbackCount'),
      type: 'bar' as const,
      label: {
        show: true
      },
    ...
    {
      name: t('statistics.interfaceSummary.failCount'),
      type: 'bar' as const,
      label: {
        show: true
      },
    ...
    {
      name: t('statistics.interfaceSummary.maxTimeCost'),
      type: 'bar' as const,
      label: {
        show: true
      },
    ...
    {
      name: t('statistics.interfaceSummary.totalTimeCost'),
      type: 'bar' as const,
      label: {
        show: true
      },
...
const getSummary = () => {
  ...
  if (betweenDay(dateRange.value[0], dateRange.value[1]) >= 7) {
    message.error(t('statistics.dateRangeLimit'))
    return false
  }
  // 清空横坐标日期
  xAxisDate.value = []
