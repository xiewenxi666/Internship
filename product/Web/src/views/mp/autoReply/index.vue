  ...
  <ContentWrap>
    <el-tabs v-model="msgType" @tab-change="onTabChange">
      ...
      <el-tab-pane :name="MsgType.Follow">
        <template #label>
          <el-row align="middle"><Icon icon="ep:star" class="mr-2px" /> {{ t('autoReply.typeSubscribe') }}</el-row>
        </template>
      </el-tab-pane>
      <el-tab-pane :name="MsgType.Message">
        <template #label>
          <el-row align="middle"><Icon icon="ep:chat-line-round" class="mr-2px" /> {{ t('autoReply.typeMessage') }}</el-row>
        </template>
      </el-tab-pane>
      <el-tab-pane :name="MsgType.Keyword">
        <template #label>
          <el-row align="middle"><Icon icon="fa:newspaper-o" class="mr-2px" /> {{ t('autoReply.typeKeyword') }}</el-row>
        </template>
      </el-tab-pane>
    </el-tabs>
    <!-- 列表 -->
    ...
    <el-dialog
      :title="isCreating ? t('autoReply.createTitle') : t('autoReply.updateTitle')"
      v-model="showDialog"
      width="800px"
      destroy-on-close
    >
      ...
      <template #footer>
        ...
        <el-button type="primary" @click="onSubmit">{{ t('common.ok') }}</el-button>
      </template>
    </el-dialog>
  </ContentWrap>
</template>
...
const onDelete = async (id: number) => {
  await message.confirm(t('common.delDataMessage'))
