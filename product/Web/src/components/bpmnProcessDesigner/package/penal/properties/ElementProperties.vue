<template>
  <div class="panel-tab__content">
    <el-table :data="elementPropertyList" max-height="240" fit border :table-layout="'auto'">
      <el-table-column :label="t('common.index')" width="50px" type="index" />
      <el-table-column :label="t('designer.propertyName')" prop="name" min-width="100px" show-overflow-tooltip />
      <el-table-column :label="t('designer.propertyValue')" prop="value" min-width="100px" show-overflow-tooltip />
    ...
    <el-dialog
      ...
      :title="t('designer.propertyConfig')"
      width="600px"
      append-to-body
      destroy-on-close
    >
      <el-form :model="propertyForm" label-width="80px" ref="attributeFormRef">
        <el-form-item :label="t('designer.propertyName') + '：'" prop="name">
          <el-input v-model="propertyForm.name" clearable />
        </el-form-item>
        <el-form-item :label="t('designer.propertyValue') + '：'" prop="value">
          <el-input v-model="propertyForm.value" clearable />
        </el-form-item>
      </el-form>
      <template #footer>
        <el-button @click="propertyFormModelVisible = false">{{ t('common.cancel') }}</el-button>
        <el-button type="primary" @click="saveAttribute">{{ t('common.confirm') }}</el-button>
      </template>
    </el-dialog>
  </div>
</template>
...
const removeAttributes = (attr, index) => {
  ...
  ElMessageBox.confirm(t('designer.confirmRemoveProperty'), t('common.tip'), {
