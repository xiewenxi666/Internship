<template>
  <div class="panel-tab__content">
    ...
    <div v-else>
      {{ t('designer.multiInstanceNotImplemented') }}
    </div>
    <!-- 与Simple设计器配置合并，保留以前的代码 -->
    <el-form label-width="90px" style="display: none">
      <el-form-item :label="t('common.quickConfig')">
        <el-button size="small" @click="changeConfig('依次审批')">{{ t('designer.sequentialApprove') }}</el-button>
        <el-button size="small" @click="changeConfig('会签')">{{ t('designer.countersign') }}</el-button>
        <el-button size="small" @click="changeConfig('或签')">{{ t('designer.orSign') }}</el-button>
      </el-form-item>
      <el-form-item :label="t('designer.multiInstanceType')">
        <el-select v-model="loopCharacteristics" @change="changeLoopCharacteristicsType">
          <el-option :label="t('designer.parallelMultiInstance')" value="ParallelMultiInstance" />
          <el-option :label="t('designer.sequentialMultiInstance')" value="SequentialMultiInstance" />
      ...
      <template
        ...
        "
      >
        <el-form-item :label="t('designer.loopCardinality')" key="loopCardinality">
          <el-input
        ...
        <el-form-item :label="t('designer.collection')" key="collection" v-show="false">
          <el-input v-model="loopInstanceForm.collection" clearable @change="updateLoopBase" />
        </el-form-item>
        <!-- add by 芋艿：由于「元素变量」暂时用不到，所以这里 display 为 none -->
        <el-form-item :label="t('designer.elementVariable')" key="elementVariable" style="display: none">
          <el-input v-model="loopInstanceForm.elementVariable" clearable @change="updateLoopBase" />
        </el-form-item>
        <el-form-item :label="t('designer.completionCondition')" key="completionCondition">
          <el-input
        ...
        <el-form-item :label="t('designer.asyncStatus')" key="async" style="display: none">
          <el-checkbox
            v-model="loopInstanceForm.asyncBefore"
            :label="t('designer.asyncBefore')"
            :value="t('designer.asyncBefore')"
            @change="updateLoopAsync('asyncBefore')"
          />
          <el-checkbox
            v-model="loopInstanceForm.asyncAfter"
            :label="t('designer.asyncAfter')"
            :value="t('designer.asyncAfter')"
            @change="updateLoopAsync('asyncAfter')"
          />
          <el-checkbox
            v-model="loopInstanceForm.exclusive"
            v-if="loopInstanceForm.asyncAfter || loopInstanceForm.asyncBefore"
            :label="t('designer.exclude')"
            :value="t('designer.exclude')"
            @change="updateLoopAsync('exclusive')"
          />
        </el-form-item>
        <el-form-item
          :label="t('designer.retryCycle')"
          prop="timeCycle"
          v-if="loopInstanceForm.asyncAfter || loopInstanceForm.asyncBefore"
          key="timeCycle"
        >
