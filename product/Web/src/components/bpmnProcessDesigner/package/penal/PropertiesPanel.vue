  ...
  <div class="process-panel__container" :style="{ width: `${width}px`, maxHeight: '600px' }">
    <el-collapse v-model="activeTab" v-if="isReady">
      ...
      <el-collapse-item name="condition" v-if="elementType === 'Process'" key="message">
        <template #title><Icon icon="ep:comment" />{{ t('designer.messageAndSignal') }}</template>
        <signal-and-massage />
      </el-collapse-item>
      <el-collapse-item name="condition" v-if="conditionFormVisible" key="condition">
        <template #title><Icon icon="ep:promotion" />{{ t('designer.flowCondition') }}</template>
        <flow-condition :business-object="elementBusinessObject" :type="elementType" />
      </el-collapse-item>
      <el-collapse-item name="condition" v-if="formVisible" key="form">
        <template #title><Icon icon="ep:list" />{{ t('designer.form') }}</template>
      ...
      <el-collapse-item
        ...
        key="multiInstance"
      >
        <template #title><Icon icon="ep:help-filled" />{{ t('designer.multiInstanceApproval') }}</template>
      ...
      <el-collapse-item name="listeners" key="listeners">
        <template #title><Icon icon="ep:bell-filled" />{{ t('designer.executionListener') }}</template>
        <element-listeners :id="elementId" :type="elementType" />
      </el-collapse-item>
      <el-collapse-item name="taskListeners" v-if="elementType === 'UserTask'" key="taskListeners">
        <template #title><Icon icon="ep:bell-filled" />{{ t('designer.taskListener') }}</template>
        <user-task-listeners :id="elementId" :type="elementType" />
      </el-collapse-item>
      <el-collapse-item name="extensions" key="extensions">
        <template #title><Icon icon="ep:circle-plus-filled" />{{ t('designer.extensionProperties') }}</template>
        <element-properties :id="elementId" :type="elementType" />
      </el-collapse-item>
      <el-collapse-item name="other" key="other">
        <template #title><Icon icon="ep:promotion" />{{ t('designer.other') }}</template>
        <element-other-config :id="elementId" />
      </el-collapse-item>
      <el-collapse-item name="customConfig" key="customConfig">
        <template #title><Icon icon="ep:tools" />{{ t('designer.customConfig') }}</template>
      ...
      <el-collapse-item v-if="elementType === 'IntermediateCatchEvent'" name="timeEvent">
        <template #title><Icon icon="ep:timer" />{{ t('designer.timeEvent') }}</template>
        <!-- 相关 issue：https://gitee.com/mitedtsmcode/mitedtsm-ui-admin-vue3/issues/ICNRW2 -->
