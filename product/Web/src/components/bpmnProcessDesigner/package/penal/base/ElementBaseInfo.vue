  ...
  <div class="panel-tab__content">
    <el-form label-width="90px" :model="needProps" :rules="rules">
      <div v-if="needProps.type == 'bpmn:Process'">
        ...
        <el-form-item :label="t('designer.processKey')" prop="id">
          <el-input
            v-model="needProps.id"
            :placeholder="t('designer.processKeyPlaceholder')"
            :disabled="needProps.id !== undefined && needProps.id.length > 0"
            @change="handleKeyUpdate"
          />
        </el-form-item>
        <el-form-item :label="t('designer.processName')" prop="name">
          <el-input
            v-model="needProps.name"
            :placeholder="t('designer.processNamePlaceholder')"
            clearable
      ...
      <div v-else>
        <el-form-item :label="t('designer.nodeId')">
          <el-input v-model="elementBaseInfo.id" clearable @change="updateBaseInfo('id')" />
        </el-form-item>
        <el-form-item :label="t('designer.nodeName')">
...
const rules = reactive({
  id: [{ required: true, message: t('designer.processKeyRequired'), trigger: 'blur' }],
  name: [{ required: true, message: t('designer.processNameRequired'), trigger: 'blur' }]
})
