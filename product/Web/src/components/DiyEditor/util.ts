...
// 页面组件库 - 名称通过 i18n 动态获取
import { useI18n } from '@/hooks/web/useI18n'

export const getPageLibs = (): DiyComponentLibrary[] => {
  const { t } = useI18n('mall.promotion.diy')
  return [
    {
      name: t('editor.library.basicComponents'),
      extended: true,
      components: [
        'SearchBar',
        'NoticeBar',
    ...
    {
      name: t('editor.library.imageTextComponents'),
      extended: true,
      components: [
        'ImageBar',
        'Carousel',
    ...
    { name: t('editor.library.productComponents'), extended: true, components: ['ProductCard', 'ProductList'] },
    {
      name: t('editor.library.userComponents'),
      extended: true,
      components: ['UserCard', 'UserOrder', 'UserWallet', 'UserCoupon']
    },
    {
      name: t('editor.library.marketingComponents'),
      extended: true,
      components: [
...
// 保持向后兼容的常量（在组件中应使用 getPageLibs()）
export const PAGE_LIBS: DiyComponentLibrary[] = [
  {
    name: '基础组件',
    extended: true,
    components: [
