<template>
  <div v-loading="loading" class="overflow-auto">
    ...
    />
    <Dialog v-model="errorDialogVisible" :title="t('simpleDesigner.saveFailed')" width="400" :fullscreen="false">
      <div class="mb-2">{{ t('simpleDesigner.incompleteNodes') }}</div>
      <div
        class="mb-3 b-rounded-1 bg-gray-100 p-2 line-height-normal"
        v-for="(item, index) in errorNodes"
        :key="index"
      >
        {{ item.name }} : {{ getNodeDefaultText(item.type) }}
      </div>
      <template #footer>
        <el-button type="primary" @click="errorDialogVisible = false">{{ t('simpleDesigner.gotIt') }}</el-button>
      </template>
    </Dialog>
  </div>
</template>
...
import { SimpleFlowNode, NodeType, NodeId, NODE_DEFAULT_TEXT_KEY } from './consts'
...
const getNodeDefaultText = (type: NodeType): string => {
  const key = NODE_DEFAULT_TEXT_KEY.get(type)
...
const updateModel = () => {
  if (!processNodeTree.value) {
    processNodeTree.value = {
      name: t('simpleDesigner.initiator'),
      type: NodeType.START_USER_NODE,
      id: NodeId.START_USER_NODE_ID,
      childNode: {
        id: NodeId.END_EVENT_NODE_ID,
        name: t('simpleDesigner.end'),
