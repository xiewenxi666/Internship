  ...
  >
    ...
    <div>
      <el-form ref="formRef" :model="configForm" label-position="top" :rules="formRules">
        <el-form-item :label="t('simpleDesigner.delayTime')" prop="delayType">
          <el-radio-group v-model="configForm.delayType">
            <el-radio-button
              v-for="item in DELAY_TYPE"
              :key="item.value"
              :label="item.value === DelayTypeEnum.FIXED_TIME_DURATION ? t('simpleDesigner.fixedDuration') : t('simpleDesigner.fixedDate')"
              :value="item.value"
            />
          </el-radio-group>
        </el-form-item>
        <el-form-item v-if="configForm.delayType === DelayTypeEnum.FIXED_TIME_DURATION">
          ...
          <el-text>{{ t('simpleDesigner.enterNextNode') }}</el-text>
        </el-form-item>
...
const formRules = reactive({
  delayType: [{ required: true, message: t('simpleDesigner.validation.timeoutRequired'), trigger: 'change' }],
  timeDuration: [{ required: true, message: t('simpleDesigner.validation.timeoutRequired'), trigger: 'change' }],
  dateTime: [{ required: true, message: t('simpleDesigner.validation.timeoutRequired'), trigger: 'change' }]
})
// 配置表单数据
