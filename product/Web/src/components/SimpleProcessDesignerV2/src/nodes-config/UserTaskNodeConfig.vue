<template>
  ...
  >
    ...
    <div v-if="currentNode.type === NodeType.USER_TASK_NODE" class="flex flex-items-center mb-3">
      <span class="font-size-16px mr-3">{{ t('simpleDesigner.approvalTypeLabel') }} :</span>
    ...
    <el-tabs type="border-card" v-model="activeTabName" v-if="approveType === ApproveType.USER">
      <el-tab-pane :label="nodeTypeName === '审批' ? t('simpleDesigner.approver') : t('simpleDesigner.handler')" name="user">
        <div>
          <el-form ref="formRef" :model="configForm" label-position="top" :rules="formRules">
            <el-form-item :label="nodeTypeName === '审批' ? t('simpleDesigner.approverSetting') : t('simpleDesigner.handlerSetting')" prop="candidateStrategy">
              <el-radio-group
            ...
            <el-form-item
              ...
              :label="t('simpleDesigner.specifyRoleLabel')"
              prop="roleIds"
            >
              <el-select
                filterable
            ...
            <el-form-item
              ...
              "
              :label="t('simpleDesigner.specifyDeptLabel')"
              prop="deptIds"
              span="24"
            >
              <el-tree-select
            ...
            <el-form-item
              ...
              :label="t('simpleDesigner.specifyPostLabel')"
              prop="postIds"
              span="24"
            >
              <el-select
            ...
            <el-form-item
              ...
              :label="t('simpleDesigner.specifyUserLabel')"
              prop="userIds"
              span="24"
            >
              <el-select
            ...
            <el-form-item
              ...
              :label="t('simpleDesigner.specifyUserGroupLabel')"
              prop="userGroups"
            >
              <el-select
                filterable
            ...
            <el-form-item
              ...
              :label="t('simpleDesigner.formUserFieldLabel')"
              prop="formUser"
            >
            ...
            <el-form-item
              ...
              :label="t('simpleDesigner.formDeptFieldLabel')"
              prop="formDept"
            >
            ...
            <el-form-item
              ...
              :label="t('simpleDesigner.processExpressionLabel')"
              prop="expression"
            >
              <el-input
                type="textarea"
            ...
            <el-form-item :label="t('simpleDesigner.multiApproveMethod')" prop="approveMethod">
            ...
            <div v-if="currentNode.type === NodeType.USER_TASK_NODE">
              <el-divider content-position="left">{{ t('simpleDesigner.rejectHandlerTitle') }}</el-divider>
              ...
              <el-form-item
                ...
                :label="t('simpleDesigner.returnNodeLabel')"
                prop="returnNodeId"
              >
                <el-select
                  filterable
            ...
            <div v-if="currentNode.type === NodeType.USER_TASK_NODE">
              <el-divider content-position="left">{{ t('simpleDesigner.timeoutHandlerTitle') }}</el-divider>
              <el-form-item :label="t('simpleDesigner.enableSwitch')" prop="timeoutHandlerEnable">
                <el-switch
                  v-model="configForm.timeoutHandlerEnable"
                  :active-text="t('simpleDesigner.on')"
                  :inactive-text="t('simpleDesigner.off')"
                  @change="timeoutHandlerChange"
                />
              </el-form-item>
              <el-form-item
                :label="t('simpleDesigner.executeAction')"
              ...
              <el-form-item :label="t('simpleDesigner.timeoutSetting')" v-if="configForm.timeoutHandlerEnable">
                <span class="mr-2">{{ t('simpleDesigner.whenExceed') }}</span>
                ...
                >
                  <el-option
                    ...
                    :key="item.value"
                    :label="t(`simpleDesigner.timeUnit.${item.value === TimeUnitType.MINUTE ? 'minute' : item.value === TimeUnitType.HOUR ? 'hour' : 'day'}`)"
                    :value="item.value"
                  />
                </el-select>
                {{ t('simpleDesigner.notHandled') }}
              </el-form-item>
              <el-form-item
                :label="t('simpleDesigner.maxReminderCount')"
                prop="maxRemindCount"
            ...
            <el-divider content-position="left">{{ nodeTypeName === '审批' ? t('simpleDesigner.assignEmptyTitle') : t('simpleDesigner.handlerSetting') }}</el-divider>
            ...
            <div v-if="currentNode.type === NodeType.USER_TASK_NODE">
              <el-divider content-position="left">{{ t('simpleDesigner.assignStartUserTitle') }}</el-divider>
            ...
            <div v-if="currentNode.type === NodeType.USER_TASK_NODE">
              <el-divider content-position="left">{{ t('simpleDesigner.signRequiredTitle') }}</el-divider>
              <el-form-item prop="signEnable">
                <el-switch v-model="configForm.signEnable" :active-text="t('simpleDesigner.yes')" :inactive-text="t('simpleDesigner.no')" />
              </el-form-item>
            </div>

            <div v-if="currentNode.type === NodeType.USER_TASK_NODE">
              <el-divider content-position="left">{{ t('simpleDesigner.approvalOpinionTitle') }}</el-divider>
              <el-form-item prop="reasonRequire">
                <el-switch
                  v-model="configForm.reasonRequire"
                  :active-text="t('simpleDesigner.required')"
                  :inactive-text="t('simpleDesigner.notRequired')"
                />
              </el-form-item>
            </div>
            <div>
              <el-divider content-position="left">{{ t('simpleDesigner.skipExpressionTitle') }}</el-divider>
      ...
      <el-tab-pane
        :label="t('simpleDesigner.buttonSettingTitle')"
        v-if="currentNode.type === NodeType.USER_TASK_NODE"
        name="buttons"
      >
        <div class="button-setting-pane">
          <div class="button-setting-desc">{{ t('simpleDesigner.operationButton') }}</div>
          <div class="button-setting-title">
            <div class="button-title-label">{{ t('simpleDesigner.operationButton') }}</div>
            <div class="pl-4 button-title-label">{{ t('simpleDesigner.displayName') }}</div>
            <div class="button-title-label">{{ t('simpleDesigner.enableLabel') }}</div>
          </div>
      ...
      <el-tab-pane :label="t('simpleDesigner.listener')" name="listener">
        <UserTaskListener
...
const deptLevelLabel = computed(() => {
  let label = t('simpleDesigner.deptLeaderSourceLabel')
  if (configForm.value.candidateStrategy == CandidateStrategy.MULTI_LEVEL_DEPT_LEADER) {
    label = label + t('simpleDesigner.fromAssignedDept')
  } else if (configForm.value.candidateStrategy == CandidateStrategy.FORM_DEPT_LEADER) {
    label = label + t('simpleDesigner.fromFormDept')
  } else {
    label = label + t('simpleDesigner.fromInitiatorDept')
  }
  return label
})
// 监控节点的变化
...
const formRules = reactive({
  candidateStrategy: [{ required: true, message: t('simpleDesigner.validation.approverSettingRequired'), trigger: 'change' }],
  userIds: [{ required: true, message: t('simpleDesigner.validation.userRequired'), trigger: 'change' }],
  roleIds: [{ required: true, message: t('simpleDesigner.validation.roleRequired'), trigger: 'change' }],
  deptIds: [{ required: true, message: t('simpleDesigner.validation.deptRequired'), trigger: 'change' }],
  userGroups: [{ required: true, message: t('simpleDesigner.validation.userGroupRequired'), trigger: 'change' }],
  formUser: [{ required: true, message: t('simpleDesigner.validation.formUserFieldRequired'), trigger: 'change' }],
  formDept: [{ required: true, message: t('simpleDesigner.validation.formDeptFieldRequired'), trigger: 'change' }],
  postIds: [{ required: true, message: t('simpleDesigner.validation.postRequired'), trigger: 'change' }],
  expression: [{ required: true, message: t('simpleDesigner.validation.expressionRequired'), trigger: 'blur' }],
  approveMethod: [{ required: true, message: t('simpleDesigner.validation.multiApproveMethodRequired'), trigger: 'change' }],
  approveRatio: [{ required: true, message: t('simpleDesigner.validation.approveRatioRequired'), trigger: 'blur' }],
  returnNodeId: [{ required: true, message: t('simpleDesigner.validation.returnNodeRequired'), trigger: 'change' }],
  timeoutHandlerEnable: [{ required: true }],
  timeoutHandlerType: [{ required: true }],
  timeDuration: [{ required: true, message: t('simpleDesigner.validation.timeoutRequired'), trigger: 'blur' }],
  maxRemindCount: [{ required: true, message: t('simpleDesigner.validation.reminderCountRequired'), trigger: 'blur' }],
  assignEmptyHandlerType: [{ required: true }],
  assignEmptyHandlerUserIds: [{ required: true, message: t('simpleDesigner.validation.userRequired'), trigger: 'change' }],
