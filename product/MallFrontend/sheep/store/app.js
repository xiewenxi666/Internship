...
import sys from './sys';
import { baseUrl, h5Url, staticUrl } from '@/sheep/config';
...
const app = defineStore({
  ...
  actions: {
    ...
    async init(templateId = null) {
      ...
      if (true) {
        this.info = {
          ...
          cdnurl: staticUrl === 'local' ? '' : (staticUrl || 'https://file.sheepjs.com'), // 云存储域名
    ...
    // 刷新装修模板（用于语言切换后重新获取翻译后的数据）
    async refreshTemplate(templateId = null) {
      await adaptTemplate(this.template, templateId);
    },
  },
  persist: {
