...
<template>
  <su-popup :show="show" type="bottom" round="20" @close="emits('close')" showClose>
    <view class="model-box">
      <view class="title ss-m-t-16 ss-m-l-20 ss-flex">{{ t('discount') }}</view>
      <view v-if="state.rewardActivity && state.rewardActivity.id > 0">
        <view class="titleLi">{{ t('promotion') }}</view>
        <scroll-view
          class="model-content"
          scroll-y
      ...
      <view class="titleLi">{{ t('couponsAvailable') }}</view>
      <scroll-view
        class="model-content"
        scroll-y
        :scroll-with-animation="false"
        ...
        <view class="actBox" v-for="item in state.couponInfo" :key="item.id">
          <view class="boxCont ss-flex ss-col-top ss-m-b-40">
            <view class="model-content-tag2">
              ...
              <view class="impose"> {{ t('coupon.minAmount', { amount: fen2yuan(item.usePrice) }) }} </view>
            </view>
              ...
              <view class="ss-m-b-24 cotBu-txt">
                {{
                  item.validityType == 1
                    ...
                    : t('coupon.validDays', { days: item.fixedStartTerm + '-' + item.fixedEndTerm })
                }}
              </view>
            </view>
            <view class="coupon" @click.stop="getBuy(item.id)" v-if="item.canTake"> {{ t('coupon.receive') }} </view>
            <view class="coupon2" v-else> {{ t('coupon.received') }} </view>
          </view>
        </view>
      </scroll-view>
      <view class="nullBox" v-else> {{ t('noCouponsAvailable') }} </view>
    </view>
  </su-popup>
</template>
<script setup>
