...
<template>
  <su-popup :show="show" round="10" @close="onClosePoster" type="center" class="popup-box">
    ...
    <view
      ...
      v-if="!!painterImageUrl"
    >
      <button class="cancel-btn ss-reset-button" @tap="onClosePoster">{{ t('cancel') }}</button>
...
<script setup>
  ...
  const onSavePoster = () => {
    if (['WechatOfficialAccount', 'H5'].includes(sheep.$platform.name)) {
      sheep.$helper.toast(t('longPressSave'));
      return;
    }
    ...
    uni.saveImageToPhotosAlbum({
      ...
      success: (res) => {
        onClosePoster();
        sheep.$helper.toast(t('saveSuccess'));
      },
      fail: (err) => {
        sheep.$helper.toast(t('saveFailed'));
