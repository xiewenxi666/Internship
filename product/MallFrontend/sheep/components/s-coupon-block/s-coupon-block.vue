        ...
        >
          <template v-slot:btn>
            ...
            <button
              ...
              :style="[btnStyles]"
            >
              <view class="btn-text">{{ t('claimNow') }}</view>
            </button>
            <button
              v-else
...
<script setup>
  ...
  const formatCouponDiscountType = (coupon) => {
    ...
    return t('unknownType', { type: coupon.discountType });
  };
  ...
  const formatValidityType = (row) => {
    if (row.validityType === CouponTemplateValidityTypeEnum.DATE.type) {
      return t('validFromTo', { start: formatDate(row.validStartTime), end: formatDate(row.validEndTime) });
    }
    if (row.validityType === CouponTemplateValidityTypeEnum.TERM.type) {
      return t('validDaysRange', { start: row.fixedStartTerm, end: row.fixedEndTerm });
    }
    return t('unknownType', { type: row.validityType });
  };

  // 格式化【优惠券标题】
  const formatCouponTitle = (coupon) => {
    const amount = floatToFixed2(coupon.usePrice);
    if (coupon.discountType === PromotionDiscountTypeEnum.PRICE.type) {
      return t('fullReduceFormat', { amount, discount: floatToFixed2(coupon.discountPrice) });
    }
    if (coupon.discountType === PromotionDiscountTypeEnum.PERCENT.type) {
      return t('percentFormat', { amount, percent: formatDiscountPercent(coupon.discountPercent) });
    }
    return t('unknownType', { type: coupon.discountType });
  };
