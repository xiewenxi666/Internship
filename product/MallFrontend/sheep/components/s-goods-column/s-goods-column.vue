...
<template>
  <view class="ss-goods-wrap">
    ...
    >
      ...
      <view v-if="seckillTag" class="seckill-tag ss-flex ss-row-center">{{ t('seckill') }}</view>
      <view v-if="grouponTag" class="groupon-tag ss-flex ss-row-center">
        <view class="tag-icon">{{ t('groupon') }}</view>
      </view>
      <image
        class="lg-img-box"
        :src="sheep.$url.cdn(data.image || data.picUrl)"
      ...
      <slot name="cart">
        <view class="buy-box ss-flex ss-col-center ss-row-center" v-if="buttonShow">{{ t('goBuy') }}</view>
      </slot>
    </view>
    ...
    <view v-if="size === 'sl'" class="sl-goods-card ss-flex-col" :style="[elStyles]" @tap="onClick">
      ...
      <slot name="cart">
        <view class="buy-box ss-flex ss-col-center ss-row-center">{{ t('goBuy') }}</view>
      </slot>
    </view>
  </view>
</template>
...
<script setup>
  ...
  const discountText = computed(() => {
    ...
    if (promotionType === 4) {
      return t('limitedOffer');
    } else if (promotionType === 6) {
      return t('memberPrice');
    }
    return undefined;
  });
