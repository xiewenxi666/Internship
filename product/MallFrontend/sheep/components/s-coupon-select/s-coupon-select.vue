...
<template>
  ...
  >
    <view class="model-box">
      <view class="title ss-m-t-16 ss-m-l-20 ss-flex">{{ t('title') }}</view>
      <scroll-view
        class="model-content"
        scroll-y
      ...
      >
        <!--可使用的优惠券区域-->
        <view class="subtitle ss-m-l-20">{{ t('available') }}</view>
        <view
        ...
        <!--不可使用的优惠券区域-->
        <view class="subtitle ss-m-t-40 ss-m-l-20">{{ t('noAvailableCoupon') }}</view>
          ...
          <s-coupon-list :data="item" type="user" :disabled="true">
            <template v-slot:reason>
              <view class="ss-flex ss-m-t-24">
                <view class="reason-title"> {{ t('noAvailableCoupon') }}：</view>
                <view class="reason-desc">{{ item.mismatchReason || t('minAmount', { amount: 0 }) }}</view>
              </view>
            </template>
          </s-coupon-list>
        </view>
    ...
    <view class="modal-footer ss-flex">
      <button class="confirm-btn ss-reset-button" @tap="onConfirm">{{ t('confirm') }}</button>
    </view>
  </su-popup>
</template>
<script setup>
