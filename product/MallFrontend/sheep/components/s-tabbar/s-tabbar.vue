...
<script setup>
  ...
  // TabBar URL 到国际化 key 的映射
  const tabBarKeyMap = {
    '/pages/index/index': 'tabBar.home',
    '/pages/index/category': 'tabBar.category',
    '/pages/index/cart': 'tabBar.cart',
    '/pages/index/user': 'tabBar.mine',
  };

  // 获取国际化后的 TabBar 文本
  const getTabBarText = (item) => {
    const key = tabBarKeyMap[item.url];
