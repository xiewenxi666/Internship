        ...
        <view class="header-right ss-flex-col ss-row-between ss-flex-1">
          <view class="goods-title ss-line-2">
            <view class="tig ss-flex ss-col-center">
              ...
              <view class="tig-title">{{ t('groupPrice') }}</view>
            </view>
          ...
          <view class="header-right-bottom ss-flex ss-col-center ss-row-between">
            ...
            <view class="stock-text ss-m-l-20">
              {{ t('stock') }}{{ state.selectedSku.stock || goodsInfo.stock }}{{ t('stockUnit') }}
            </view>
          </view>
        </view>
      </view>
      <view class="modal-content ss-flex-1">
        <scroll-view scroll-y="true" class="modal-content-scroll">
          ...
          <view class="buy-num-box ss-flex ss-col-center ss-row-between">
            <view class="label-text">{{ t('buyQuantity') }}</view>
            <su-number-box
              :min="1"
        ...
        <view class="buy-box ss-flex ss-col-center ss-flex ss-col-center ss-row-center">
          <view class="ss-flex">
            <button class="ss-reset-button origin-price-btn ss-flex-col">
              <view class="btn-title">{{ t('peopleGroup', { count: grouponNum }) }}</view>
            </button>
            <button class="ss-reset-button btn-tox ss-flex-col" @tap="onBuy">
              ...
              <view v-if="grouponAction === 'create'">{{ t('startGroup') }}</view>
              <view v-else-if="grouponAction === 'join'">{{ t('joinGroup') }}</view>
            </button>
          </view>
        </view>
      </view>
...
<script setup>
  ...
  function onBuy() {
    if (!state.selectedSku.id || state.selectedSku.id <= 0) {
      sheep.$helper.toast(t('selectSku'));
      return;
    }
    if (state.selectedSku.stock <= 0) {
      sheep.$helper.toast(t('stockInsufficient'));
      return;
    }
