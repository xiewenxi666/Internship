  ...
  <view class="ss-user-info-wrap ss-p-t-50" :style="[bgStyle, { marginLeft: `${data.space}px` }]">
      ...
      <view class="left-box ss-flex ss-col-center ss-m-l-36">
        ...
        <view>
          <view class="nickname-box ss-flex ss-col-center">
            <view class="nick-name ss-m-r-20">{{ userInfo?.nickname || nickname || t('pleaseLogin') }}</view>
          </view>
        </view>
      </view>
    ...
    >
      <view class="ss-flex">
        ...
        <view class="mobile-title ss-m-l-20"> {{ t('bindPhoneHint') }}</view>
      </view>
      <button class="ss-reset-button bind-btn" @tap="onBind">{{ t('goBind') }}</button>
    </view>
  </view>
</template>
