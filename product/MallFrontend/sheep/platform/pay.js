...
export default class SheepPay {
  ...
  async wechatOfficialAccountPay() {
    ...
    $wxsdk.wxpay(payConfig, {
      ...
      cancel: () => {
        sheep.$helper.toast(t('pay.canceled'));
      },
      fail: (error) => {
        if (error.errMsg.indexOf('chooseWXPay:没有此SDK或暂不支持此SDK模拟') >= 0) {
          sheep.$helper.toast(t('pay.wechatPayFailed'));
          return;
        }
        this.payResult('fail');
      },
  ...
  async copyPayLink() {
    ...
    uni.showModal({
      title: t('pay.alipayPay'),
      content: t('pay.copyLinkHint'),
  ...
  async wechatAppPay() {
    ...
    if (code !== 0) {
      sheep.$helper.toast(t('pay.getPayInfoFailed'));
      return;
    }
    ...
    uni.requestPayment({
      ...
      fail: (err) => {
        if (err.errMsg === 'requestPayment:fail cancel') {
          ...
          sheep.$helper.toast(t('pay.failed') + '：' + err.errMsg);
          that.payResult('fail');
        }
      },
    });
...
export function goBindWeixin() {
  uni.showModal({
    title: t('pay.wechatPay'),
    content: t('pay.bindWechatHint'),
