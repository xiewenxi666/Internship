<!-- 搜索界面 -->
<template>
  <s-layout :bgStyle="{ color: '#FFF' }" class="set-wrap" :title="t('pageTitle.search')">
    <view class="ss-p-x-24">
      ...
      <view class="ss-flex ss-row-between ss-col-center">
        <view class="serach-history">{{ t('searchHistory') }}</view>
        <button class="clean-history ss-reset-button" @tap="onDelete">{{ t('clearSearchHistory') }}</button>
      </view>
