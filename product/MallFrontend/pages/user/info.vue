<!-- 用户信息 -->
<template>
  <s-layout :title="t('userInfoTitle')" class="set-userinfo-wrap">
    <uni-forms
      :model="state.model"
      :rules="state.rules"
      labelPosition="left"
    ...
    >
      ...
      <view class="ss-flex ss-row-center ss-col-center ss-p-t-60 ss-p-b-0 bg-white">
        <view class="header-box-content">
          ...
          <view class="avatar-action">
            ...
            <!-- #endif -->
            <!-- #ifndef MP -->
            <button class="ss-reset-button avatar-action-btn" @tap="onChangeAvatar">{{ t('modify') }}</button>
            <!-- #endif -->
          </view>
        </view>
      </view>
      ...
      <view class="bg-white ss-p-x-30">
        <!-- 昵称 + 性别 -->
        <uni-forms-item name="nickname" :label="t('nickname')">
          <uni-easyinput
        ...
        <uni-forms-item name="sex" :label="t('gender')">
        ...
        <uni-forms-item name="mobile" :label="t('mobile')" @tap="onChangeMobile">
          <uni-easyinput
            v-model="userInfo.mobile"
            :placeholder="t('bindMobile')"
            :inputBorder="false"
            disabled
        ...
        <uni-forms-item name="password" :label="t('loginPassword')" @tap="onSetPassword">
          <uni-easyinput
            v-model="userInfo.password"
            :placeholder="t('clickModifyPassword')"
            :inputBorder="false"
            :styles="{ disableColor: '#fff' }"
            disabled
    ...
    <view v-if="sheep.$platform.name !== 'H5'">
      <view class="title-box ss-p-l-30">{{ t('thirdPartyBind') }}</view>
      <view class="account-list ss-flex ss-row-between">
        <view v-if="'WechatOfficialAccount' === sheep.$platform.name" class="ss-flex ss-col-center">
          ...
          />
          <text class="list-name">{{ t('wechatOA') }}</text>
        </view>
        <view v-if="'WechatMiniProgram' === sheep.$platform.name" class="ss-flex ss-col-center">
          ...
          />
          <text class="list-name">{{ t('wechatMP') }}</text>
        </view>
        <view v-if="'App' === sheep.$platform.name" class="ss-flex ss-col-center">
          ...
          />
          <text class="list-name">{{ t('wechatOpen') }}</text>
        </view>
        <view class="ss-flex ss-col-center">
          ...
          <view class="bind-box ss-m-l-20">
            ...
            <button v-else class="ss-reset-button bind-btn" @tap="bindThirdOauth">{{ t('bind') }}</button>
          </view>
        </view>
      </view>
    </view>
    ...
    <su-fixed bottom placeholder bg="none">
      <view class="footer-box ss-p-20">
        <button class="ss-rest-button logout-btn ui-Shadow-Main" @tap="onSubmit">{{ t('save') }}</button>
      </view>
    </su-fixed>
  </s-layout>
</template>
...
<script setup>
  ...
  function unBindThirdOauth() {
    uni.showModal({
      ...
      content: t('unbindConfirmContent'),
