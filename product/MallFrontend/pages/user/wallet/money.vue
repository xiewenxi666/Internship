<!-- 我的钱包 -->
<template>
  <s-layout class="wallet-wrap" :title="t('wallet')">
    <!-- 钱包卡片 -->
    <view class="header-box ss-flex ss-row-center ss-col-center">
      <view class="card-box ui-BG-Main ui-Shadow-Main">
        <view class="card-head ss-flex ss-col-center">
          <view class="card-title ss-m-r-10">{{ t('walletBalance') }}</view>
          <view
    ...
    <su-sticky>
      ...
      <view class="filter-box ss-p-x-30 ss-flex ss-col-center ss-row-between">
        ...
        <view class="total-box">
          <view class="ss-m-b-10">{{ t('totalIncome', { amount: fen2yuan(state.summary.totalIncome) }) }}</view>
          <view>{{ t('totalExpense', { amount: fen2yuan(state.summary.totalExpense) }) }}</view>
        </view>
      </view>
      <su-tabs
        :list="tabMaps"
    ...
    <s-empty v-if="state.pagination.total === 0" :text="t('noData')" icon="/static/data-empty.png" />
...
<script setup>
  ...
  async function getLogList() {
    ...
    const { data, code } = await PayWalletApi.getWalletTransactionPage({
      ...
      type: tabMaps.value[state.currentTab].value,
