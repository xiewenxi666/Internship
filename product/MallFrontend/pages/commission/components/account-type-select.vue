  ...
  <su-popup :show="show" class="ss-checkout-counter-wrap" @close="hideModal">
    <view class="ss-modal-box bg-white ss-flex-col">
      <view class="modal-header ss-flex-col ss-col-left">
        <text class="modal-title ss-m-b-20">{{ t('selectWithdrawMethod') }}</text>
      </view>
      ...
      <view class="modal-footer ss-flex ss-row-center ss-col-center">
        <button class="ss-reset-button save-btn" @tap="onConfirm">{{ t('confirm') }}</button>
      </view>
    </view>
  </su-popup>
</template>
...
<script setup>
  ...
  const typeList = computed(() => [
    ...
    },
    {
      ...
      title: t('wechatQrCode'), // 微信手动转账
      value: '3',
    },
    {
      icon: '/static/img/shop/pay/alipay.png',
      title: t('alipayQrCode'), // 支付宝手动转账
      value: '4',
    },
    {
      icon: '/static/img/shop/pay/wechat_api.png',
      title: t('wechatMoney'), // 微信 API 转账
      value: '5',
    },
    {
      icon: '/static/img/shop/pay/alipay_api.png',
      title: t('alipayBalance'), // 支付宝 API 转账
      value: '6',
    },
  ]);
