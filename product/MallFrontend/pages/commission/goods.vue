<!-- 分销商品列表  -->
<template>
  <s-layout :title="t('promoteGoods')" :onShareAppMessage="state.shareInfo">
        ...
        <template #rightBottom>
          <view class="ss-flex ss-row-between">
            <view class="commission-num" v-if="item.brokerageMinPrice === undefined">
              {{ t('expectedCommission') }}：{{ t('calculating') }}
            </view>
            <view
              class="commission-num"
              v-else-if="item.brokerageMinPrice === item.brokerageMaxPrice"
            >
              {{ t('expectedCommission') }}：{{ fen2yuan(item.brokerageMinPrice) }}
            </view>
...
<script setup>
  ...
  onReady(() => {
    setPageTitle('pages/commission/goods');
  });
