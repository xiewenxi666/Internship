<!-- 分销 - 订单明细 -->
<template>
  <s-layout :title="t('orderTitle')" :class="state.scrollTop ? 'order-warp' : ''" navbar="inner">
    <view
      class="header-box"
      :style="[
        {
      ...
      <view class="team-data-box ss-flex ss-col-center ss-row-between" style="width: 100%">
        <view class="data-card" style="width: 100%">
          <view class="total-item" style="width: 100%">
            <view class="item-title" style="text-align: center">{{ t('totalPromoteOrders') }}</view>
    ...
    <view class="order-box">
      <view class="order-item" v-for="item in state.pagination.list" :key="item">
        <view class="order-header">
          <view class="no-box ss-flex ss-col-center ss-row-between">
            <text class="order-code">{{ t('orderNo') }}：{{ item.bizId }}</text>
            <text class="order-state">
              {{ item.status === 0 ? t('statusPending') : item.status === 1 ? t('statusSettled') : t('statusCanceled') }}
              ( {{ t('orderCommission') }} {{ fen2yuan(item.price) }} {{ t('unitYuan') }} )
            </text>
          </view>
      ...
      <!-- 数据为空 -->
      <s-empty v-if="state.pagination.total === 0" icon="/static/order-empty.png" :text="t('noOrderData')" />
      <!-- 加载更多 -->
      <uni-load-more
...
<script setup>
  ...
  onReady(() => {
    setPageTitle('pages/commission/order');
  });
