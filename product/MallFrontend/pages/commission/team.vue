<!-- 页面 -->
<template>
  <s-layout :title="t('myTeam')" :class="state.scrollTop ? 'team-wrap' : ''" navbar="inner">
    <view
      class="header-box"
      :style="[
        {
      ...
      <view class="team-data-box ss-flex ss-col-center ss-row-between" style="width: 100%">
        <view class="data-card" style="width: 100%">
          <view class="total-item" style="width: 100%">
            <view class="item-title" style="text-align: center">{{ t('promoteCount') }}</view>
      ...
      <view style="padding: 0 20rpx">
        <view class="nav acea-row row-around l1" style="margin-top: 20rpx">
          <view :class="state.level == 1 ? 'item on' : 'item'" @click="setType(1)">
            {{ t('firstLevel') }}({{ state.summary.firstBrokerageUserCount || 0 }})
          </view>
          <view :class="state.level == 2 ? 'item on' : 'item'" @click="setType(2)">
            {{ t('secondLevel') }}({{ state.summary.secondBrokerageUserCount || 0 }})
          </view>
        </view>
        <view
        ...
        >
          <view class="input">
            <input
              :placeholder="t('clickSearchMember')"
        ...
        <view class="list">
          ...
          <block v-for="(item, index) in state.pagination.list" :key="index">
            <view class="item acea-row row-between-wrapper" style="display: flex">
              ...
              >
                <view>
                  <text class="num font-color">{{ item.brokerageUserCount || 0 }} </text>{{ t('person') }}
                </view>
                <view>
          ...
          <block v-if="state.pagination.list.length === 0">
            <view style="text-align: center; margin-top: 30rpx">{{ t('noPromoters') }}</view>
          </block>
        </view>
      </view>
    </view>
...
<script setup>
  ...
  onReady(() => {
    setPageTitle('pages/commission/team');
  });
