<!-- 分销 - 佣金明细 -->
<template>
  <s-layout class="wallet-wrap" :title="t('commission')">
    <!-- 钱包卡片 -->
    <view class="header-box ss-flex ss-row-center ss-col-center">
      <view class="card-box ui-BG-Main ui-Shadow-Main">
        <view class="card-head ss-flex ss-col-center">
          <view class="card-title ss-m-r-10">{{ t('currentCommission') }}</view>
          <view
    ...
    >
      <view class="ss-p-x-20 ss-p-y-30">
        <view class="model-title ss-m-b-30 ss-m-l-20">{{ t('toBalance') }}</view>
        <view class="model-subtitle ss-m-b-100 ss-m-l-20">{{ t('transferToBalanceTip') }}</view>
...
import { getCurrencyCode } from '@/sheep/config/currency';
  ...
  onReady(() => {
    setPageTitle('pages/commission/wallet');
  });
  ...
  async function onConfirm() {
    ...
    uni.showModal({
      title: t('tip'),
      content: t('confirmTransferToBalance'),
  ...
  async function onRequestMerchantTransfer(item) {
    ...
    if (!requestMerchantTransfer) {
      sheep.$helper.toast(t('onlyWechatSupport'));
      return;
    }
    // 获取提现详情
    ...
    if (data.status === 11) {
      sheep.$helper.toast(t('withdrawConfirmed'));
      item.status = 11;
      return;
    }
    if (!data.transferChannelMchId || !data.transferChannelPackageInfo) {
      sheep.$helper.toast(t('withdrawInfoError'));
      return;
    }
    // 调用微信确认收款
    ...
    await requestMerchantTransfer(
      ...
      async (res) => {
        ...
        if (data && data.status !== 11) {
          sheep.$helper.toast(t('confirmReceiptSuccessWithDelay'));
          return;
        }
        sheep.$helper.toast(t('confirmReceiptSuccess'));
        // 更新到列表中
        item.status = 11;
      },
    );
