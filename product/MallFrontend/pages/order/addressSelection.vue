  ...
  <view class="allAddress" :style="state.isPickUp ? '' : 'padding-top:10rpx;'">
    <view class="nav flex flex-wrap">
      <view
        ...
        v-if="state.isPickUp"
        :data-express-text="t('expressDelivery')"
        :data-pickup-text="t('storePickUp')"
      />
      <view
        class="item font-color"
    ...
    >
      <view class="addressCon" v-if="state.addressInfo.name">
        ...
        <view class="flex flex-wrap">
          <text class="default font-color" v-if="state.addressInfo.defaultStatus">[{{ t('default') }}]</text>
          <text class="line2">
      ...
      <view class="addressCon" v-else>
        <view class="setaddress">{{ t('setAddress') }}</view>
      </view>
      <view class="iconfont">
    ...
    >
      ...
      <view class="addressCon" v-else>
        <view class="setaddress">{{ t('selectPickUpStore') }}</view>
      </view>
      <view class="iconfont">
        <view class="ss-rest-button">
          <text class="_icon-forward" />
...
<style scoped lang="scss">
  ...
  .allAddress .nav .item.on::before {
    position: absolute;
    bottom: 0;
    content: attr(data-express-text);
    font-size: 28rpx;
    display: block;
    height: 0;
    width: 336rpx;
