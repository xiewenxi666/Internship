...
<template>
  <s-layout :title="t('title')">
    ...
    <s-empty v-if="state.pagination.total === 0" icon="/static/order-empty.png" :text="t('noOrder')" />
    <view v-if="state.pagination.total > 0">
      ...
      >
        <view class="order-card-header ss-flex ss-col-center ss-row-between ss-p-x-20">
          <view class="order-no">{{ t('orderNoLabel') }}{{ order.no }}</view>
        ...
        <view class="pay-box ss-m-t-30 ss-flex ss-row-right ss-p-r-20">
          <view class="ss-flex ss-col-center">
            <view class="discounts-title pay-color"
              >{{ t('productCount', { count: order.productCount }) }}</view
            >
