<!-- 订单详情 -->
<template>
  <s-layout :title="t('orderDetail')" class="index-wrap" navbar="inner">
    <view
    ...
    <view class="order-price-box">
      <view class="notice-item ss-flex ss-row-between">
        <text class="title">{{ t('goodsTotal') }}</text>
        <view class="ss-flex">
          <text class="detail">{{ fen2yuan(state.orderInfo.totalPrice) }}</text>
        </view>
      </view>
      <view class="notice-item ss-flex ss-row-between">
        <text class="title">{{ t('freight') }}</text>
      ...
      <view class="notice-item ss-flex ss-row-between" v-if="state.orderInfo.couponPrice > 0">
        <text class="title">{{ t('couponAmount') }}</text>
        <text class="detail">-{{ fen2yuan(state.orderInfo.couponPrice, false) }}</text>
      </view>
      <view class="notice-item ss-flex ss-row-between" v-if="state.orderInfo.pointPrice > 0">
        <text class="title">{{ t('pointDeduction') }}</text>
        <text class="detail">-{{ fen2yuan(state.orderInfo.pointPrice, false) }}</text>
      </view>
      <view class="notice-item ss-flex ss-row-between" v-if="state.orderInfo.discountPrice > 0">
        <text class="title">{{ t('activityDiscount') }}</text>
        <text class="detail">-{{ fen2yuan(state.orderInfo.discountPrice, false) }}</text>
      </view>
      <view class="notice-item ss-flex ss-row-between" v-if="state.orderInfo.vipPrice > 0">
        <text class="title">{{ t('vipDiscount') }}</text>
        <text class="detail">-{{ fen2yuan(state.orderInfo.vipPrice, false) }}</text>
      </view>
      <view class="notice-item all-rpice-item ss-flex ss-m-t-20">
        <text class="title">{{ state.orderInfo.payStatus ? t('paid') : t('toPay') }}</text>
      ...
      >
        <text class="title">{{ t('refunded') }}</text>
...
<script setup>
  ...
  onReady(() => {
    setPageTitle('pages/order/detail');
  });
  ...
  async function onConfirm(orderId, ignore = false) {
    ...
    uni.showModal({
      title: t('tip', {}, { scope: 'common' }),
      content: t('confirmReceiveConfirm'),
      success: async function (res) {
        if (!res.confirm) {
          return;
        }
  ...
  function mpConfirm(orderId) {
    if (!wx.openBusinessView) {
      sheep.$helper.toast(t('pleaseUpgradeWechat'));
      return;
    }
    wx.openBusinessView({
