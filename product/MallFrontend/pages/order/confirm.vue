  ...
  <s-layout :title="t('confirmOrder')">
    ...
    <view class="order-card-box ss-m-b-14">
      ...
      <view class="order-item ss-flex ss-col-center ss-row-between ss-p-x-20 bg-white ss-r-10">
        <view class="item-title">{{ t('remark') }}</view>
        <view class="ss-flex ss-col-center">
          <uni-easyinput
            maxlength="20"
            :placeholder="t('remarkPlaceholder2')"
    ...
    <view class="bg-white total-card-box ss-p-20 ss-m-b-14 ss-r-10">
      <view class="total-box-content border-bottom">
        <view class="order-item ss-flex ss-col-center ss-row-between">
          <view class="item-title">{{ t('goodsAmount') }}</view>
          <view class="ss-flex ss-col-center">
        ...
        >
          <view class="item-title">{{ t('exchangePoint') }}</view>
          <view class="ss-flex ss-col-center">
            <image
        ...
        >
          <view class="item-title">{{ t('pointDeduction') }}</view>
          <view class="ss-flex ss-col-center">
            {{ state.pointStatus || state.orderPayload.pointActivityId ? t('remainingPoint') : t('currentPoint') }}
            <image
              :src="sheep.$url.static('/static/img/shop/goods/score1.svg')"
              class="score-img"
            />
        ...
        >
          <view class="item-title">{{ t('freight') }}</view>
          <view class="ss-flex ss-col-center">
            <text class="item-value ss-m-r-24" v-if="state.orderInfo.price.deliveryPrice > 0">
              +{{ fen2yuan(state.orderInfo.price.deliveryPrice) }}
            </text>
            <view class="item-value ss-m-r-24" v-else>{{ t('freightFree') }}</view>
          </view>
        </view>
        <!-- 门店自提时，需要填写姓名和手机号 -->
        <view
        ...
        >
          <view class="item-title">{{ t('contactName') }}</view>
          <view class="ss-flex ss-col-center">
            <uni-easyinput
              maxlength="20"
        ...
        <view
          ...
          v-if="addressState.deliveryType === 2"
        >
          <view class="item-title">{{ t('contactPhone') }}</view>
          <view class="ss-flex ss-col-center">
            <uni-easyinput
              maxlength="20"
        ...
        >
          <view class="item-title">{{ t('coupon') }}</view>
          <view class="ss-flex ss-col-center" @tap="state.showCoupon = true">
            <text class="item-value text-red" v-if="state.orderPayload.couponId > 0">
              -{{ fen2yuan(state.orderInfo.price.couponPrice, false) }}
            </text>
            <text
              class="item-value"
              :class="
            ...
            >
              {{
                state.couponInfo.filter((coupon) => coupon.match).length > 0
                  ? t('couponsAvailable', { count: state.couponInfo.filter((coupon) => coupon.match).length })
                  : t('noCoupon')
              }}
            </text>
        ...
        >
          <view class="item-title">{{ t('activityDiscount') }}</view>
          <view class="ss-flex ss-col-center" @tap="state.showDiscount = true">
            <text class="item-value text-red">
              -{{ fen2yuan(state.orderInfo.price.discountPrice, false) }}
            </text>
            <text class="_icon-forward item-icon" />
          </view>
        </view>
        ...
        >
          <view class="item-title">{{ t('vipDiscount') }}</view>
          <view class="ss-flex ss-col-center">
            <text class="item-value text-red">
              -{{ fen2yuan(state.orderInfo.price.vipPrice, false) }}
            </text>
          </view>
        </view>
      </view>
      <view class="total-box-footer ss-font-28 ss-flex ss-row-right ss-col-center ss-m-r-28">
        <view class="total-num ss-m-r-20">
          {{ t('totalItems', { count: state.orderInfo.items.reduce((acc, item) => acc + item.count, 0) }) }}
        </view>
        <view>{{ t('total') }}</view>
...
<script setup>
  ...
  onReady(() => {
    setPageTitle('pages/order/confirm');
  });
  ...
  onLoad(async (options) => {
    // 解析参数
    if (!options.data) {
      sheep.$helper.toast(t('paramError'));
      return;
    }
