<!-- 售后列表 -->
<template>
  <s-layout :title="t('aftersaleList')">
    <!-- tab -->
    ...
    <s-empty v-if="state.pagination.total === 0" icon="/static/data-empty.png" :text="t('noData')" />
    <!-- 列表 -->
    <view v-if="state.pagination.total > 0">
      ...
      >
        <view class="order-head ss-flex ss-col-center ss-row-between">
          <text class="no">{{ t('serviceNoLabel') }}{{ order.no }}</text>
        ...
        <view class="apply-box ss-flex ss-col-center ss-row-between border-bottom ss-p-x-20">
          <view class="ss-flex ss-col-center">
            <view class="title ss-m-r-20">{{ order.way === 10 ? t('aftersaleRefund') : t('aftersaleReturn') }}</view>
...
<script setup>
  ...
  onReady(() => {
    setPageTitle('pages/order/aftersale/list');
  });
  ...
  const tabMaps = computed(() => [
    ...
    {
      name: t('aftersaleApplying'),
      value: [10],
    },
    {
      name: t('aftersaleProcessing'),
      value: [20, 30, 40],
    },
    {
      name: t('aftersaleCompleted'),
      value: [50],
    },
    {
      name: t('aftersaleRejected'),
      value: [61, 62, 63],
    },
  ]);
