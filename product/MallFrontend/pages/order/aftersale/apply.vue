<!-- 售后申请 -->
<template>
  <s-layout :title="t('aftersaleApply')">
    <!-- 售后商品 -->
    ...
    <uni-forms ref="form" v-model="formData" :rules="rules" label-position="top">
      ...
      <view class="refund-item">
        <view class="item-title ss-m-b-20">{{ t('aftersaleType') }}</view>
      ...
      <view class="refund-item ss-flex ss-col-center ss-row-between" @tap="state.showModal = true">
        <text class="item-title">{{ t('refundAmount') }}</text>
      ...
      <!-- 申请原因 -->
      <view class="refund-item ss-flex ss-col-center ss-row-between" @tap="state.showModal = true">
        <text class="item-title">{{ t('applyReason') }}</text>
        <view class="ss-flex refund-cause ss-col-center">
          <text class="ss-m-r-20" v-if="formData.applyReason">{{ formData.applyReason }}</text>
          <text class="ss-m-r-20" v-else>{{ t('selectReasonPlaceholder') }}</text>
      ...
      <!-- 留言 -->
      <view class="refund-item">
        <view class="item-title ss-m-b-20">{{ t('relatedDescription') }}</view>
    ...
    <su-fixed bottom placeholder>
      <view class="foot-wrap">
        <view class="foot_box ss-flex ss-col-center ss-row-between ss-p-x-30">
          ...
          <button class="ss-reset-button ui-BG-Main-Gradient sub-btn" @tap="submit">{{ t('submit', {}, { scope: 'common' }) }}</button>
        </view>
      </view>
    </su-fixed>
...
<script setup>
  ...
  onReady(() => {
    setPageTitle('pages/order/aftersale/apply');
  });
  ...
  const state = reactive({
    ...
    wayList: computed(() => [
      {
        text: t('aftersaleRefund'),
        value: '10',
      },
      {
        text: t('aftersaleReturn'),
        value: '20',
      },
    ]),
