<template>
  <view class="order-details">
    ...
    <view v-if="orderInfo.deliveryType === 2 && orderInfo.payStatus" class="writeOff borRadius14">
      <view class="title">{{ t('verifyInfo') }}</view>
      ...
      <view class="rules">
        <view class="item">
          <view class="rulesTitle flex flex-wrap align-center"> {{ t('verifyTime') }} </view>
          <view class="info">
            {{ t('daily') }}
        ...
        <view class="item">
          ...
          <view class="info">{{ t('usageInstructionsDesc') }}</view>
        </view>
      </view>
    </view>
    <view
...
<script setup>
  ...
  const showMaoLocation = () => {
    if (!props.systemStore.latitude || !props.systemStore.longitude) {
      sheep.$helper.toast(t('missingLatLng'));
      return;
    }
    uni.openLocation({
