...
<template>
  <view v-if="state.list.length > 0" class="groupon-list detail-card ss-p-x-20">
    <view class="join-activity ss-flex ss-row-between ss-m-t-30">
      <view class="">{{ t('peopleJoined', { count: state.list.length }) }}</view>
      ...
      <view class="ss-flex ss-col-center">
        <view class="ss-flex-col ss-col-bottom ss-m-r-20">
          <view class="title ss-flex ss-m-b-14">
            {{ t('needPeople', { count: record.userSize - record.userCount }) }}
          </view>
          <view class="end-time">{{ endTime(record.expireTime) }}</view>
        </view>
        <view class="">
          <button class="ss-reset-button go-btn" @tap.stop="onJoinGroupon(record)"> {{ t('joinGroup') }} </button>
        </view>
      </view>
    </view>
  </view>
...
<script setup>
  ...
  function endTime(time) {
    ...
    if (durationTime.ms <= 0) {
      return t('groupDismissed');
    }

    return t('timeRemainingFormat', { h: durationTime.h, m: durationTime.m, s: durationTime.s });
  }
