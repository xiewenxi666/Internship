        ...
        <view class="title-card detail-card ss-m-y-14 ss-m-x-20 ss-p-x-20 ss-p-y-34">
          <view class="ss-flex ss-row-between ss-m-b-60">
            <view>
              <view class="price-box ss-flex ss-col-bottom ss-m-b-18">
                ...
                <view class="tig ss-flex ss-col-center">
                  ...
                  <view class="tig-title">{{ t('grouponPrice') }}</view>
                </view>
              </view>
            ...
            <view class="countdown-box" v-if="endTime.ms > 0">
              <view class="countdown-title ss-m-b-20">{{ t('timeRemaining') }}</view>
            ...
            <view class="countdown-title" v-else> {{ t('activityEnded') }} </view>
          </view>
      ...
      <detail-tabbar v-model="state.goodsInfo">
        <view class="buy-box ss-flex ss-col-center ss-p-r-20">
          ...
          >
            ...
            <view v-if="state.activity.startTime > new Date().getTime()">{{ t('activityNotStarted') }}</view>
            <view v-else-if="state.activity.endTime <= new Date().getTime()">{{ t('activityEnded') }}</view>
            <view v-else>
              <view v-if="state.goodsInfo.stock === 0">{{ t('soldOut') }}</view>
              <view v-else>{{ t('startGroupon') }}</view>
            </view>
          </button>
        </view>
      </detail-tabbar>
...
<script setup>
  ...
  onReady(() => {
    setPageTitle('pages/goods/groupon');
  });
