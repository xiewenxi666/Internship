...
<script setup>
  ...
  onReady(() => {
    setPageTitle('pages/goods/comment/list');
  });
  ...
  const typeList = computed(() => [
    { type: 0, name: t('allReview') },
    { type: 1, name: t('goodReview') },
    { type: 2, name: t('mediumReview') },
    { type: 3, name: t('badReview') },
  ]);
  ...
  async function getList() {
    ...
    let res = await CommentApi.getCommentPage(
      ...
      typeList.value[state.currentTab].type,
    );
    if (res.code !== 0) {
      return;
    }
