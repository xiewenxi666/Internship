<!-- 签到界面  -->
<template>
  <s-layout :title="t('signInTitle')">
    <s-empty v-if="state.loading" icon="/static/data-empty.png" :text="t('signInNotStarted')" />
    ...
    <view class="sign-wrap" v-else-if="!state.loading">
      ...
      <view class="content-box calendar">
        <view class="sign-everyday ss-flex ss-col-center ss-row-between ss-p-x-30">
          <text class="sign-everyday-title">{{ t('signInCalendar') }}</text>
          <view class="sign-num-box">
            {{ t('signInContinuous', { days: '' }) }}<text class="sign-num">{{ state.signInfo.continuousDay }}</text> {{ t('common.day', { count: '' }) }}
          </view>
        </view>
        <view
        ...
        <view class="myDateTable">
          <view class="ss-flex ss-col-center ss-row-center sign-box ss-m-y-40">
            ...
            <button class="ss-reset-button already-btn" v-else disabled> {{ t('signInAlready') }} </button>
          </view>
        </view>
      </view>
      ...
      <view class="bg-white ss-m-t-16 ss-p-t-30 ss-p-b-60 ss-p-x-40">
        <view class="activity-title ss-m-b-30">{{ t('signInRule') }}</view>
        <view class="activity-des">1.{{ t('signInTotal', { days: state.signInfo.totalDay }) }}</view>
        <view class="activity-des">
          2.{{ t('signInTip1', { days: state.maxDay }) }}
        </view>
        <view class="activity-des"> 3.{{ t('signInTip2') }}</view>
      </view>
    </view>
      ...
      <view class="model-box ss-flex-col">
        <view class="ss-m-t-56 ss-flex-col ss-col-center">
          ...
          <view class="score-title">
            <text v-if="state.signResult.point">{{ state.signResult.point }} {{ t('signInPoint') }} </text>
            <text v-if="state.signResult.experience"> {{ state.signResult.experience }} {{ t('signInExperience') }}</text>
          </view>
          <view class="model-title ss-flex ss-col-center ss-m-t-22 ss-m-b-30">
            {{ t('signInContinuousResult', { days: state.signResult.day }) }}
          </view>
        </view>
        <view class="model-bg ss-flex-col ss-col-center ss-row-right">
          <view class="title ss-m-b-64">{{ t('signInSuccess') }}</view>
          <view class="ss-m-b-40">
            <button class="ss-reset-button confirm-btn" @tap="onConfirm">{{ t('common.confirm') }}</button>
          </view>
        </view>
      </view>
    </su-popup>
