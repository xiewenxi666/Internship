    ...
    <view class="list-content">
      ...
      <view class="content-header ss-flex-col ss-col-center ss-row-center">
        <view class="content-header-title ss-flex ss-row-center">
          ...
          <text class="pic_count">{{ t('grouponParticipants', { count: state.summaryData.userCount || 0 }) }}</text>
        </view>
      </view>
      <scroll-view
        class="scroll-box"
        ...
        <view class="goods-box ss-m-b-20" v-for="item in state.pagination.list" :key="item.id">
          ...
          >
            <template v-slot:cart>
              <button class="ss-reset-button cart-btn">{{ t('grouponGoJoin') }}</button>
            </template>
          </s-goods-column>
        </view>
        <uni-load-more
...
<script setup>
  ...
  onReady(() => {
    setPageTitle('pages/activity/groupon/list');
  });
