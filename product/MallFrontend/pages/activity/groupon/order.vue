    ...
    <view v-if="state.pagination.total > 0">
      ...
      >
        <view class="order-card-header ss-flex ss-col-center ss-row-between ss-p-x-20">
          <view class="order-no">{{ t('grouponOrderId') }}{{ record.id }}</view>
        ...
        <view class="border-bottom">
          ...
          >
            <template #groupon>
              <view class="ss-flex">
                <view class="sales-title"> {{ t('grouponPeople', { count: record.userSize }) }} </view>
              </view>
            </template>
          </s-goods-item>
        </view>
        <view class="order-card-footer ss-flex ss-row-right ss-p-x-20">
          ...
          >
            {{ record.status === 0 ? t('grouponInvite') : t('grouponViewDetail') }}
          </button>
        </view>
      </view>
    </view>
...
<script setup>
  ...
  import { onLoad, onReachBottom, onPullDownRefresh, onReady } from '@dcloudio/uni-app';
  ...
  onReady(() => {
    setPageTitle('pages/activity/groupon/order');
  });
  ...
  const tabMaps = [
    ...
    {
      name: t('grouponSuccess'),
      value: 1,
    },
    {
      name: t('grouponFailed'),
      value: 2,
    },
  ];
