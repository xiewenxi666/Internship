<!-- 充值记录 -->
<template>
  <s-layout class="widthdraw-log-wrap" :title="t('rechargeLog')">
    <!-- 记录卡片 -->
    <view class="wallet-log-box ss-p-b-30">
      <view class="log-list" v-for="item in state.pagination.list" :key="item">
        <view class="head ss-flex ss-col-center ss-row-between">
          <view class="title">{{ t('rechargeAmountLabel') }}</view>
          <view class="num" :class="item.refundStatus === 10 ? 'danger-color' : 'success-color'">
            {{ fen2yuan(item.payPrice) }} {{ t('yuan', {}, { namespace: 'common' }) }}
            <text v-if="item.bonusPrice > 0">（{{ t('bonusGiftLabel') }} {{ fen2yuan(item.bonusPrice) }} {{ t('yuan', {}, { namespace: 'common' }) }}）</text>
          </view>
        </view>
        <view class="status-box item ss-flex ss-col-center ss-row-between">
          <view class="item-title">{{ t('payStatus') }}</view>
          <view
            class="status-text"
            :class="item.refundStatus === 10 ? 'danger-color' : 'success-color'"
          >
            {{ item.refundStatus === 10 ? t('refunded') : t('paid') }}
          </view>
        </view>
        <view class="time-box item ss-flex ss-col-center ss-row-between">
          <text class="item-title">{{ t('rechargeChannel') }}</text>
          <view class="time ss-ellipsis-1">{{ item.payChannelName }}</view>
        </view>
        <view class="time-box item ss-flex ss-col-center ss-row-between">
          <text class="item-title">{{ t('rechargeOrderNo') }}</text>
          <view class="time"> {{ item.payOrderChannelOrderNo }} </view>
        </view>
        <view class="time-box item ss-flex ss-col-center ss-row-between">
          <text class="item-title">{{ t('rechargeTime') }}</text>
          <view class="time">
