<!-- 充值界面 -->
<template>
  <s-layout :title="t('recharge')" class="withdraw-wrap" navbar="inner">
    <view
    ...
    >
      <view class="">
        <view class="num-title">{{ t('currentBalance') }}</view>
    ...
    <view class="recharge-box">
      <view class="recharge-card-box">
        <view class="input-label ss-m-b-50">{{ t('rechargeAmount') }}</view>
        ...
        <view class="face-value-box ss-flex ss-flex-wrap ss-m-y-40">
          ...
          >
            <text class="face-value-title">{{ fen2yuan(item.payPrice) }}{{ t('yuan', {}, { namespace: 'common' }) }}</text>
            <view v-if="item.bonusPrice" class="face-value-tag">
              {{ t('bonusGift') }} {{ fen2yuan(item.bonusPrice) }} {{ t('yuan', {}, { namespace: 'common' }) }}
            </view>
          </button>
        </view>
        <button
