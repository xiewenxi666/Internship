<!-- 支付结果页面 -->
<template>
  <s-layout :bgStyle="{ color: '#FFF' }" :title="t('payResult')">
    <view class="pay-result-box ss-flex-col ss-row-center ss-col-center">
      ...
      />
      <view class="tip-text ss-m-b-30" v-if="payResult === 'success'">{{ t('paySuccess') }}</view>
      <view class="tip-text ss-m-b-30" v-if="payResult === 'failed'">{{ t('payFailed') }}</view>
      <view class="tip-text ss-m-b-30" v-if="payResult === 'closed'">{{ t('orderClosed') }}</view>
      <view class="tip-text ss-m-b-30" v-if="payResult === 'waiting'">{{ t('queryingResult') }}</view>
      ...
      >
        ...
        <view class="subscribe-title ss-m-r-48 ss-m-l-16">{{ t('subscribeTitle') }}</view>
        <view class="subscribe-start" @tap="subscribeMessage">{{ t('subscribeNow') }}</view>
      </view>
      <!-- #endif -->
    </view>
  </s-layout>
...
<script setup>
  ...
  function showRepayModal() {
    ...
    uni.showModal({
      title: t('repayConfirmTitle'),
      content: t('repayConfirmContent'),
      cancelText: t('cancel', {}, { namespace: 'common' }),
      confirmText: t('repayConfirmBtn'),
