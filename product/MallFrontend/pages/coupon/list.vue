<!-- 优惠券中心  -->
<template>
  <s-layout :bgStyle="{ color: '#f2f2f2' }" :title="t('title')">
      ...
      <view v-for="item in state.pagination.list" :key="item.id">
        ...
        >
          <template #default>
            ...
            >
              {{ item.canTake ? t('receiveNow') : t('received') }}
            </button>
          </template>
        </s-coupon-list>
      </view>
      ...
      <view v-for="item in state.pagination.list" :key="item.id">
        ...
        >
          <template #default>
            ...
            >
              {{ item.status === 1 ? t('useNow') : item.status === 2 ? t('used') : t('expired') }}
            </button>
          </template>
        </s-coupon-list>
      </view>
...
<script setup>
  ...
  onLoad((Option) => {
    setPageTitle('pages/coupon/list');
    // 领劵中心
