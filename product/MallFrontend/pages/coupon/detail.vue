    ...
    <view class="bg-white">
        ...
        <view class="detail-box">
          ...
          <view class="top ss-flex-col ss-col-center">
            ...
            <view class="subtitle ss-m-b-50">
              {{
                state.coupon.discountType === 1
                  ? t('fullReduceFormat', { amount: fen2yuan(state.coupon.usePrice), discount: fen2yuan(state.coupon.discountPrice) })
                  : t('percentFormat', { amount: fen2yuan(state.coupon.usePrice), percent: 100 - state.coupon.discountPercent })
              }}
            </view>
            <button
              ...
              @click="getCoupon"
            >
              <text v-if="state.id > 0">{{ state.coupon.canTake ? t('receiveNow') : t('received') }}</text>
              <text v-else>
                {{
            ...
            <view class="time ss-m-y-30" v-if="state.coupon.validityType === 2">
              {{ t('validDaysTip', { days: state.coupon.fixedEndTerm }) }}
            </view>
            <view class="time ss-m-y-30" v-else>
              {{ t('validDateTip', { start: sheep.$helper.timeFormat(state.coupon.validStartTime, 'yyyy-mm-dd'), end: sheep.$helper.timeFormat(state.coupon.validEndTime, 'yyyy-mm-dd') }) }}
            </view>
            <view class="coupon-line ss-m-t-14"></view>
          </view>
          <view class="bottom">
            <view class="type ss-flex ss-col-center ss-row-between ss-p-x-30">
              <view>{{ t('couponType') }}</view>
              <view>{{ state.coupon.discountType === 1 ? t('fullReduce') : t('discount') }}</view>
            </view>
            <uni-collapse>
              <uni-collapse-item :title="t('couponDescription')" v-if="state.coupon.description">
      ...
      <su-sticky v-else bgColor="#fff">
        <view class="goods-title ss-p-20">
          {{ state.coupon.productScope === 2 ? t('scopeGoodsAvailable') : t('scopeCategory') }}
        </view>
        <su-tabs
          :scrollable="true"
          :list="state.tabMaps"
...
<script setup>
  ...
  onLoad((options) => {
    setPageTitle('pages/coupon/detail');
    state.id = options.id;
