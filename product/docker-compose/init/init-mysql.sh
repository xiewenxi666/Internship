...
if [ -d "$SQL_NEW_DIR" ]; then
    ...
    # 4.6 执行 new-receivable-plan-status.sql
    if [ -f "${SQL_NEW_DIR}/new-receivable-plan-status.sql" ]; then
        echo "Executing: new-receivable-plan-status.sql"
        mysql -uroot -p"${MYSQL_ROOT_PASSWORD}" "${MYSQL_DATABASE}" < "${SQL_NEW_DIR}/new-receivable-plan-status.sql"
    fi
fi
