<!-- 执行器选择 -->
<template>
  <Dialog :title="t('designer.pleaseSelectListener')" v-model="dialogVisible" width="1024px">
    <ContentWrap>
      <el-table v-loading="loading" :data="list" :stripe="true" :show-overflow-tooltip="true" :table-layout="'auto'">
        ...
        <el-table-column :label="t('common.type')" align="center" prop="type">
          <template #default="scope">
            <dict-tag :type="DICT_TYPE.BPM_PROCESS_LISTENER_TYPE" :value="scope.row.type" />
          </template>
        </el-table-column>
        <el-table-column :label="t('designer.eventType')" align="center" prop="event" />
        <el-table-column :label="t('designer.fieldType')" align="center" prop="valueType">
          <template #default="scope">
            <dict-tag
        ...
        <el-table-column :label="t('designer.fieldValue')" align="center" prop="value" />
        <el-table-column :label="t('common.action')" align="center">
          <template #default="scope">
            <el-button link type="primary" @click="select(scope.row)"> {{ t('common.select') }} </el-button>
          </template>
        </el-table-column>
      </el-table>
      <!-- 分页 -->
