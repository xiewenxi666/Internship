<template>
  <div class="panel-tab__content">
    ...
    <div style="margin-top: 10px; display: flex; align-items: center">
      <span>{{ t('designer.timerCondition') }}：</span>
      <el-input
        v-model="condition"
      ...
      >
        <template #suffix>
          <el-tooltip v-if="!valid" :content="t('designer.formatError')" placement="top">
    ...
    >
      <el-date-picker
        v-model="dateValue"
        type="datetime"
        :placeholder="t('designer.selectDateTime')"
        style="width: 100%"
        @change="onDateChange"
      />
      <template #footer>
        <el-button @click="showDatePicker = false">{{ t('common.cancel') }}</el-button>
        <el-button type="primary" @click="onDateConfirm">{{ t('common.confirm') }}</el-button>
      </template>
    </el-dialog>
    <!-- 持续时长选择器 -->
    <el-dialog
      v-model="showDurationDialog"
      :title="t('designer.timeConfig')"
      width="600px"
      @close="showDurationDialog = false"
    >
      <DurationConfig :value="condition" @change="onDurationChange" />
      <template #footer>
        <el-button @click="showDurationDialog = false">{{ t('common.cancel') }}</el-button>
        <el-button type="primary" @click="onDurationConfirm">{{ t('common.confirm') }}</el-button>
      </template>
    </el-dialog>
    <!-- 循环配置器 -->
    <el-dialog
    ...
    >
      ...
      <template #footer>
        <el-button @click="showCycleDialog = false">{{ t('common.cancel') }}</el-button>
        <el-button type="primary" @click="onCycleConfirm">{{ t('common.confirm') }}</el-button>
      </template>
    </el-dialog>
    <!-- 帮助说明 -->
    <el-dialog v-model="showHelp" :title="t('designer.formatDescription')" width="600px" @close="showHelp = false">
      <div v-html="helpHtml"></div>
      <template #footer>
        <el-button @click="showHelp = false">{{ t('common.close') }}</el-button>
      </template>
    </el-dialog>
  </div>
</template>
...
const placeholder = computed(() => {
  if (type.value === 'time') return t('designer.timePlaceholder')
  if (type.value === 'duration') return t('designer.durationPlaceholder')
  if (type.value === 'cycle') return t('designer.cyclePlaceholder')
  return ''
})
