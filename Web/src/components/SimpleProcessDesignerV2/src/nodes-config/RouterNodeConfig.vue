  ...
  >
    ...
    <div>
        ...
        <el-card class="mb-15px" v-for="(item, index) in routerGroups" :key="index">
          <template #header>
            <div class="flex flex-items-center">
              <el-text size="large">{{ t('simpleDesigner.routerLabel') }}{{ index + 1 }}</el-text>
      ...
      <el-button class="w-1/1" type="primary" :icon="Plus" @click="addRouterGroup">
        {{ t('simpleDesigner.addRouterBranch') }}
      </el-button>
    </div>
    <template #footer>
      <el-divider />
...
const getShowText = () => {
  if (!routerGroups.value || !Array.isArray(routerGroups.value) || routerGroups.value.length <= 0) {
    message.warning(t('simpleDesigner.pleaseConfigRoute'))
    return ''
  }
  for (const route of routerGroups.value) {
    if (!route.nodeId || !route.conditionType) {
      message.warning(t('simpleDesigner.pleaseCompleteRouteConfig'))
      return ''
    }
  ...
  return `${routerGroups.value.length}${t('simpleDesigner.routeBranchCount')}`
}
