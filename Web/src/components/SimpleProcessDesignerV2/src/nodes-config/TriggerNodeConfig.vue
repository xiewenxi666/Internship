  ...
  >
    ...
    <div>
      <el-form ref="formRef" :model="configForm" label-position="top" :rules="formRules">
        <el-form-item :label="t('simpleDesigner.triggerTypeLabel')" prop="type">
          <el-select v-model="configForm.type" @change="changeTriggerType">
            <el-option
              v-for="(item, index) in TRIGGER_TYPES"
              :key="index"
              :value="item.value"
              :label="t(`simpleDesigner.triggerTypes.${item.value === TriggerTypeEnum.HTTP_REQUEST ? 'sendHttpRequest' : item.value === TriggerTypeEnum.HTTP_CALLBACK ? 'receiveHttpCallback' : item.value === TriggerTypeEnum.FORM_UPDATE ? 'modifyFormData' : 'deleteFormData'}`)"
            />
          </el-select>
        </el-form-item>
        <!-- HTTP 请求触发器 -->
        ...
        <div v-if="configForm.type === TriggerTypeEnum.FORM_UPDATE">
            ...
            <el-card class="w-580px mt-4">
              <template #header>
                <div class="flex items-center justify-between">
                  <div>{{ t('simpleDesigner.modifyFormSetting') }} {{ index + 1 }}</div>
                  <el-button
                    type="primary"
                    plain
                    circle
              ...
              >
                <Icon icon="ep:link" class="mr-5px" />{{ t('simpleDesigner.addCondition') }}
              </el-button>
              <el-divider content-position="left">{{ t('simpleDesigner.modifyFormFieldSetting') }}</el-divider>
              <!-- 表单字段修改设置 -->
              <div
              ...
              >
                <div class="mr-2 flex items-center">
                  <el-form-item>
                    <el-select
                      ...
                      :placeholder="t('simpleDesigner.pleaseSelectFormField')"
                      :disabled="key !== ''"
                    >
                      <el-option
                ...
                <div class="mx-2"><el-form-item>{{ t('simpleDesigner.valueSetTo') }}</el-form-item></div>
                <div class="mr-2">
                  <el-form-item
                    :prop="`formSettings.${index}.updateFormFields.${key}`"
                    :rules="{
                      required: true,
                      message: t('simpleDesigner.variableNotEmpty'),
                      trigger: 'blur'
                    }"
                  >
                    <el-input
                      ...
                      :placeholder="t('simpleDesigner.pleaseInputValue')"
                      :disabled="!key"
                    />
                  </el-form-item>
                </div>
              ...
              <el-button type="primary" text @click="addFormFieldSetting(formSetting)">
                <Icon icon="ep:memo" class="mr-5px" />{{ t('simpleDesigner.addModifyField') }}
              </el-button>
            </el-card>
          </div>
          ...
          <el-button class="mt-6" type="primary" text @click="addFormSetting">
            <Icon icon="ep:setting" class="mr-5px" />{{ t('simpleDesigner.addSetting') }}
          </el-button>
        </div>
        ...
        <div v-if="configForm.type === TriggerTypeEnum.FORM_DELETE">
          <div v-for="(formSetting, index) in configForm.formSettings" :key="index">
            <el-card class="w-580px mt-4">
              <template #header>
                <div class="flex items-center justify-between">
                  <div>{{ t('simpleDesigner.deleteFormSetting') }} {{ index + 1 }}</div>
                  <el-button
                    type="primary"
                    plain
                    circle
              ...
              <el-divider content-position="left">{{ t('simpleDesigner.deleteFormFieldSetting') }}</el-divider>
              <!-- 表单字段删除设置 -->
              <div class="flex flex-wrap gap-2">
                <el-select
                  v-model="formSetting.deleteFields"
                  multiple
                  :placeholder="t('simpleDesigner.pleaseSelectDeleteFields')"
                  class="w-full"
                >
                  <el-option
    ...
    <template #footer>
      ...
      <div>
        ...
        <el-button @click="cancelConfig">{{ t('simpleDesigner.cancel') }}</el-button>
      </div>
    </template>
  </el-drawer>
</template>
...
const formRules = reactive({
  type: [{ required: true, message: t('simpleDesigner.triggerTypeLabel'), trigger: 'change' }],
  'httpRequestSetting.url': [{ required: true, message: t('simpleDesigner.triggerTypeLabel'), trigger: 'blur' }]
})
// 触发器配置表单数据
  ...
  if (
    ...
    for (const [index, setting] of configForm.value.formSettings!.entries()) {
      if (!setting.updateFormFields || Object.keys(setting.updateFormFields).length === 0) {
        message.warning(t('simpleDesigner.pleaseAddFormField', { index: index + 1 }))
        return ''
      }
    }
    showText = t('simpleDesigner.triggerTypes.modifyFormData')
  } else if (configForm.value.type === TriggerTypeEnum.FORM_DELETE) {
    for (const [index, setting] of configForm.value.formSettings!.entries()) {
      if (!setting.deleteFields || setting.deleteFields.length === 0) {
        message.warning(t('simpleDesigner.pleaseSelectDeleteField', { index: index + 1 }))
        return ''
      }
    }
    showText = t('simpleDesigner.triggerTypes.deleteFormData')
  }
  return showText
}
