  ...
  >
    <div class="w-full">
      ...
      <el-form label-width="80px">
        ...
        <!-- 第二行：坐标显示 -->
        <el-form-item :label="t('common.currentCoordinate')">
          <div class="flex items-center gap-4">
            <span>{{ t('common.longitude') }}: {{ state.longitude || '-' }}</span>
            <span>{{ t('common.latitude') }}: {{ state.latitude || '-' }}</span>
          </div>
        </el-form-item>
      </el-form>
      <!-- 第三行：地图 -->
      ...
      <div v-else class="w-full h-[400px] mt-[10px] flex items-center justify-center">
        <span class="text-gray-400">{{ t('common.mapLoading') }}</span>
      </div>
    </div>
    <template #footer>
      <el-button @click="handleConfirm" type="primary">{{ t('common.ok') }}</el-button>
