<template>
  <el-aside class="kefu pt-5px h-100%" width="260px">
    <div class="color-[#999] font-bold my-10px">
      {{ t('conversationList') }}({{ kefuStore.getConversationList.length }})
    </div>
    <div
        ...
        <div class="ml-10px w-100%">
          <div class="flex justify-between items-center w-100%">
            ...
            <span class="color-[#999]" style="font-size: 13px">
              {{ lastMessageTimeMap.get(item.id) ?? t('calculating') }}
            </span>
          </div>
          <!-- 最后聊天内容 -->
          <div
...
const updateConversationPinned = async (adminPinned: boolean) => {
  ...
  message.notifySuccess(adminPinned ? t('pinSuccess') : t('unpinSuccess'))
  // 2. 关闭右键菜单，更新会话列表
  closeRightMenu()
...
const deleteConversation = async () => {
  // 1. 删除会话
  await message.confirm(t('deleteConfirm'))
