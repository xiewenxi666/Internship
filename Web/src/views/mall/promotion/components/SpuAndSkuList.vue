<template>
  <el-table :data="spuData" :expand-row-keys="expandRowKeys" row-key="id" :table-layout="'auto'">
    ...
    <el-table-column key="id" align="center" :label="t('product.spu.id')" prop="id" />
    <el-table-column :label="t('product.spu.picUrl')" min-width="80">
      <template #default="{ row }">
        <el-image :src="row.picUrl" class="h-30px w-30px" @click="imagePreview(row.picUrl)" />
      </template>
    </el-table-column>
    <el-table-column :show-overflow-tooltip="true" :label="t('product.spu.name')" min-width="300" prop="name" />
    <el-table-column align="center" :label="t('product.spu.price')" min-width="90" prop="price">
      <template #default="{ row }">
        {{ formatToFraction(row.price) }}
      </template>
    </el-table-column>
    <el-table-column align="center" :label="t('product.spu.salesCount')" min-width="90" prop="salesCount"  fixed="right" />
    <el-table-column align="center" :label="t('product.spu.stock')" min-width="90" prop="stock" />
    <el-table-column
    ...
    >
      <template #default="scope">
        <el-button link type="primary" @click="deleteSpu(scope.row.id)"> {{ t('common.delete') }}</el-button>
      </template>
    </el-table-column>
  </el-table>
</template>
...
const spuPropertyList = ref<SpuProperty<T>[]>([]) // spuId 对应sku 的属性列
const expandRowKeys = ref<string[]>([]) // 控制展开行需要设row-key 属性才能使用，该属性为展开行的 keys 数组
/**
 * 获取所sku 活动配置
 *
 * @param extendedAttribute sku 上扩展的属性，例：秒杀活动 sku 扩展属productConfig 请参seckillActivity.ts
 */
...
const deleteSpu = async (spuId: number) => {
  await message.confirm(t('promotion.deleteSpuConfirm', { id: spuId }))
...
watch(
  ...
  (data) => {
    ...
    // 解决如果之前选择的是单规spu 的话后面选择多规sku 多规格属性信息不展示的问题。解决方法：SkuList 组件重新渲染（行折叠会干掉包含的组件展开时会重新加载
    setTimeout(() => {
