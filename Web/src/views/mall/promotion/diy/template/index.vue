<template>
  <doc-alert title="【营销】商城装" url="https://doc.iocoder.cn/mall/diy/" />
  ...
  <ContentWrap>
    <el-table v-loading="loading" :data="list" :stripe="true" :show-overflow-tooltip="true" :table-layout="'auto'">
      ...
      <el-table-column :label="t('template.previewImage')" align="center" prop="previewPicUrls">
      ...
      <el-table-column :label="t('template.name')" align="center" prop="name" min-width="180" />
      <el-table-column :label="t('template.isUsed')" align="center" prop="used">
        <template #default="scope">
          <dict-tag :type="DICT_TYPE.INFRA_BOOLEAN_STRING" :value="scope.row.used" />
        </template>
      </el-table-column>
      <el-table-column :label="t('template.remark')" align="center" prop="remark" min-width="180"  fixed="right" />
      <el-table-column
...
const { t } = useI18n('mall.promotion.diy') // 国际
...
const handleUse = async (row: DiyTemplateApi.DiyTemplateVO) => {
  try {
    // 使用模板的二次确
    await message.confirm(t('template.useConfirm', { name: row.name }))
    // 发起删除
