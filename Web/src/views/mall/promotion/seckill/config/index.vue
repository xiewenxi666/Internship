<template>
        ...
        <el-col :span="8">
          <el-form-item :label="t('mall.promotion.seckill.configName')" prop="name">
            <el-input
              ...
              :placeholder="t('mall.promotion.seckill.configNamePlaceholder')"
              clearable
  ...
  <ContentWrap>
    <el-table v-loading="loading" :data="list" :stripe="true" :show-overflow-tooltip="true" :table-layout="'auto'">
      <el-table-column :label="t('mall.promotion.seckill.configName')" align="center" prop="name" />
      <el-table-column :label="t('mall.promotion.seckill.configStartTime')" align="center" prop="startTime" />
      <el-table-column :label="t('mall.promotion.seckill.configEndTime')" align="center" prop="endTime" />
      <el-table-column :label="t('mall.promotion.seckill.sliderPicUrls')" align="center" prop="sliderPicUrls">
      ...
      <el-table-column :label="t('mall.promotion.seckill.status')" align="center" prop="status">
        <template #default="scope">
          <el-switch
...
const handleStatusChange = async (row: SeckillConfigVO) => {
  try {
    ...
    await message.confirm(row.status === CommonStatusEnum.ENABLE ? t('mall.promotion.seckill.configEnableConfirm') : t('mall.promotion.seckill.configDisableConfirm'))
    // 发起修改状�?    await SeckillConfigApi.updateSeckillConfigStatus(row.id, row.status)
    // 刷新列表
    await getList()
  } catch {
    // 取消后，进行恢复按钮
