<template>
  <Dialog v-model="dialogVisible" :appendToBody="true" :title="t('select.selectProduct')" width="70%">
    <ContentWrap>
      <el-form
        ref="queryFormRef"
        :model="queryParams"
      ...
      <el-table v-loading="loading" :data="list" show-overflow-tooltip :table-layout="'auto'">
        ...
        <el-table-column key="id" align="center" :label="t('select.productId')" prop="id" min-width="60" />
        <el-table-column :label="t('select.productImage')" min-width="80">
          <template #default="{ row }">
            <el-image
              :src="row.picUrl"
              class="h-30px w-30px"
        ...
        <el-table-column :label="t('select.productName')" min-width="200" prop="name" />
        <el-table-column :label="t('select.productCategory')" min-width="100" prop="categoryId">
          <template #default="{ row }">
...
const getList = async () => {
  ...
  try {
    ...
    // checkbox绑定undefined会有问题，需要给一个boolean值
    list.value.forEach(
