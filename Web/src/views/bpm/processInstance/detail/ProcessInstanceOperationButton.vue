      ...
      <div class="flex flex-col flex-1 pt-20px px-20px" v-loading="formLoading">
        ...
        >
          <el-card v-if="runningTask?.formId > 0" class="mb-15px !-mt-10px">
            <template #header>
              <span class="el-icon-picture-outline"> {{ t('approval.fillForm') }}【{{ runningTask?.formName }}】 </span>
            </template>
            <form-create
          ...
          <el-form-item :label="t('approval.opinionLabel', { type: nodeTypeName })" prop="reason">
            <el-input
              v-model="approveReasonForm.reason"
              :placeholder="t('approval.opinionPlaceholder', { type: nodeTypeName })"
              type="textarea"
              :rows="4"
            />
          </el-form-item>
          <el-form-item
            :label="t('approval.nextNodeApprover')"
            prop="nextAssignees"
          ...
          <el-form-item
            ...
            :label="t('approval.sign')"
            prop="signPicUrl"
            ref="approveSignFormRef"
          >
            <el-button @click="signRef.open()">{{ t('approval.clickSign') }}</el-button>
            <el-image
    ...
    >
      ...
      <div class="flex flex-col flex-1 pt-20px px-20px" v-loading="formLoading">
        <el-form
          ...
          label-width="100px"
        >
          <el-form-item :label="t('approval.approvalOpinion')" prop="reason">
            <el-input
              v-model="rejectReasonForm.reason"
              :placeholder="t('approval.approvalOpinionPlaceholder')"
              type="textarea"
              :rows="4"
            />
          </el-form-item>
    ...
    >
      ...
      <div class="flex flex-col flex-1 pt-20px px-20px" v-loading="formLoading">
        <el-form
          ...
          label-width="100px"
        >
          <el-form-item :label="t('approval.copyUser')" prop="copyUserIds">
            <el-select
          ...
          <el-form-item :label="t('approval.copyReason')" prop="copyReason">
            <el-input
              v-model="copyForm.copyReason"
              clearable
              :placeholder="t('approval.copyReasonPlaceholder')"
              type="textarea"
              :rows="3"
            />
          </el-form-item>
    ...
    >
      ...
      <div class="flex flex-col flex-1 pt-20px px-20px" v-loading="formLoading">
        <el-form
          ...
          label-width="100px"
        >
          <el-form-item :label="t('approval.newAssignee')" prop="assigneeUserId">
    ...
    >
      ...
      <div class="flex flex-col flex-1 pt-20px px-20px" v-loading="formLoading">
        <el-form
          ...
          label-width="100px"
        >
          <el-form-item :label="t('approval.receiver')" prop="delegateUserId">
    ...
    >
      ...
      <div class="flex flex-col flex-1 pt-20px px-20px" v-loading="formLoading">
        <el-form
          ...
          label-width="100px"
        >
          <el-form-item :label="t('approval.addSignHandler')" prop="addSignUserIds">
          ...
          <el-form-item>
            <el-button :disabled="formLoading" type="primary" @click="handlerAddSign('before')">
              {{ t('approval.addSignBefore') }}{{ getButtonDisplayName(OperationButtonType.ADD_SIGN) }}
            </el-button>
            <el-button :disabled="formLoading" type="primary" @click="handlerAddSign('after')">
              {{ t('approval.addSignAfter') }}{{ getButtonDisplayName(OperationButtonType.ADD_SIGN) }}
            </el-button>
    ...
    >
      <template #reference>
        <div @click="openPopover('deleteSign')" class="hover-bg-gray-100 rounded-xl p-6px">
          <Icon :size="14" icon="ep:semi-select" />&nbsp; {{ t('approval.deleteSign') }}
        </div>
      </template>
      <div class="flex flex-col flex-1 pt-20px px-20px" v-loading="formLoading">
        <el-form
          ...
          label-width="100px"
        >
          <el-form-item :label="t('approval.deleteSignUser')" prop="deleteSignTaskId">
    ...
    >
      ...
      <div class="flex flex-col flex-1 pt-20px px-20px" v-loading="formLoading">
        <el-form
          ...
          label-width="100px"
        >
          <el-form-item :label="t('approval.returnNode')" prop="targetTaskDefinitionKey">
          ...
          <el-form-item :label="t('approval.returnReason')" prop="returnReason">
            <el-input
              v-model="returnForm.returnReason"
              clearable
              :placeholder="t('approval.returnReasonPlaceholder')"
              type="textarea"
              :rows="3"
            />
          </el-form-item>
    ...
    >
      <template #reference>
        <div @click="openPopover('cancel')" class="hover-bg-gray-100 rounded-xl p-6px">
          <Icon :size="14" icon="fa:mail-reply" />&nbsp; {{ t('approval.cancel') }}
        </div>
      </template>
      <div class="flex flex-col flex-1 pt-20px px-20px" v-loading="formLoading">
        <el-form
          ...
          label-width="100px"
        >
          <el-form-item :label="t('approval.cancelReason')" prop="cancelReason">
            <span class="text-#878c93 text-12px">&nbsp; {{ t('approval.cancelTip') }}</span>
            <el-input
...
const approveReasonRule = computed(() => {
  return {
    reason: [
      { required: reasonRequire.value, message: t('approval.opinionRequired'), trigger: 'blur' }
    ],
    signPicUrl: [{ required: true, message: t('approval.signRequired'), trigger: 'change' }],
    nextAssignees: [{ required: true, message: t('approval.nextNodeApproverRequired'), trigger: 'blur' }]
  }
})
...
const rejectReasonRule = computed(() => {
  return {
    reason: [{ required: reasonRequire.value, message: t('approval.opinionRequired'), trigger: 'blur' }]
  }
})
...
const copyFormRule = reactive<FormRules<typeof copyForm>>({
  copyUserIds: [{ required: true, message: t('approval.copyUserRequired'), trigger: 'change' }]
})
...
const transferFormRule = reactive<FormRules<typeof transferForm>>({
  assigneeUserId: [{ required: true, message: t('approval.newAssigneeRequired'), trigger: 'change' }],
  reason: [{ required: true, message: t('approval.opinionRequired'), trigger: 'blur' }]
})
...
const delegateFormRule = reactive<FormRules<typeof delegateForm>>({
  delegateUserId: [{ required: true, message: t('approval.receiverRequired'), trigger: 'change' }],
  reason: [{ required: true, message: t('approval.opinionRequired'), trigger: 'blur' }]
})
...
const addSignFormRule = reactive<FormRules<typeof addSignForm>>({
  addSignUserIds: [{ required: true, message: t('approval.addSignHandlerRequired'), trigger: 'change' }],
  reason: [{ required: true, message: t('approval.opinionRequired'), trigger: 'blur' }]
})
...
const deleteSignFormRule = reactive<FormRules<typeof deleteSignForm>>({
  deleteSignTaskId: [{ required: true, message: t('approval.deleteSignUserRequired'), trigger: 'change' }],
  reason: [{ required: true, message: t('approval.opinionRequired'), trigger: 'blur' }]
})
...
const returnFormRule = reactive<FormRules<typeof returnForm>>({
  targetTaskDefinitionKey: [{ required: true, message: t('approval.returnNodeRequired'), trigger: 'change' }],
  returnReason: [{ required: true, message: t('approval.returnReasonRequired'), trigger: 'blur' }]
})
...
const cancelFormRule = reactive<FormRules<typeof cancelForm>>({
  cancelReason: [{ required: true, message: t('approval.cancelReasonRequired'), trigger: 'blur' }]
})
...
const openPopover = async (type: string) => {
  ...
  if (type === 'approve') {
    ...
    if (!valid) {
      message.warning(t('approval.formValidationFailed'))
      return
    }
  ...
  if (type === 'return') {
    ...
    if (returnList.value.length === 0) {
      message.warning(t('approval.noReturnNode'))
      return
    }
  }
...
const validateNextAssignees = () => {
  ...
  for (const item of nextAssigneesActivityNode.value) {
    if (isEmpty(approveReasonForm.nextAssignees[item.id])) {
      message.warning(t('approval.nextNodeApproverNotEmpty'))
      return false
    }
  }
  return true
...
const handleCopy = async () => {
  ...
  try {
    ...
    message.success(t('approval.operationSuccess'))
  } finally {
    formLoading.value = false
  }
}
...
const getDeleteSignUserLabel = (task: any): string => {
  ...
  return `${nickname} ( ${t('approval.deptLabel')}${deptName} )`
}
/** 处理减签 */
const handlerDeleteSign = async () => {
  ...
  try {
    ...
    message.success(t('approval.deleteSignSuccess'))
