    ...
    <div class="mx-auto">
      ...
      >
        ...
        <div class="w-200px flex items-center overflow-hidden">
          ...
          <span class="ml-10px text-16px truncate" :title="formData.name || t('process.model.createProcess')">
            {{ formData.name || t('process.model.createProcess') }}
          </span>
        </div>
        ...
        <div class="w-200px flex items-center justify-end gap-2">
          <el-button v-if="actionType === 'update'" type="success" @click="handleDeploy">
            {{ t('process.model.publish') }}
          </el-button>
          <el-button type="primary" @click="handleSave">
            <span v-if="actionType === 'definition'">{{ t('process.model.recover') }}</span>
            <span v-else>{{ t('process.model.save') }}</span>
          </el-button>
        </div>
      </div>
...
const steps = computed(() => [
  { title: t('process.model.basicInfo'), validator: validateBasic },
  { title: t('process.model.formDesign'), validator: validateForm },
  { title: t('process.model.processDesign'), validator: validateProcess },
  { title: t('process.model.moreSettings'), validator: null }
])
    ...
    if (route.params.type === 'copy') {
      ...
      if (formData.value.bpmnXml) {
        formData.value.bpmnXml = formData.value.bpmnXml.replaceAll(
          formData.value.name,
          formData.value.name + t('process.model.copyTitle').replace('复制流程', '')
        )
        formData.value.bpmnXml = formData.value.bpmnXml.replaceAll(
          formData.value.key,
      ...
      formData.value.name += t('process.model.copyTitle').replace('复制流程', '')
      formData.value.key += '_copy'
      tagsView.setTitle(t('process.model.copyTitle'))
    }
  } else {
    // 情况三：新增场景
...
const validateAllSteps = async () => {
  try {
    ...
    try {
      ...
      currentStep.value = 0
      throw new Error(t('process.model.completeBasicInfo'))
    }
    ...
    try {
      ...
      currentStep.value = 1
      throw new Error(t('process.model.completeFormInfo'))
    }
    ...
    try {
      ...
      currentStep.value = 2
      throw new Error(t('process.model.completeProcessDesign'))
    }
...
const handleSave = async () => {
  try {
    ...
    if (actionType === 'definition') {
      ...
      // 提示成功
      message.success(t('process.model.recover'))
    } else if (actionType === 'update') {
      // 修改场景
      await ModelApi.updateModel(modelData)
      // 提示成功
      message.success(t('process.model.updateSuccess'))
    } else if (actionType === 'copy') {
      // 情况三：复制场景
      formData.value.id = await ModelApi.createModel(modelData)
      // 提示成功
      message.success(t('process.model.copySuccess'))
    } else {
      // 情况四：新增场景
      formData.value.id = await ModelApi.createModel(modelData)
      // 提示成功
      message.success(t('process.model.createSuccess'))
    }
    ...
    message.warning(error.message || t('process.model.completeAllSteps'))
  }
}
...
const handleDeploy = async () => {
  try {
    ...
    if (!formData.value.id) {
      await message.confirm(t('process.model.publishConfirm'))
    }
    // 校验所有步骤
    await validateAllSteps()
    ...
    message.success(t('process.model.publishSuccess'))
    // 返回列表页
    await router.push({ name: 'BpmModel' })
  } catch (error: any) {
    console.error('发布失败:', error)
    message.warning(error.message || t('process.model.publishFailed'))
  }
}
...
const handleStepClick = async (index: number) => {
  try {
    ...
    message.warning(t('process.model.completeAllSteps'))
  }
}
