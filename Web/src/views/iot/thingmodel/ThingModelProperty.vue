...
<template>
  <el-form-item
    :rules="[{ required: true, message: t('thing.model.dataTypeRequired'), trigger: 'change' }]"
    :label="t('thing.model.dataType')"
    prop="property.dataType"
  >
    <el-select v-model="property.dataType" :placeholder="t('thing.model.selectDataType')" @change="handleChange">
  ...
  />
  <!-- 布尔型配置 -->
  <el-form-item v-if="property.dataType === IoTDataSpecsDataTypeEnum.BOOL" :label="t('thing.model.boolValue')">
    <template v-for="(item, index) in property.dataSpecsList" :key="item.value">
      <div class="flex items-center justify-start w-1/1 mb-5px">
        ...
        <el-form-item
          ...
          :rules="[
            { required: true, message: t('thing.model.enumDescRequired') },
        ...
        >
          <el-input
            v-model="item.name"
            :placeholder="t('thing.model.boolExample', { value: item.value === 0 ? t('thing.model.boolOff') : t('thing.model.boolOn') })"
            class="w-255px!"
          />
        </el-form-item>
      </div>
  ...
  <el-form-item
    ...
    :label="t('thing.model.dataLength')"
    prop="property.dataSpecs.length"
  >
    <el-input v-model="property.dataSpecs.length" class="w-255px!" :placeholder="t('thing.model.dataLengthPlaceholder')">
      <template #append>{{ t('thing.model.bytes') }}</template>
    </el-input>
  </el-form-item>
  <!-- 时间型配置 -->
  <el-form-item
    v-if="property.dataType === IoTDataSpecsDataTypeEnum.DATE"
    :label="t('thing.model.timeFormat')"
    prop="date"
  >
    <el-input class="w-255px!" disabled :placeholder="t('thing.model.timeFormatPlaceholder')" />
  </el-form-item>
  <!-- 数组型配置-->
  ...
  <ThingModelStructDataSpecs
    ...
    v-model="property.dataSpecsList"
  />
  <el-form-item v-if="!isStructDataSpecs && !isParams" :label="t('thing.model.accessMode')" prop="property.accessMode">
