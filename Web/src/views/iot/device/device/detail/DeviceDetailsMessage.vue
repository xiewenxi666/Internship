...
<template>
  <ContentWrap>
    ...
    <!-- 消息列表 -->
    <el-table v-loading="loading" :data="list" :stripe="true" class="whitespace-nowrap" :table-layout="'auto'">
      <el-table-column label="时间" align="center" prop="ts" min-width="180">
        <template #default="scope">
          {{ formatDate(scope.row.ts) }}
        </template>
      </el-table-column>
      <el-table-column label="上行/下行" align="center" prop="upstream" min-width="140">
        <template #default="scope">
      ...
      <el-table-column label="是否回复" align="center" prop="reply" min-width="140">
        <template #default="scope">
          <dict-tag :type="DICT_TYPE.INFRA_BOOLEAN_STRING" :value="scope.row.reply" />
        </template>
      </el-table-column>
      <el-table-column label="请求编号" align="center" prop="requestId" min-width="300" />
      <el-table-column label="请求方法" align="center" prop="method" min-width="140">
        <template #default="scope">
