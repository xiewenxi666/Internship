  ...
  <el-card class="border border-[var(--el-border-color-light)] rounded-8px" shadow="never">
    <template #header>
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-8px">
          ...
          <span class="text-16px font-600 text-[var(--el-text-color-primary)]">{{ t('rule.scene.actionConfig.title') }}</span>
          <el-tag size="small" type="info">{{ actions.length }} {{ t('rule.scene.actionConfig.actionCount') }}</el-tag>
        </div>
        <div class="flex items-center gap-8px">
          <el-button type="primary" size="small" @click="addAction">
            <Icon icon="ep:plus" />
            {{ t('rule.scene.actionConfig.addAction') }}
          </el-button>
        </div>
      </div>
    </template>
    ...
    <div class="p-0">
      ...
      <div v-if="actions.length === 0">
        <el-empty :description="t('rule.scene.actionConfig.noActionConfig')">
          <el-button type="primary" @click="addAction">
            <Icon icon="ep:plus" />
            {{ t('rule.scene.actionConfig.addFirstAction') }}
          </el-button>
        </el-empty>
      </div>
      ...
      <div v-else class="space-y-24px">
        ...
        >
          ...
          >
            <div class="flex items-center gap-12px">
              <div class="flex items-center gap-8px text-16px font-600 text-blue-700">
                ...
                <span>{{ t('rule.scene.actionConfig.actionIndex') }} {{ index + 1 }}</span>
              </div>
            ...
            <div class="flex items-center gap-8px">
              ...
              >
                <Icon icon="ep:delete" />
                {{ t('rule.scene.actionConfig.delete') }}
              </el-button>
            </div>
          </div>
          ...
          <div class="p-16px space-y-16px">
            <!-- 执行类型选择 -->
            <div class="w-full">
              <el-form-item :label="t('rule.scene.actionConfig.actionType')" required>
                <el-select
                  :model-value="action.type"
                  @update:model-value="(value) => updateActionType(index, value)"
                  @change="(value) => onActionTypeChange(action, value)"
                  :placeholder="t('rule.scene.actionConfig.selectActionType')"
                  class="w-full"
                >
                  <el-option
            ...
            >
              <div class="flex items-center gap-8px mb-8px">
                ...
                <span class="text-14px font-600 text-[var(--el-text-color-primary)]">{{ t('rule.scene.actionConfig.triggerAlert') }}</span>
                <el-tag size="small" type="warning">{{ t('rule.scene.actionConfig.autoExecute') }}</el-tag>
              </div>
              <div class="text-12px text-[var(--el-text-color-secondary)] leading-relaxed">
                {{ t('rule.scene.actionConfig.alertTip') }}
              </div>
            </div>
          </div>
        </div>
      ...
      <div v-if="actions.length > 0" class="text-center py-16px">
        <el-button type="primary" plain @click="addAction">
          <Icon icon="ep:plus" />
          {{ t('rule.scene.actionConfig.continueAddAction') }}
        </el-button>
      </div>
    </div>
  </el-card>
