          ...
          >
            <div class="flex items-center gap-12px">
              <div class="flex items-center gap-8px text-16px font-600 text-green-700">
                ...
                <span>{{ t('rule.scene.deviceTrigger.mainCondition') }}</span>
              </div>
              <el-tag size="small" type="success">{{ t('rule.scene.deviceTrigger.mustSatisfy') }}</el-tag>
            </div>
          </div>
        ...
        >
          <div class="flex items-center gap-12px">
            <div class="flex items-center gap-8px text-16px font-600 text-green-700">
              ...
              <span>{{ t('rule.scene.deviceTrigger.additionalConditionGroups') }}</span>
            </div>
            <el-tag size="small" type="success">{{ t('rule.scene.deviceTrigger.andRelation') }}</el-tag>
            <el-tag size="small" type="info">
              {{ trigger.conditionGroups?.length || 0 }} {{ t('rule.scene.deviceTrigger.subConditionCount') }}
            </el-tag>
          </div>
          <div class="flex items-center gap-8px">
            ...
            >
              <Icon icon="ep:plus" />
              {{ t('rule.scene.deviceTrigger.addSubConditionGroup') }}
            </el-button>
            <el-button type="danger" size="small" text @click="removeConditionGroup">
              <Icon icon="ep:delete" />
              {{ t('rule.scene.deviceTrigger.deleteConditionGroup') }}
            </el-button>
          </div>
        </div>
                ...
                >
                  <div class="flex items-center gap-12px">
                    <div class="flex items-center gap-8px text-16px font-600 text-orange-700">
                      ...
                      <span>{{ t('rule.scene.deviceTrigger.subConditionGroupIndex') }} {{ subGroupIndex + 1 }}</span>
                    </div>
                    <el-tag size="small" type="warning" class="font-500">{{ t('rule.scene.deviceTrigger.innerConditionAndRelation') }}</el-tag>
                    <el-tag size="small" type="info"> {{ subGroup?.length || 0 }}{{ t('rule.scene.deviceTrigger.conditionCount') }} </el-tag>
                  </div>
                  <el-button
                    type="danger"
                    size="small"
                  ...
                  >
                    <Icon icon="ep:delete" />
                    {{ t('rule.scene.deviceTrigger.deleteGroup') }}
                  </el-button>
                </div>
              ...
              >
                <div class="flex items-center gap-8px">
                  ...
                  <div class="px-16px py-6px bg-orange-100 border-2 border-orange-300 rounded-full">
                    <span class="text-14px font-600 text-orange-600">{{ t('rule.scene.deviceTrigger.or') }}</span>
                  </div>
                  <!-- 连接线 -->
                  <div class="w-32px h-1px bg-orange-300"></div>
                </div>
        ...
        >
          <div class="flex flex-col items-center gap-12px">
            ...
            <div class="text-orange-600">
              <p class="text-14px font-500 mb-4px">{{ t('rule.scene.deviceTrigger.noSubConditionGroup') }}</p>
              <p class="text-12px">{{ t('rule.scene.deviceTrigger.addSubConditionGroupTip') }}</p>
            </div>
          </div>
        </div>
      </div>
