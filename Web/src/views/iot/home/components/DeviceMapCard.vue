  ...
  <el-card class="chart-card" shadow="never">
    <template #header>
      <div class="flex items-center justify-between">
        <span class="text-base font-medium text-gray-600">{{ t('home.deviceDistributionMap') }}</span>
    ...
    <div v-else-if="!hasData" class="h-[500px] flex justify-center items-center">
      <el-empty :description="t('home.noLocationData')" />
    </div>
    ...
    const infoContent = `
      <div style="padding: 8px; min-width: 180px;">
        ...
        <div style="color: #666; font-size: 12px; line-height: 1.8;">
          <div>${t('home.product')}: ${device.productName || '-'}</div>
          <div>${t('home.status')}: <span style="color: ${config.color}; font-weight: 500;">${config.name}</span></div>
        </div>
        <div style="margin-top: 8px; padding-top: 8px; border-top: 1px solid #eee;">
          <a href="javascript:void(0)" style="color: #409EFF; font-size: 12px; text-decoration: none;">${t('home.clickToViewDetails')} →</a>
        </div>
      </div>
    `
