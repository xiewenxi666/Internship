<template>
  <Dialog :title="dialogTitle" v-model="dialogVisible">
    ...
    >
      <el-row :gutter="20">
        <el-col :span="12">
          <el-form-item :label="t('signIn.config.day')" prop="day">
            <el-input-number v-model="formData.day" :min="1" :max="7" :precision="0" />
            <el-text class="mx-1" style="margin-left: 10px" type="danger">
              {{ t('signIn.config.dayTip') }}
            </el-text>
          </el-form-item>
        </el-col>
        <el-col :span="12">
          <el-form-item :label="t('signIn.config.point')" prop="point">
      ...
      <el-row :gutter="20">
        <el-col :span="12">
          <el-form-item :label="t('signIn.config.experience')" prop="experience">
            <el-input-number v-model="formData.experience" :min="0" :precision="0" />
          </el-form-item>
        </el-col>
        <el-col :span="12">
          <el-form-item :label="t('signIn.config.status')" prop="status">
...
const awardValidator = (rule: any, _value: any, callback: any) => {
  if (!formData.value.point && !formData.value.experience) {
    callback(new Error(t('signIn.config.awardRequired')))
    return
  }
...
const formRules = reactive({
  ...
  experience: [
    { required: true, message: t('signIn.config.experienceRequired'), trigger: 'blur' },
    { validator: awardValidator, trigger: 'blur' }
  ]
})
