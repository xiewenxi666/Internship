...
<template>
  <el-tabs type="border-card" v-model="currentTab">
    ...
    <el-tab-pane :name="ReplyType.Text">
      <template #label>
        <el-row align="middle"><Icon icon="ep:document" /> {{ t('reply.text') }}</el-row>
      </template>
    ...
    <el-tab-pane :name="ReplyType.Image">
      <template #label>
        <el-row align="middle"><Icon icon="ep:picture" class="mr-5px" /> {{ t('reply.image') }}</el-row>
      </template>
    ...
    <el-tab-pane :name="ReplyType.Voice">
      <template #label>
        <el-row align="middle"><Icon icon="ep:phone" /> {{ t('reply.voice') }}</el-row>
      </template>
    ...
    <el-tab-pane :name="ReplyType.Video">
      <template #label>
        <el-row align="middle"><Icon icon="ep:share" /> {{ t('reply.video') }}</el-row>
      </template>
    ...
    <el-tab-pane :name="ReplyType.News">
      <template #label>
        <el-row align="middle"><Icon icon="ep:reading" /> {{ t('reply.news') }}</el-row>
      </template>
    ...
    <el-tab-pane :name="ReplyType.Music">
      <template #label>
        <el-row align="middle"><Icon icon="ep:service" />{{ t('reply.music') }}</el-row>
      </template>
