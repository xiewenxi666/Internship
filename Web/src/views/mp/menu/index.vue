...
const onDeleteMenu = async () => {
  try {
    await message.confirm(t('common.delMessage'))
...
const onSave = async () => {
  try {
    await message.confirm(t('menu.confirmSave'))
    loading.value = true
    await MpMenuApi.saveMenu(accountId.value, menuListToBackend())
    getList()
    message.notifySuccess(t('menu.publishSuccess'))
  } finally {
    loading.value = false
  }
}
...
const onClear = async () => {
  try {
    ...
    handleQuery()
    message.notifySuccess(t('menu.clearSuccess'))
  } finally {
    loading.value = false
  }
}
