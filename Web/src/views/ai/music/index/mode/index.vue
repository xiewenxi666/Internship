<template>
  <ContentWrap class="w-300px h-full mb-[0!important]">
    <el-radio-group v-model="generateMode" class="mb-15px">
      <el-radio-button value="desc"> {{ t('music.descMode') }} </el-radio-button>
      <el-radio-button value="lyric"> {{ t('music.lyricMode') }} </el-radio-button>
    </el-radio-group>

    <!-- 描述模式/歌词模式 切换 -->
    <component :is="generateMode === 'desc' ? desc : lyric" ref="modeRef" />

    <el-button type="primary" round class="w-full" @click="generateMusic"> {{ t('music.createMusic') }} </el-button>
  </ContentWrap>
</template>
