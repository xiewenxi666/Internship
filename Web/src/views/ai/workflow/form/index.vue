    ...
    <div class="mx-auto">
      ...
      >
        ...
        <div class="w-200px flex items-center overflow-hidden">
          ...
          <span class="ml-10px text-16px truncate" :title="formData.name || tAi('workflow.createProcess')">
            {{ formData.name || tAi('workflow.createProcess') }}
          </span>
        </div>
        ...
        <div class="w-200px flex items-center justify-end gap-2">
          <el-button type="primary" @click="handleSave"> {{ tAi('workflow.save') }} </el-button>
        </div>
      </div>
...
const steps = computed(() => [
  { title: tAi('workflow.basicInfo'), validator: validateBasic },
  { title: tAi('workflow.workflowDesign'), validator: validateWorkflow }
])
...
const validateAllSteps = async () => {
  try {
    ...
    try {
      ...
      currentStep.value = 0
      throw new Error(tAi('workflow.completeBasicInfo'))
    }
    ...
    try {
      ...
      currentStep.value = 1
      throw new Error(tAi('workflow.completeWorkflowInfo'))
    }
    return true
  } catch (error) {
    throw error
...
const handleSave = async () => {
  try {
    ...
    // 保存成功，提示并跳转到列表页
    message.success(tAi('workflow.saveSuccess'))
    delView(unref(router.currentRoute))
    await router.push({ name: 'AiWorkflow' })
  } catch (error: any) {
    console.error('保存失败:', error)
    message.warning(error.message || tAi('workflow.completeAllSteps'))
  }
}
...
const handleStepClick = async (index: number) => {
  try {
    ...
    message.warning(tAi('workflow.completeCurrentStep'))
  }
}
