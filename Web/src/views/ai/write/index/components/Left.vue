    ...
    >
      <div>
        <template v-if="selectedTab === 1">
          <ReuseLabel :hint-click="() => example('write')" :hint="t('write.example')" :label="t('write.writeContent')" />
          <el-input
            v-model="formData.prompt"
            :maxlength="500"
            :rows="5"
            :placeholder="t('write.pleaseInputWriteContent')"
            showWordLimit
            type="textarea"
          />
        </template>
        ...
        <template v-else>
          <ReuseLabel :hint-click="() => example('reply')" :hint="t('write.example')" :label="t('write.originalContent')" />
          <el-input
            v-model="formData.originalContent"
            :maxlength="500"
            :rows="5"
            :placeholder="t('write.pleaseInputOriginal')"
            showWordLimit
            type="textarea"
          />

          <ReuseLabel :label="t('write.replyContent')" />
          <el-input
            v-model="formData.prompt"
            :maxlength="500"
            :rows="5"
            :placeholder="t('write.pleaseInputReplyContent')"
            showWordLimit
            type="textarea"
          />
        </template>

        <ReuseLabel :label="t('write.length')" />
        <Tag v-model="formData.length" :tags="getIntDictOptions(DICT_TYPE.AI_WRITE_LENGTH)" />
        <ReuseLabel :label="t('write.format')" />
        <Tag v-model="formData.format" :tags="getIntDictOptions(DICT_TYPE.AI_WRITE_FORMAT)" />
        <ReuseLabel :label="t('write.tone')" />
        <Tag v-model="formData.tone" :tags="getIntDictOptions(DICT_TYPE.AI_WRITE_TONE)" />
        <ReuseLabel :label="t('write.language')" />
        ...
        <div class="flex items-center justify-center mt-3">
          ...
          <el-button :loading="isWriting" color="#846af7" @click="submit">{{ t('write.generate') }}</el-button>
        </div>
      </div>
    </div>
  </div>
...
const tabs = computed(() => [
  { text: t('write.writeTab'), value: AiWriteTypeEnum.WRITING },
  { text: t('write.replyTab'), value: AiWriteTypeEnum.REPLY }
])
...
const submit = () => {
  if (selectedTab.value === 2 && !formData.value.originalContent) {
    message.warning(t('write.pleaseInputOriginalContent'))
    return
  }
  if (!formData.value.prompt) {
    message.warning(`${t('write.pleaseInputWriteContent')}`)
    return
  }
  emits('submit', {
