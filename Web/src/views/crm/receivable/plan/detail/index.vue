<template>
  ...
  <el-col>
    <el-tabs>
      <el-tab-pane :label="t('receivablePlan.basicInfoTab')">
        <ReceivablePlanDetailsInfo :receivable-plan="receivablePlan" />
      </el-tab-pane>
      <el-tab-pane :label="t('receivablePlan.operateLogTab')">
        <OperateLogV2 :log-list="logList" />
      </el-tab-pane>
      <el-tab-pane :label="t('receivablePlan.teamMemberTab')">
        <PermissionList
...
onMounted(async () => {
  if (!params.id) {
    message.warning(t('receivablePlan.paramError'))
    close()
    return
  }
