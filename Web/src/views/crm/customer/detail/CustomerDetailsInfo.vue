...
const loadTradeStats = async () => {
  ...
  try {
    const { getReceivablePage } = await import('@/api/crm/receivable')
    const rData = await getReceivablePage({ customerId: customer.id, pageSize: 100 })
    ...
    const { getContractPage } = await import('@/api/crm/contract')
    const cData = await getContractPage({ customerId: customer.id, pageSize: 1 })
