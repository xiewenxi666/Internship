<template>
  ...
  <ContentWrap class="mt-10px">
    ...
    >
      ...
      <el-table-column align="center" fixed="left" :label="t('crm.contact.name')" prop="name">
      ...
      <el-table-column align="center" :label="t('crm.contact.mobile')" prop="mobile" />
      <el-table-column align="center" :label="t('crm.contact.post')" prop="post" />
      <el-table-column align="center" :label="t('crm.contact.parentName')" prop="parentName" />
      <el-table-column align="center" :label="t('crm.contact.master')" min-width="100" prop="master">
        <template #default="scope">
...
const createContactBusinessList = async (contactIds: number[]) => {
  ...
  // 刷新列表
  message.success(t('crm.contact.relateSuccess'))
  handleQuery()
}
...
const deleteContactBusinessList = async () => {
  ...
  if (data.contactIds.length === 0) {
    return message.error(t('crm.contact.noSelectContact'))
  }
  await ContactApi.deleteContactBusinessList2(data)
  // 刷新列表
  message.success(t('crm.contact.unrelateSuccess'))
  handleQuery()
}
