              ...
              >
                <template #append>
                  ...
                  <span v-if="mobileCodeTimer > 0" class="getMobileCode" style="cursor: pointer">
                    {{ mobileCodeTimer }}{{ t('login.resendCode') }}
                  </span>
                </template>
              </el-input>
              <!-- </el-button> -->
...
const validatePass2 = (_rule, value, callback) => {
  if (value === '') {
    callback(new Error(t('login.pleaseReEnterPassword')))
...
const rules = {
  tenantName: [{ required: true, min: 2, max: 20, trigger: 'blur', message: t('login.tenantLength') }],
  mobile: [{ required: true, min: 11, max: 11, trigger: 'blur', message: t('login.mobileLength') }],
  password: [
    {
      required: true,
      min: 4,
      ...
      trigger: 'blur',
      message: t('login.passwordLength4To16')
    }
  ],
