package com.meession.etm.module.infra.service.codegen.inner;
...
import com.meession.etm.framework.apilog.core.enums.OperateTypeEnum;
...
import com.meession.etm.module.infra.enums.codegen.CodegenVOTypeEnum;
...
public class CodegenEngine {
    ...
    /**
     * 是否为 mitedtsm-cloud 项目，用于解决 Boot 和 Cloud 的 api 模块兼容性问题
     *
     * true  - 需要有 mitedtsm-module-xxx-api 模块
    ...
    public CodegenEngine() {
        ...
        this.cloudEnable = ClassUtils.isPresent("com.meession.etm.module.infra.framework.rpc.config.RpcConfiguration",
    ...
    private Map<String, String> getTemplates(Integer frontType) {
        ...
        if (Boolean.FALSE.equals(cloudEnable)) {
            SERVER_TEMPLATES.forEach((templatePath, filePath) -> {
                filePath = StrUtil.replace(filePath, "/mitedtsm-module-${table.moduleName}-api", "");
                filePath = StrUtil.replace(filePath, "/mitedtsm-module-${table.moduleName}-server", "");
    ...
    private static String javaModuleFilePath(String path, String module, String src) {
        return "mitedtsm-module-${table.moduleName}/" + // 顶级模块
                "mitedtsm-module-${table.moduleName}-" + module + "/" + // 子模块
                "src/" + src + "/java/${basePackage}/module/${table.moduleName}/" + path + ".java";
    }

    private static String mapperXmlFilePath() {
        return "mitedtsm-module-${table.moduleName}/" + // 顶级模块
                "mitedtsm-module-${table.moduleName}-server/" + // 子模块
    ...
    private static String vueFilePath(String path) {
        return "mitedtsm-ui-${sceneEnum.basePackage}-vue2/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3FilePath(String path) {
        return "mitedtsm-ui-${sceneEnum.basePackage}-vue3/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3UniappFilePath(String path) {
        return "mitedtsm-ui-${sceneEnum.basePackage}-uniapp/" + // 顶级目录
                "src/" + path;
    }

    private static String vue3VbenFilePath(String path) {
        return "mitedtsm-ui-${sceneEnum.basePackage}-vben/" + // 顶级目录
                "src/" + path;
    }
