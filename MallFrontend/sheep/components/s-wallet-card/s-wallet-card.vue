	...
	<view class="ss-wallet-menu-wrap ss-flex ss-col-center" :style="[bgStyle, { marginLeft: `${data.space}px` }]">
		<view class="menu-item ss-flex-1 ss-flex-col ss-row-center ss-col-center"
			...
			<view class="value-box ss-flex ss-col-bottom">
				<view class="value-text ss-line-1">{{ fen2yuan(userWallet.balance, false) || '0.00' }}</view>
				<view class="unit-text ss-m-l-6">{{ t('unitYuan') }}</view>
			</view>
			<view class="menu-title ss-m-t-28">{{ t('accountBalance') }}</view>
		</view>
		<view class="menu-item ss-flex-1 ss-flex-col ss-row-center ss-col-center"
			@tap="sheep.$router.go('/pages/user/wallet/score')">
			<view class="value-box ss-flex ss-col-bottom">
				<view class="value-text">{{ userInfo.point || 0 }}</view>
				<view class="unit-text ss-m-l-6">{{ t('unitPcs') }}</view>
			</view>
			<view class="menu-title ss-m-t-28">{{ t('point') }}</view>
		</view>
      ...
      ">
			<view class="value-box ss-flex ss-col-bottom">
				...
				<view class="unit-text ss-m-l-6">{{ t('unitSheet') }}</view>
			</view>
			<view class="menu-title ss-m-t-28">{{ t('coupon.title') }}</view>
		</view>
		<view class="menu-item ss-flex-col ss-row-center ss-col-center menu-wallet"
          @tap="sheep.$router.go('/pages/user/wallet/money')">
			...
			<view class="menu-title ss-m-t-30">{{ t('myWallet') }}</view>
		</view>
	</view>
</template>
