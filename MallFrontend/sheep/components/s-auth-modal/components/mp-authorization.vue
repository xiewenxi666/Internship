  ...
  <view>
    ...
    <view class="head-box ss-m-b-60 ss-flex-col">
      <view class="ss-flex ss-m-b-20">
        <view class="head-title ss-m-r-40 head-title-animation">{{ t('authorizationInfo') }}</view>
      </view>
      <view class="head-subtitle">{{ t('completeProfileTip') }}</view>
    </view>
    ...
    >
      ...
      <uni-forms-item name="avatar" :label="t('avatar')">
        <button
      ...
      <view class="foot-box">
        <button class="ss-reset-button authorization-btn" @tap="onConfirm"> {{ t('confirmAuthorization') }} </button>
      </view>
    </uni-forms>
  </view>
</template>
...
<script setup>
  ...
  async function onConfirm() {
    ...
    if (!nickname) {
      sheep.$helper.toast(t('pleaseEnterNickname'));
      return;
    }
    if (!avatar) {
      sheep.$helper.toast(t('pleaseSelectAvatar'));
      return;
    }
    // 发起更新
    ...
    // 更新成功
    if (code === 0) {
      sheep.$helper.toast(t('authorizationSuccess'));
