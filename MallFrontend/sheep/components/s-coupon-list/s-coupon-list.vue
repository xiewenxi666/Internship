<template>
  <view class="ss-m-20" :style="{ opacity: disabled ? '0.5' : '1' }">
    <view class="content">
      ...
      >
        {{ data.discountType === 1 ? t('fullReduce') : t('discount') }}
      </view>
      <view class="title ss-m-x-30 ss-p-t-18">
        <view class="ss-flex ss-row-between">
          ...
          <view>
            ...
            >
              <view class="value-reduce ss-m-b-10" v-if="data.discountType === 1">{{ getCurrencyCode() }}</view>
        ...
        <view class="ss-flex ss-row-between ss-m-t-16">
          ...
          >
            {{ t('validDaysTip', { days: data.fixedEndTerm }) }}
          </view>
          <view class="sellby-text" :class="isDisable ? 'disabled-color' : 'subtitle-color'" v-else>
            {{ t('validDateTip', { start: sheep.$helper.timeFormat(data.validStartTime, 'yyyy-mm-dd'), end: sheep.$helper.timeFormat(data.validEndTime, 'yyyy-mm-dd') }) }}
          </view>
          <view class="value-enough" :class="isDisable ? 'disabled-color' : 'subtitle-color'">
            {{ t('minAmount', { amount: fen2yuan(data.usePrice) }) }}
          </view>
        </view>
      </view>
    </view>
