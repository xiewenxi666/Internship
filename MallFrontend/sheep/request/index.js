...
import { t, getLocaleValue } from '@/sheep/i18n';
...
http.interceptors.request.use(
  (config) => {
    ...
    config.header['Accept-Language'] = getLocaleValue();
    return config;
  },
  (error) => {
  ...
  (response) => {
    ...
    if (response.data.code !== 0) {
      ...
      if ((response.data.code + '').includes('1011007')) {
        ...
        // 错误提示
        uni.showToast({
          title: response.data.msg || t('error.serverBusy'),
          icon: 'none',
          mask: true,
        });
      }
  ...
  (error) => {
    ...
    if (error !== undefined) {
      switch (error.statusCode) {
        case 400:
          errorMessage = t('error.requestError');
          break;
        case 401:
          errorMessage = isLogin ? t('http.loginExpired') : t('http.pleaseLogin');
          // 正常情况下，后端不会返回 401 错误，所以这里不处理 handleAuthorized
          break;
        case 403:
          errorMessage = t('error.accessDenied');
          break;
        case 404:
          errorMessage = t('error.requestError');
          break;
        case 408:
          ...
          break;
        case 429:
          errorMessage = t('error.requestFrequent');
          break;
        case 500:
          errorMessage = t('error.serverBusy');
          break;
        case 501:
          errorMessage = t('error.serviceNotImplemented');
          break;
        case 502:
          errorMessage = t('error.networkErrorShort');
          break;
        case 503:
          errorMessage = t('error.serviceUnavailable');
          break;
        case 504:
          errorMessage = t('error.networkTimeout');
          break;
        case 505:
          errorMessage = t('error.httpNotSupported');
          break;
      }
      if (error.errMsg.includes('timeout')) errorMessage = t('error.requestTimeout');
      // #ifdef H5
      if (error.errMsg.includes('Network'))
        errorMessage = window.navigator.onLine ? t('error.serverAbnormal') : t('error.checkNetwork');
      // #endif
    }
...
const handleAuthorized = () => {
  ...
  return Promise.reject({
    code: 401,
    msg: userStore.isLogin ? t('http.loginExpired') : t('http.pleaseLogin'),
  });
};
