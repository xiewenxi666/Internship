  ...
  <s-layout :title="t('cashier')">
    <view class="bg-white ss-modal-box ss-flex-col">
      ...
      <view class="modal-content ss-flex-1">
        <view class="pay-title ss-p-l-30 ss-m-y-30">{{ t('selectPayMethod') }}</view>
            ...
            >
              ...
              <view class="check-box ss-flex ss-col-center ss-p-l-10">
                <view class="userInfo-money ss-m-r-10" v-if="item.value === 'wallet'">
                  {{ t('balanceLabel') }}: {{ fen2yuan(userWallet.balance) }}{{ t('yuan', {}, { namespace: 'common' }) }}
                </view>
                <radio
                  :value="item.value"
...
<script setup>
  ...
  const onPay = () => {
    if (state.payment === '') {
      sheep.$helper.toast(t('payMethodRequired'));
      return;
    }
    if (state.payment === 'wallet') {
      uni.showModal({
        title: t('confirmPayTitle'),
        content: t('confirmPayContent'),
  ...
  const payDescText = computed(() => {
    ...
    if (state.payStatus === 1) {
      ...
      return `${t('remainingTime')} ${time.h}:${time.m}:${time.s} `;
    }
    if (state.payStatus === -2) {
