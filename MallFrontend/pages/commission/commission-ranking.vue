<!-- 佣金排行榜 -->
<template>
  <s-layout :title="t('commissionRanking')">
    <view class="commission-rank">
      <view class="header">
        <view class="rank" v-if="state.position">
          {{ t('yourCurrentRank') }}
          <text class="num">{{ state.position }}</text>
        </view>
        <view class="rank" v-else>{{ t('yourCurrentNoRank') }}</view>
      </view>
      <view class="wrapper">
...
<script setup>
  ...
  onReady(() => {
    setPageTitle('pages/commission/commission-ranking');
  });

  const tabMaps = computed(() => [t('weekRank'), t('monthRank')]);
