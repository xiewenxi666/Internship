<!-- 分佣提现 -->
<template>
  <s-layout :title="t('withdrawTitle')" class="withdraw-wrap" navbar="inner">
    ...
    >
      <view class="">
        <view class="num-title">{{ t('availableWithdrawAmount') }}</view>
    ...
    <view class="draw-card">
      <view class="bank-box ss-flex ss-col-center ss-row-between ss-m-b-30">
        <view class="name">{{ t('withdrawTo') }}</view>
        <view class="bank-list ss-flex ss-col-center" @tap="onAccountSelect(true)">
          <view v-if="!state.accountInfo.type" class="empty-text">{{ t('pleaseSelectMethod') }}</view>
          <view v-if="state.accountInfo.type === '1'" class="empty-text">{{ t('walletBalance') }}</view>
          <view v-if="state.accountInfo.type === '2'" class="empty-text">{{ t('bankTransfer') }}</view>
          <view v-if="state.accountInfo.type === '3'" class="empty-text">{{ t('wechatAccount') }}</view>
          <view v-if="state.accountInfo.type === '4'" class="empty-text">{{ t('alipayAccount') }}</view>
          <view v-if="state.accountInfo.type === '5'" class="empty-text">{{ t('wechatMoney') }}</view>
          <text class="cicon-forward" />
        </view>
      </view>
      <!-- 提现金额 -->
      <view class="card-title">{{ t('withdrawAmount') }}</view>
      <view class="input-box ss-flex ss-col-center border-bottom">
        <view class="unit">{{ getCurrencyCode() }}</view>
        <uni-easyinput
          :inputBorder="false"
      ...
      >
        ...
        <uni-easyinput
          ...
          :placeholder="t('pleaseEnterWithdrawAccount')"
        />
      </view>
      <!-- 收款码 -->
      <view class="card-title" v-show="['3', '4'].includes(state.accountInfo.type)">{{ t('qrCode') }}</view>
      <view
      ...
      <!-- 提现银行 -->
      <view class="card-title" v-show="state.accountInfo.type === '2'">{{ t('withdrawBank') }}</view>
      <view
        class="input-box ss-flex ss-col-center border-bottom"
      ...
      <!-- 开户地址 -->
      <view class="card-title" v-show="state.accountInfo.type === '2'">{{ t('bankAddress') }}</view>
      <view
        class="input-box ss-flex ss-col-center border-bottom"
        v-show="state.accountInfo.type === '2'"
      >
        ...
        <uni-easyinput
          ...
          :placeholder="t('pleaseEnterBankAddress')"
        />
      </view>
    ...
    <view class="draw-notice">
      <view class="title ss-m-b-30">{{ t('withdrawNotice') }}</view>
      <view class="draw-list"> {{ t('minWithdrawAmount') }} {{ fen2yuan(state.minPrice) }} {{ t('unitYuan') }} </view>
      <view class="draw-list">
        {{ t('frozenCommissionNotice') }}：<text>{{ fen2yuan(state.brokerageInfo.frozenPrice) }}</text>
        （{{ t('frozenDaysNoticePrefix') }} {{ state.frozenDays }} {{ t('frozenDaysNotice') }}）
      </view>
    </view>
...
<script setup>
  ...
  onReady(() => {
    setPageTitle('pages/commission/withdraw');
  });
  ...
  const onConfirm = async () => {
    ...
    if (
      ...
      sheep.$helper.toast(t('pleaseEnterCorrectAmount'));
      return;
    }
    if (!state.accountInfo.type) {
      sheep.$helper.toast(t('pleaseSelectWithdrawMethod'));
      return;
    }
    let openid;
    ...
    uni.showModal({
      ...
      content: t('withdrawSubmitted'),
      cancelText: t('continueWithdraw'),
      confirmText: t('viewRecord'),
      success: (res) => {
        if (res.confirm) {
