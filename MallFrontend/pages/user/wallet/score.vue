<!-- 我的积分 -->
<template>
  <s-layout class="wallet-wrap" :title="t('myPoints')" navbar="inner">
    <view
    ...
    >
      ...
      <view class="score-box ss-flex-col ss-row-center ss-col-center">
        <view class="ss-m-b-30">
          <text class="all-title ss-m-r-8">{{ t('currentPoints') }}</text>
        </view>
    ...
    <view class="list-box">
      ...
      <s-empty v-else :text="t('noData')" icon="/static/data-empty.png" />
    </view>
...
<script setup>
  ...
  async function getLogList() {
    ...
    let { code, data } = await PointApi.getPointRecordPage({
      ...
      addStatus: state.currentTab > 0 ? tabMaps.value[state.currentTab].value : undefined,
