<!-- 物流追踪 -->
<template>
  <s-layout :title="t('logisticsTracking')">
    <view class="log-wrap">
      <!-- 商品信息 -->
      <view class="log-card ss-flex ss-m-20 ss-r-10" v-if="goodsImages.length > 0">
        ...
        <view class="log-card-msg">
          ...
          <!--          </view>-->
          <view class="ss-m-b-8">{{ t('expressNo') }}{{ state.info.logisticsNo }}</view>
          <view>{{ t('expressCompany') }}{{ state.info.logisticsName }}</view>
        </view>
      </view>
...
<script setup>
  ...
  onReady(() => {
    setPageTitle('pages/order/express/log');
  });
  ...
  function handleClick(data) {
    ...
    if (platform === 'devtools') {
      uni.showToast({ title: t('realDeviceOnly'), icon: 'none' });
      handleCopy(phoneNumber);
      return;
    }

    if (platform === 'wechat') {
      uni.showToast({ title: t('pleaseDialManually'), icon: 'none' });
      handleCopy(phoneNumber);
      return;
    }
    ...
    uni.makePhoneCall({
      ...
      fail: (err) => {
        ...
        uni.showToast({ title: t('dialFailed'), icon: 'none' });
        handleCopy(phoneNumber);
      },
    });
  }
  function handleCopy(text) {
    uni.setClipboardData({
      data: text,
      success: () => {
        uni.showToast({ title: t('copySuccess'), icon: 'success' });
      },
      fail: () => {
