  ...
  >
    ...
    >
      <view class="filter-list-box">
        <view
          class="filter-item"
          v-for="(item, index) in tabList[state.currentTab].list"
          :key="item.value"
    ...
    <uni-load-more
      ...
      @tap="loadMore"
    />
    <s-empty v-if="state.pagination.total === 0" icon="/static/soldout-empty.png" :text="t('noGoods')" />
  </s-layout>
</template>
...
<script setup>
  ...
  const onFilterItem = (val) => {
    ...
    if (
      state.currentSort === tabList.value[0].list[val].sort &&
      state.currentOrder === tabList.value[0].list[val].order
    ) {
      state.showFilter = false;
      return;
    }
