    ...
    <view v-if="state.data && !state.loading">
      ...
      >
        ...
        >
          <template #groupon>
            <view class="ss-flex">
              <view class="sales-title">{{ t('grouponPeople', { count: state.data.headRecord.userSize }) }}</view>
              <view class="num-title ss-m-l-20">{{ t('grouponSoldCount', { count: state.data.headRecord.userCount }) }}</view>
            </view>
          </template>
        </s-goods-item>
      </view>
      ...
      <view class="countdown-box detail-card ss-p-t-44 ss-flex-col ss-col-center">
        ...
        <view v-if="state.data.headRecord.status === 2">
          <view class="countdown-title ss-flex">
            ...
            {{ state.data.orderId ? t('grouponTimeoutRefund') : t('grouponDisbanded') }}
          </view>
        </view>
        ...
        <view v-if="state.data.headRecord.status === 0">
          <view v-if="state.data.headRecord.expireTime <= new Date().getTime()">
            <view class="countdown-title ss-flex">
              <text class="cicon-info"></text>
              {{ t('grouponEndedNotice') }}
            </view>
          </view>
          <view class="countdown-title ss-flex" v-else>
            {{ t('grouponStillNeed') }}
            <view class="num"
              >{{ state.data.headRecord.userSize - state.data.headRecord.userCount }}{{ t('people', {}, 'common') }}</view
            >
            {{ t('grouponPeopleSuccess') }}
        ...
        <view class="ss-m-t-60 ss-flex ss-flex-wrap ss-row-center">
          ...
          <view class="header-avatar ss-m-r-24 ss-m-b-20">
            ...
            <view class="header-tag ss-flex ss-col-center ss-row-center">{{ t('grouponLeader') }}</view>
          </view>
          <!-- 团员 -->
          <view
      ...
      >
        ...
        <button class="ss-reset-button join-btn" v-else @tap="onCreateGroupon"> {{ t('grouponCreateGroup') }} </button>
      </view>
...
<script setup>
  ...
  onReady(() => {
    setPageTitle('pages/activity/groupon/detail');
  });
